...
Pour tester **l'environnement Cloudflare complet** (Worker, `/admin`, analytics first-party, Durable Object SQLite et proxy Vigilance), utiliser simplement :

```bash
npm run cloudflare
```

La commande construit `dist/`, complète automatiquement `.dev.vars` avec des secrets locaux aléatoires s'ils sont absents, puis npm lance Wrangler en mode local sur `http://localhost:8787` via le shell natif du système (compatible Windows/macOS/Linux). Le mot de passe admin local généré est affiché dans le terminal et conservé dans `.dev.vars`. Le dashboard est accessible sur l'URL canonique `http://localhost:8787/admin` (ou `http://127.0.0.1:8787/admin`). Les données du Durable Object sont persistées dans `.wrangler/state` et restent totalement séparées de la production. La route admin est explicitement exclue du cache PWA. La clé `METEOFRANCE_API_KEY` reste facultative et doit être ajoutée manuellement dans `.dev.vars` pour tester la Vigilance officielle.

Le serveur de prévisualisation statique `npm run preview` écoute par défaut sur `http://127.0.0.1:4173` et reproduit la résolution des fichiers HTML sans extension (`/meteo/toulouse` → `dist/meteo/toulouse.html`). Il renvoie aussi `dist/404.html` avec un vrai statut HTTP 404 pour les URL inconnues, comme la configuration Cloudflare de production. Il ne lance pas le Worker ni le stockage analytics. Le tracking reste volontairement désactivé sur ce port. Un serveur statique basique comme `python3 -m http.server` ne réalise pas cette résolution et peut donc répondre 404 sur ces URL propres, même si le build est correct.

Le build, la prévisualisation statique et les tests utilisent uniquement Node.js (`tools/build-site.mjs`, `tools/preview-site.mjs` et `tools/run-tests.mjs`). `npm run cloudflare` utilise Wrangler via `npx` afin d'exécuter le runtime Cloudflare local. `npm run tests` découvre récursivement les fichiers `tests/<fonctionnalité>/<portée>/*.test.mjs`, les exécute dans un ordre stable et retourne un code d'erreur si au moins une suite échoue. Les suites peuvent aussi être filtrées par fonctionnalité, portée ou nom de fichier via `tools/run-tests.mjs`. Avant une release, `npm run audit:release` vérifie en plus la syntaxe de toutes les sources JavaScript, les liens de documentation et les artefacts interdits, exécute toute la suite, construit `dist/` puis contrôle son contenu public. En mode Cloudflare local, le tracking est autorisé uniquement sur `localhost:8787` / `127.0.0.1:8787`; en production, il reste limité à `meteocompare.app` et `www.meteocompare.app` en HTTPS.
