...
Le serveur de prévisualisation statique `npm run preview` écoute par défaut sur `http://127.0.0.1:4173` et reproduit la résolution des fichiers HTML sans extension (`/meteo/toulouse` → `dist/meteo/toulouse.html`). Il ne lance pas le Worker ni le stockage analytics. Le tracking reste volontairement désactivé sur ce port. Un serveur statique basique comme `python3 -m http.server` ne réalise pas cette résolution et peut donc répondre 404 sur ces URL propres, même si le build est correct.
