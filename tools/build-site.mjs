...
function cityLink(city){
  return `<a class="seo-city-link" href="${html(cityPublicPath(city))}"><strong>${html(city.name)}</strong><span>${html(city.department || city.region)}</span><span aria-hidden="true">→</span></a>`;
}
function rootPrerender(){
  const links=SEO_CITIES.slice(0,40).map(cityLink).join('');
  return `<main class="page home-page seo-prerender"><section class="home-hero"><div class="home-hero-copy"><span class="home-hero-kicker">Comparaison multi-modèles</span><h1>Comparez les modèles météo et leur convergence</h1><p>MeteoCompare rassemble plusieurs prévisions météo pour rendre visibles les accords, les écarts et l’évolution des modèles sans masquer leurs valeurs brutes.</p></div></section><section class="section-card seo-prerender-copy"><div class="section-head"><div><h2>Une prévision météo qui montre aussi l’incertitude</h2><p>Consultez la température, la pluie et le vent, puis comparez la dispersion entre modèles et leur convergence. Les prévisions actualisées se chargent dans votre navigateur.</p></div></div></section><section class="section-card seo-directory"><div class="section-head"><div><span class="home-section-kicker">Explorer</span><h2>Prévisions météo par ville</h2><p>Accédez directement aux comparaisons multi-modèles des principales villes françaises.</p></div></div><div class="seo-link-grid">${links}</div></section></main>`;
}
