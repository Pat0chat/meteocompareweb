...
Les Durable Objects `AnalyticsStore` et `VigilanceCache` utilisent SQLite et sont déclarés dans `wrangler.jsonc`. `VigilanceCache` fournit le niveau L2 partagé du cache Vigilance afin qu’un miss sur un PoP Cloudflare n’entraîne pas automatiquement un nouvel appel Météo-France. `AnalyticsStore` stocke également l’historique des sondes de services. `wrangler.jsonc` déclare un Cron Trigger `*/30 * * * *` afin d’exécuter ces sondes toutes les 30 minutes même lorsque l’administration n’est pas ouverte. `ANALYTICS_HASH_SECRET` est obligatoire pour l’ingestion analytics et n’est jamais remplacé par le secret de session admin : les deux secrets restent strictement séparés. `/admin` est protégé par une session signée côté Worker. La route canonique `/admin` est transmise telle quelle au binding `ASSETS` : il ne faut pas la réécrire en `/admin.html`, car le `html_handling` automatique de Cloudflare redirige précisément `/admin.html` vers `/admin`, ce qui créerait une boucle. Les assets et réponses privées admin sont servis avec `Cache-Control: no-store` et le Service Worker les contourne explicitement. Le statut des services réutilise le dernier snapshot central pendant 15 minutes afin qu’un auto-refresh fréquent de l’interface ne multiplie pas les appels aux fournisseurs externes. Ne pas renommer ces chemins sans mettre à jour `js/analytics-config.js`, `admin.js` et les tests associés.
...
Le Worker envoie cette clé uniquement côté serveur dans l'en-tête `apikey: <API_KEY>` (avec `Accept: */*`, comme le curl généré par l'explorateur DPVigilance) et met le produit national `cartevigilance/encours` dans un cache L1 edge de 10 minutes et un cache L2 partagé de 10 minutes. Le TTL edge est réduit à la durée de fraîcheur restante du L2 afin que les deux niveaux ne s’additionnent jamais. Le navigateur et l'application Android ne reçoivent jamais la clé.
