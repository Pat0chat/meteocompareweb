...
export async function restoreLocalBackup(value,{replace=true}={}){
  ...
  await writeMap(value.data.forecasts,'forecast',saveForecast);await writeMap(value.data.normals,'normals',(id,v)=>saveNormals(id,v));await writeMap(value.data.bias,'bias',(id,v)=>saveBias(id,v));await writeMap(value.data.evolution,'evolution',(id,v)=>saveEvolution(id,v));await writeMap(value.data.marine,'marine',(id,v)=>saveMarine(id,v));
