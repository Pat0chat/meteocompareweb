...
async function fetchOne(model,{timeoutMs=10000}={}){
  ...
  try{
    const res=await fetch(`${METADATA_BASE}/${encodeURIComponent(metadataKey)}/latest.json`,{signal:controller.signal,cache:'no-store',credentials:'omit',referrerPolicy:'no-referrer'});
