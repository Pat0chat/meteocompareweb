...
function healthStatus(diag,meta,model,nowMs){
  ...
  if(['PARTIAL','VARIABLE_MISSING'].includes(diag.status))return 'DEGRADED';
  if(meta?.error)return 'METADATA_UNAVAILABLE';
  const ref=finiteDate(meta?.referenceTime);if(ref==null)return diag.status==='RECOVERED'?'RECOVERED':'METADATA_UNAVAILABLE';
  const expected=ref+(Number(model.updateMinutes)||60)*60_000,delay=Math.max(0,(nowMs-expected)/60_000);
  if(delay>(Number(model.updateMinutes)||60)+DELAY_TOLERANCE_MIN)return 'MISSED_RUNS';
...
export function buildModelHealthReport(forecast,models,enabledIds,metadata={},history=[],nowMs=Date.now()){
  const diag=buildCityDiagnostics(forecast,models,enabledIds),rows=diag.rows.map(d=>{
    const model=models.find(m=>m.id===d.modelId),meta=metadata?.[d.modelId]||null,status=healthStatus(d,meta,model,nowMs),refMs=finiteDate(meta?.referenceTime),expectedRunAt=refMs==null?null:new Date(refMs+(Number(model?.updateMinutes)||60)*60_000).toISOString(),delayMinutes=refMs==null?null:Math.max(0,Math.round((nowMs-(refMs+(Number(model?.updateMinutes)||60)*60_000))/60_000));
    return {...d,healthStatus:status,metadataAvailable:Boolean(meta&&!meta.error),metadataError:meta?.error||null,referenceTime:meta?.referenceTime||null,lastModifiedTime:meta?.lastModifiedTime||null,expectedRunAt,delayMinutes,responseMs:Number.isFinite(meta?.responseMs)?meta.responseMs:null,missingVariables:missingVariables(d),incident24h:0,incident7d:0};
  });
...
export function appendHealthSnapshot(history,report,nowMs=Date.now()){
  ...
  if(current.length&&nowMs-current.at(-1).capturedAt<HISTORY_MIN_INTERVAL_MS)return current;
  ...
  return [...current,{capturedAt:nowMs,rows}].slice(-800);
}
export function countIncidentEpisodes(history,modelId,windowMs,nowMs=Date.now()){
  const snaps=(history||[]).filter(x=>x?.capturedAt>=nowMs-windowMs).sort((a,b)=>a.capturedAt-b.capturedAt);let count=0,prevIncident=false;
