...
import { biasLeadDaysForModel } from '../models.js';

export function normalizePreviousRuns(raw,city,models,startDate,endDate){
  const h=raw?.hourly||{};
  const timeline=(h.time||[]).map((ts,i)=>({ts,i,date:typeof ts==='string'?ts.slice(0,10):''})).filter(x=>x.date>=startDate&&x.date<=endDate),axis=new Map();
  for(const x of timeline){const row=axis.get(x.date)||{count:0,first:x.ts,last:x.ts};row.count++;row.last=x.ts;axis.set(x.date,row);}
  const records=[],single=models.length===1;
  const lookup=(base,model,leadDay)=>{const lead=`${base}_previous_day${leadDay}`,keys=[];for(const key of [model.apiKey,...model.aliases])keys.push(`${lead}_${key}`,`${base}_${key}_previous_day${leadDay}`);if(single)keys.push(lead);for(const k of keys)if(Array.isArray(h[k]))return h[k];return [];};
  for(const model of models)for(const leadDay of biasLeadDaysForModel(model)){
    const series={TEMPERATURE:lookup('temperature_2m',model,leadDay),PRECIPITATION:lookup('precipitation',model,leadDay),WIND_SPEED:lookup('wind_speed_10m',model,leadDay)},byDate=new Map();
    for(const x of timeline){let a=byDate.get(x.date);if(!a){a={TEMPERATURE:[],PRECIPITATION:[],WIND_SPEED:[]};byDate.set(x.date,a);}for(const v of Object.keys(series)){const z=series[v][x.i];if(Number.isFinite(z)&&(v==='TEMPERATURE'||z>=0))a[v].push(z);}}
    for(const [date,a] of byDate){const day=axis.get(date),n=day?.count||0;if(n<23||n>25||day?.first?.slice(11,16)!=='00:00'||day?.last?.slice(11,16)!=='23:00')continue;const values={TEMPERATURE:a.TEMPERATURE.length===n?Math.max(...a.TEMPERATURE):null,PRECIPITATION:a.PRECIPITATION.length===n?a.PRECIPITATION.reduce((sum,v)=>sum+v,0):null,WIND_SPEED:a.WIND_SPEED.length===n?Math.max(...a.WIND_SPEED):null};for(const [variable,value] of Object.entries(values))if(Number.isFinite(value))records.push({modelId:model.id,variable,targetDate:date,issuedDate:addDays(date,-leadDay),leadDay,value});}
  }
  return records;
}
