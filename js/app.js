...
function renderGraphicForecastView(cityId){
  ...
  const step=54,leftPad=126,rightPad=34,totalWidth=Math.round(leftPad+points.length*step+rightPad),tempPlotHeight=340,rainPlotHeight=160,windPlotHeight=160,tempTop=58,tempBottom=292,rainTop=48,rainBottom=136,windTop=48,windBottom=136;
  ...
  const tempTicks=Array.from({length:6},(_,i)=>tempMin+(tempMax-tempMin)*i/5),verticalGrid=(height,top,bottom)=>points.map((point,index)=>`<line class="graphic-grid vertical ${index%6===0?'major':''}" x1="${x(index)}" x2="${x(index)}" y1="${top}" y2="${bottom}"/>`).join(''),dayBreaks=(top,bottom)=>points.map((point,index)=>index===0||point.date!==points[index-1].date?`<line class="graphic-day-break" x1="${x(index)-step/2}" x2="${x(index)-step/2}" y1="${top}" y2="${bottom}"/>`:'').join('');
  const tempGrid=tempTicks.map(value=>`<line class="graphic-grid horizontal" x1="${leftPad}" x2="${totalWidth-rightPad}" y1="${yTemp(value)}" y2="${yTemp(value)}"/>`).join('')+verticalGrid(tempPlotHeight,0,tempPlotHeight),rainGrid=rainAxis.ticks.map(value=>`<line class="graphic-grid horizontal rain" x1="${leftPad}" x2="${totalWidth-rightPad}" y1="${yRain(value)}" y2="${yRain(value)}"/>`).join('')+verticalGrid(rainPlotHeight,0,rainPlotHeight),windGrid=windAxis.ticks.map(value=>`<line class="graphic-grid horizontal wind" x1="${leftPad}" x2="${totalWidth-rightPad}" y1="${yWind(value)}" y2="${yWind(value)}"/>`).join('')+verticalGrid(windPlotHeight,0,windPlotHeight);
  ...
  const tempTargets=points.map((point,index)=>{if(!Number.isFinite(point.temperatureC))return '';const px=x(index),py=yTemp(point.temperatureC),id=`graphic-temp-tip-${index}`,edge=graphicTooltipEdgeClass(index,points.length),label=`${dateLabel(point.date,locale)} ${timeLabel(point.timestamp)} · ${t('temperature')} ${fmt(point.temperatureC,1)} °C`;return `<div class="graphic-condition" style="left:${px}px;top:${(Math.max(10,py-46)/tempPlotHeight*100).toFixed(3)}%" title="${attr(localizedConditionInfo(point.condition).label)}">${point.condition?weatherIcons.render(point.condition,{size:'small'}):''}</div><button class="graphic-point graphic-temp-point${edge}" data-graphic-tooltip-target="temperature" data-graphic-index="${index}" style="left:${px}px;top:${(py/tempPlotHeight*100).toFixed(3)}%" aria-label="${attr(label)}" aria-describedby="${attr(id)}" aria-expanded="false"><i></i><span class="graphic-temp-value">${fmt(point.temperatureC)}°</span>${graphicTemperatureTooltip(point,id)}</button>`;}).join('');
  const rainTargets=points.map((point,index)=>{const amount=Number.isFinite(point.precipitationMm)?point.precipitationMm:Number.isFinite(point.precipitationConditionalMm)?point.precipitationConditionalMm:null;if(!Number.isFinite(amount)&&!Number.isFinite(point.precipitationPercent))return '';const px=x(index),barAmount=Math.max(0,amount||0),height=Math.max(barAmount>0?3:1,(barAmount/rainMax)*(rainBottom-rainTop)),top=yRain(barAmount),prob=Math.max(0,Math.min(100,Number(point.precipitationPercent)||0)),id=`graphic-rain-tip-${index}`,edge=graphicTooltipEdgeClass(index,points.length),label=`${dateLabel(point.date,locale)} ${timeLabel(point.timestamp)} · ${t('precipitation')} ${Number.isFinite(amount)?fmt(amount,1)+' mm':'—'} · ${Math.round(prob)} %`;return `<button class="graphic-rain-bar${edge}" data-graphic-tooltip-target="rain" data-graphic-index="${index}" style="left:${px-step*.31}px;top:${(top/rainPlotHeight*100).toFixed(3)}%;width:${Math.max(12,step*.62)}px;height:${(height/rainPlotHeight*100).toFixed(3)}%;--prob:${prob}%" aria-label="${attr(label)}" aria-describedby="${attr(id)}" aria-expanded="false">${barAmount>=.2?`<span>${fmt(barAmount,barAmount<10?1:0)}</span>`:''}${graphicRainTooltip(point,id)}</button>`;}).join('');
  const windTargets=points.map((point,index)=>{if(!Number.isFinite(point.windKmh))return '';const px=x(index),py=yWind(point.windKmh),id=`graphic-wind-tip-${index}`,edge=graphicTooltipEdgeClass(index,points.length),label=`${dateLabel(point.date,locale)} ${timeLabel(point.timestamp)} · ${t('wind')} ${fmt(point.windKmh)} km/h`;return `<button class="graphic-point graphic-wind-point${edge}" data-graphic-tooltip-target="wind" data-graphic-index="${index}" style="left:${px}px;top:${(py/windPlotHeight*100).toFixed(3)}%" aria-label="${attr(label)}" aria-describedby="${attr(id)}" aria-expanded="false"><i></i>${graphicWindTooltip(point,id)}</button>`;}).join('');
