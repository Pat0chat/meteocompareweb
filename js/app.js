...
function renderForecastEngineLineChart(dates,matrix,selected,{key,label,unit='',digits=1}){
  ...
  const w=1080,h=268,pad={l:54,r:22,t:20,b:42},rawMin=Math.min(...finite),rawMax=Math.max(...finite),rawSpan=Math.max(.5,rawMax-rawMin),min=rawMin-rawSpan*.10,max=rawMax+rawSpan*.10,span=Math.max(.5,max-min),x=i=>pad.l+(dates.length<=1?0:i/(dates.length-1))*(w-pad.l-pad.r),y=v=>pad.t+(max-v)/span*(h-pad.t-pad.b),ticks=Array.from({length:4},(_,i)=>min+(span*i/3));
...
function renderForecastEngineComparisonModal(cityId){
  ...
  const visualCharts=`<div class="forecast-engine-chart-grid">${renderForecastEngineLineChart(dates,matrix,selected,{key:'tempMax',label:t('forecastEngineTempTrend'),unit:' °C',digits:1})}${renderForecastEngineLineChart(dates,matrix,selected,{key:'precipExpected',label:t('forecastEngineRainTrend'),unit:' mm',digits:1})}${renderForecastEngineLineChart(dates,matrix,selected,{key:'wind',label:t('forecastEngineWindTrend'),unit:' km/h',digits:0})}</div>`;
  return `<div class="forecast-engine-overview">${engineCards}</div><div class="banner info forecast-engine-reading"><b>${esc(t('forecastEngineReading'))}</b><span>${esc(t('forecastEngineReadingBody'))}</span></div><section class="forecast-engine-visual-section"><div class="forecast-engine-visual-head"><div><h3>${esc(t('forecastEngineVisualOverview'))}</h3><p>${esc(t('forecastEngineVisualOverviewBody'))}</p></div></div>${visualCharts}${renderForecastEngineDivergenceTimeline(dates,matrix)}</section><section class="forecast-engine-comparison-days"><div class="forecast-engine-comparison-title"><div><h3>${esc(t('forecastEngineInspectDetails'))}</h3><p>${esc(t('forecastEngineDailyOutlook'))}</p></div><span>${esc(city.name)}</span></div>${dayTables||`<div class="empty-state compact">${esc(t('insufficientData'))}</div>`}</section>`;
}
