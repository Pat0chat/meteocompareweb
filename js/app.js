...
function renderCityCard(city){
  ...
  const profile=localConsensusWeights(city.id),consensusWeights=state.settings.localWeightedConsensus&&profile.ready?profile.maps:null,now=currentConditions(f,new Date(),{weightsByVariable:consensusWeights||{}}),today=cityToday(f.city.timezone),day=cachedAggregateDay(f,today,consensusWeights),info=localizedConditionInfo(now.condition||day.condition),heat=cachedHeatmap(f,12,consensusWeights),conf=day.confidence?.overallPercent,minT=day.tempMin,maxT=day.tempMax,precip=day.precip,wind=day.wind;
...
function comparisonRenderContext(){const {t,locale}=i18n(),comparisonAggregateDay=(f,date)=>{const cityId=f?.city?.id,profile=cityId?localConsensusWeights(cityId):null,weights=state.settings.localWeightedConsensus&&profile?.ready?profile.maps:null;return cachedAggregateDay(f,date,weights);};return {t,locale,state,esc,attr,fmt,dateLabel,timeLabel,cachedAggregateDay:comparisonAggregateDay,visibleModelIds,selectedModelIds:state.compareModelIds};}
...
function renderTodaySummary(f,agg,now,cityId,profile=localConsensusWeights(cityId)){
  ...
  return `<section class="section today-summary" id="today-summary"><div class="summary-card"><div class="summary-accent" style="--summary-accent:${info.accent||'var(--primary)'}"></div><div class="summary-header"><div class="summary-now"><div class="summary-weather-icon">${conditionMarkup(now.condition||agg.condition,'normal',now.condition?Boolean(now.conditionInferred):Boolean(agg.conditionInferred))}</div><div><div class="summary-kicker">${esc(dateLabel(agg.date,i18n().locale,'long'))}</div><div class="summary-now-line"><span class="summary-current-temp">${Number.isFinite(now.temperature)?`${fmt(now.temperature,1)}°`:'—'}</span><span class="summary-condition">${esc(info.label)}</span></div><div class="summary-context-chips"><span>${esc(t('analysedModels',{models:modelCountLabel(modelCount)}))}</span><span>${esc(t('independentFamilies',{count:familyCount}))}</span><span>${esc(method)}</span>${agg.sunrise?`<span>☀ ${esc(t('sunrise'))} ${timeLabel(agg.sunrise)}</span>`:''}${agg.sunset?`<span>◐ ${esc(t('sunset'))} ${timeLabel(agg.sunset)}</span>`:''}</div></div></div><div class="global-agreement ${Number.isFinite(overall)?confidenceClass(overall):''}"><div class="global-agreement-label">${esc(t('modelConvergence'))}</div><div class="global-agreement-value">${Number.isFinite(overall)?`${overall}%`:'—'}</div><div class="global-agreement-meta">${esc(Number.isFinite(overall)?`${convergence} · ${familyCount} ${t('familiesShort')} · ${modelCountLabel(modelCount)}`:t('insufficientData'))}</div>${historicalBlock}<button class="btn agreement-link" data-action="why-confidence">ⓘ ${esc(t('whyConvergence'))}</button></div></div><div class="today-grid"><div class="summary-tile metric-temp-min"><div class="summary-tile-head"><span class="summary-metric-icon temp">${summaryMetricIcon('temperature')}</span><div><div class="tile-label">${esc(t('tempMinimum'))}</div><div class="tile-hint">${esc(t('weightedMedianCentral'))}</div></div></div><div class="summary-value-line"><div class="big-value">${Number.isFinite(agg.tempMin)?fmt(agg.tempMin,1)+' °C':'—'}</div><div class="range"><span>${esc(t('modelRange'))}</span><strong>${fmtRange(...agg.tempMinRange,' °C',1)}</strong></div></div>${summaryAgreement(c.tempMin?.percent,c.tempMin?.count)}</div><div class="summary-tile metric-temp-max"><div class="summary-tile-head"><span class="summary-metric-icon temp">${summaryMetricIcon('temperature')}</span><div><div class="tile-label">${esc(t('tempMaximum'))}</div><div class="tile-hint">${esc(t('weightedMedianCentral'))}</div></div></div><div class="summary-value-line"><div class="big-value">${Number.isFinite(agg.tempMax)?fmt(agg.tempMax,1)+' °C':'—'}</div><div class="range"><span>${esc(t('modelRange'))}</span><strong>${fmtRange(...agg.tempMaxRange,' °C',1)}</strong></div></div>${summaryAgreement(c.tempMax?.percent,c.tempMax?.count)}</div><div class="summary-tile metric-rain"><div class="summary-tile-head"><span class="summary-metric-icon rain">${summaryMetricIcon('precipitation')}</span><div><div class="tile-label">${esc(t('precipitation'))}</div><div class="tile-hint">${esc(rainHint)}</div></div></div><div class="summary-value-line"><div class="big-value">${Number.isFinite(agg.precip)?fmt(agg.precip,1)+' mm':'—'}</div><div class="range"><span>${esc(t('modelRange'))}</span><strong>${fmtRange(...agg.precipRange,' mm',1)}</strong>${Number.isFinite(agg.precipExpected)?`<small>${esc(t('rainExpectedAmount',{amount:fmt(agg.precipExpected,1)}))}</small>`:''}</div></div>${summaryAgreement(precipConfidence?.percent,precipConfidence?.count,Number.isFinite(precipConfidence?.probabilityPercent)?t('rainProbabilityShort',{probability:precipConfidence.probabilityPercent}):'')}</div><div class="summary-tile metric-wind"><div class="summary-tile-head"><span class="summary-metric-icon wind">${summaryMetricIcon('wind')}</span><div><div class="tile-label">${esc(t('wind'))}</div><div class="tile-hint">${esc(t('weightedMedianCentral'))}</div></div></div><div class="summary-value-line"><div class="big-value">${Number.isFinite(agg.wind)?fmt(agg.wind)+' km/h':'—'}</div><div class="range"><span>${esc(t('modelRange'))}</span><strong>${fmtRange(...agg.windRange,' km/h')}</strong>${Number.isFinite(agg.gust)?`<small>${esc(t('gusts'))} ${fmt(agg.gust)} km/h</small>`:''}</div></div>${summaryAgreement(c.windMax?.percent,c.windMax?.count)}</div></div></div></section>`;
}
...
function renderInsights(f,evolution,consensusWeights=null){
  ...
  for(const d of dates){
    ...
    if(Number.isFinite(pc?.probabilityPercent)&&Number.isFinite(pc?.percent)&&pc.percent<50)items.push({p:90,date:d,icon:'☂️',text:t('insightRainUncertainV2',{probability:pc.probabilityPercent,families:pc.familyCount||0})});
