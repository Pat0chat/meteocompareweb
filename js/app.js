...
function summaryAgreement(percent,count,detail=''){
  ...
  if(!Number.isFinite(percent)){
    const meta=[Number.isFinite(count)?modelCountLabel(count):'',detail].filter(Boolean).join(' · ');
    return `<div class="summary-agreement-detail muted"><div class="summary-agreement-copy"><span>${esc(t('agreement'))}</span><strong>—</strong>${meta?`<small>${esc(meta)}</small>`:''}</div></div>`;
  }
...
function renderTopbar(){
  ...
  return `<header class="topbar">${!isHome?`<button class="topbar-back" data-action="back" aria-label="${esc(t('back'))}" title="${esc(t('back'))}"><span class="topbar-back-icon">${uiIcon('back',18)}</span><span class="topbar-back-label">${esc(t('back'))}</span></button>`:''}<div class="topbar-inner">
...
function collapsibleCitySpecs(){return [
  ['today-summary','.summary-card','.summary-header'],
  ...
  ['diagnostics','.section-card','.section-head']
];}
...
function renderHomeMiniTimeline(points){
  ...
  return `<div class="home-mini-timeline" aria-label="${esc(t('homeMiniTimelineAria'))}">${points.map(point=>{
    const prob=point.precipitationPercent,amount=point.precipitationConditionalMm;
    return `<div class="home-mini-hour"><span class="home-mini-time">${esc(timeLabel(point.timestamp))}</span><strong>${Number.isFinite(point.temperatureC)?`${fmt(point.temperatureC)}°`:'—'}</strong><span class="home-mini-rain ${Number.isFinite(prob)&&prob>=30?'wet':''}">${Number.isFinite(prob)&&prob>=20?`☂ ${Math.round(prob)}%`:'·'}</span>${Number.isFinite(amount)&&amount>=.1?`<small>${fmt(amount,1)} mm</small>`:'<small>&nbsp;</small>'}</div>`;
  }).join('')}</div>`;
}
...
function renderHome(){
  ...
  const shortcuts=state.cities.slice(0,6).map(city=>`<button class="home-city-shortcut" data-city-open="${attr(city.id)}"><span>${esc(city.name)}</span><small>${esc(placeLine(city))}</small></button>`).join('');
  return `<main class="page home-page"><section class="home-hero"><div class="home-hero-copy"><span class="home-hero-kicker">${esc(t('homeModernKicker'))}</span><h1>${esc(t('homeModernTitle'))}</h1><p>${esc(t('homeModernLead'))}</p><button class="home-search-trigger" data-action="open-add-city"><span class="home-search-icon">⌕</span><span>${esc(t('homeSearchPrompt'))}</span><kbd>+</kbd></button>${shortcuts?`<div class="home-shortcuts" aria-label="${esc(t('cities'))}">${shortcuts}<button class="home-city-shortcut add" data-action="open-add-city"><span>＋ ${esc(t('addCity'))}</span></button></div>`:''}</div><div class="home-hero-mark" aria-hidden="true"><span class="home-hero-mark-ring"></span><strong>↔</strong><small>${esc(t('homeHeroMark'))}</small></div></section>
