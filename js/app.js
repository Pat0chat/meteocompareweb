...
function renderEvolutionTrajectory(e,unit,threshold,minValue=null){
  ...
  const thresholdTopLine=rawBandTop>=plotTop&&rawBandTop<=plotBottom?`<line class="evolution-threshold-line" x1="${pad.l}" x2="${width-pad.r}" y1="${bandTop}" y2="${bandTop}"/>`:'';
  const thresholdBottomLine=rawBandBottom>=plotTop&&rawBandBottom<=plotBottom?`<line class="evolution-threshold-line" x1="${pad.l}" x2="${width-pad.r}" y1="${bandBottom}" y2="${bandBottom}"/>`:'';
  const thresholdBand=thresholdValue>0?`<rect class="evolution-threshold-band" x="${pad.l}" y="${bandY}" width="${width-pad.l-pad.r}" height="${bandH}"/>${thresholdTopLine}${thresholdBottomLine}<text class="evolution-threshold-label" x="${width-pad.r-2}" y="${Math.max(plotTop+9,bandTop-3)}" text-anchor="end">+${esc(fmt(thresholdValue,1))}${esc(unit)}</text><text class="evolution-threshold-label" x="${width-pad.r-2}" y="${Math.min(plotBottom-3,bandBottom+10)}" text-anchor="end">−${esc(fmt(thresholdValue,1))}${esc(unit)}</text>`:'';
