...
function renderNow(){
  ...
  if(state.modal?.type==='radar')queueMicrotask(()=>void hydrateRadarModal());
}
...
async function hydrateRadarModal(){
  ...
  try{
    ...
    const {t,locale}=i18n();await radar.mountRadarModal({root,city,forecast:state.forecasts[cityId]||null,t,locale,onRangeChange:range=>void trackAnalyticsEvent('Rain Radar Range Changed',state.route,{range})});
...
function renderSeoCityContext(city){
  ...
  return `<section class="section section-card seo-city-context"><div class="section-head"><div><h2>${esc(t('seoCityContextTitle',{city:seo.name}))}</h2><p>${esc(t('seoCityContextLead',{city:seo.name}))}</p></div></div><div class="seo-city-context-copy"><p>${esc(t('seoCityContextLocation',{city:seo.name,department:seo.department,region:seo.region}))}</p><p>${esc(t('seoCityContextModels',{city:seo.name}))}</p></div></section>`;
}
function renderHome(){
...
async function refreshMarineData(cityId,force=false,activate=false){
  ...
  try{const data=await marine.fetchMarineForCity(city);if(!data.coastal){if(activate)toast(t('marineNotCoastal'));return;}state.marine[cityId]=data;analysisStore.mark('marine',cityId);saveMarine(cityId,data);if(activate&&!city.marineEnabled){city.marineEnabled=true;persistFavoriteCities();toast(t('marineEnabled'));}if(state.route.name==='city'&&state.route.id===cityId)render();}
...
function renderInsights(f,evolution,consensusWeights=null){
  ...
  const fact=(label,value)=>value!=null&&value!==''?{label,value:String(value)}:null;
  const compactFacts=list=>list.filter(Boolean).slice(0,4);
  for(const d of dates){
    const a=cachedAggregateDay(f,d,consensusWeights),c=a.confidence?.overallPercent,modelCount=a.data?.length||0,familyCount=a.consensusFamilyCount||0;
    if(Number.isFinite(c)&&c<45)items.push({kind:'weather',tone:'critical',category:t('insightsCategoryAgreement'),p:95,date:d,icon:'⚠️',text:t('insightStrongDisagreement',{percent:c}),facts:compactFacts([fact(t('insightsMetricConvergence'),`${Math.round(c)}%`),fact(t('insightsMetricModels'),modelCountLabel(modelCount)),fact(t('insightsMetricFamilies'),familyCount),fact(t('insightsMetricTemperatureRange'),Number.isFinite(a.tempMinRange?.[0])&&Number.isFinite(a.tempMaxRange?.[1])?`${fmt(a.tempMinRange[0],0)}–${fmt(a.tempMaxRange[1],0)} °C`:null)])});
    const pc=a.confidence?.precipitation;
    if(Number.isFinite(pc?.probabilityPercent)&&Number.isFinite(pc?.percent)&&pc.percent<50)items.push({kind:'weather',tone:'caution',category:t('precipitation'),p:90,date:d,icon:weatherIcons.renderMetric('precipitation',{size:'small'}),text:t('insightRainUncertainV2',{probability:pc.probabilityPercent,families:pc.familyCount||0}),facts:compactFacts([fact(t('insightsMetricProbability'),`${Math.round(pc.probabilityPercent)}%`),fact(t('insightsMetricConvergence'),`${Math.round(pc.percent)}%`),fact(t('insightsMetricRange'),Number.isFinite(a.precipRange?.[0])&&Number.isFinite(a.precipRange?.[1])?fmtRange(...a.precipRange,' mm',1):null),fact(t('insightsMetricModels'),modelCountLabel(pc.count||modelCount))])});
    else if(Number.isFinite(a.precip)&&a.precip>=5)items.push({kind:'weather',tone:'attention',category:t('precipitation'),p:82,date:d,icon:weatherIcons.renderMetric('rain',{size:'small'}),text:t('insightRainStrong',{mean:fmt(a.precip,1),range:fmtRange(...a.precipRange,' mm',1)}),facts:compactFacts([fact(t('insightsMetricAmount'),`${fmt(a.precip,1)} mm`),fact(t('insightsMetricProbability'),Number.isFinite(a.precipProbability)?`${Math.round(a.precipProbability)}%`:null),fact(t('insightsMetricRange'),Number.isFinite(a.precipRange?.[0])&&Number.isFinite(a.precipRange?.[1])?fmtRange(...a.precipRange,' mm',1):null),fact(t('insightsMetricConvergence'),Number.isFinite(pc?.percent)?`${Math.round(pc.percent)}%`:null)])});
    if(Number.isFinite(a.wind)&&a.wind>=35)items.push({kind:'weather',tone:'attention',category:t('wind'),p:86,date:d,icon:weatherIcons.renderMetric('wind',{size:'small'}),text:t('insightWind',{mean:fmt(a.wind),gusts:Number.isFinite(a.gust)?t('insightGusts',{value:fmt(a.gust)}):''}),facts:compactFacts([fact(t('insightsMetricWind'),`${fmt(a.wind)} km/h`),fact(t('insightsMetricGusts'),Number.isFinite(a.gust)?`${fmt(a.gust)} km/h`:null),fact(t('insightsMetricRange'),Number.isFinite(a.windRange?.[0])&&Number.isFinite(a.windRange?.[1])?fmtRange(...a.windRange,' km/h',0):null),fact(t('insightsMetricConvergence'),Number.isFinite(a.confidence?.windMax?.percent)?`${Math.round(a.confidence.windMax.percent)}%`:null)])});
    if(Number.isFinite(c)&&c>=85)items.push({kind:'weather',tone:'stable',category:t('insightsCategoryAgreement'),p:35,date:d,icon:'✓',text:t('insightHighAgreement',{percent:c}),facts:compactFacts([fact(t('insightsMetricConvergence'),`${Math.round(c)}%`),fact(t('insightsMetricModels'),modelCountLabel(modelCount)),fact(t('insightsMetricFamilies'),familyCount),fact(t('insightsMetricTemperature'),Number.isFinite(a.tempMax)?`${fmt(a.tempMax,0)} °C`:null)])});
  }
  for(let i=1;i<dates.length;i++){
    const prev=cachedAggregateDay(f,dates[i-1],consensusWeights),cur=cachedAggregateDay(f,dates[i],consensusWeights);
    if(Number.isFinite(prev.tempMax)&&Number.isFinite(cur.tempMax)&&Math.abs(cur.tempMax-prev.tempMax)>=7){
      const delta=cur.tempMax-prev.tempMax;
      items.push({kind:'weather',tone:'attention',category:t('temperature'),p:80,date:dates[i],icon:weatherIcons.renderMetric('temperature',{size:'small'}),text:t('insightThermalChange',{delta:`${delta>0?'+':''}${fmt(delta,0)}`}),facts:compactFacts([fact(t('insightsMetricChange'),`${delta>0?'+':''}${fmt(delta,0)} °C`),fact(t('insightsMetricPrevious'),`${fmt(prev.tempMax,0)} °C`),fact(t('insightsMetricForecast'),`${fmt(cur.tempMax,0)} °C`),fact(t('insightsMetricConvergence'),Number.isFinite(cur.confidence?.overallPercent)?`${Math.round(cur.confidence.overallPercent)}%`:null)])});
      break;
    }
  }
  const ev=[];
  for(const day of evolution?.days||[])for(const [v,x] of Object.entries(day.variables||{})){
    if(x.trend!=='STABLE'&&Number.isFinite(x.medianAbsDelta)){
      const threshold=v==='temperature'?1:v==='precipitation'?2:5;
      if(x.medianAbsDelta>=threshold){
        const unit=v==='temperature'?' °C':v==='precipitation'?' mm':' km/h',variableLabel=v==='temperature'?t('temperature'):v==='precipitation'?t('precipitation'):t('wind'),hours=x.previous?.[0]?.ageHours||'?';
        ev.push({kind:'revision',tone:'revision',category:t('insightsCategoryRevision'),p:92,date:day.date,icon:v==='temperature'?weatherIcons.renderMetric('temperature',{size:'small'}):v==='precipitation'?weatherIcons.renderMetric('precipitation',{size:'small'}):weatherIcons.renderMetric('wind',{size:'small'}),text:t('insightForecastRevised',{hours,trend:trendText(x.trend,x.medianDelta,unit)}),facts:compactFacts([fact(t('insightsMetricVariable'),variableLabel),fact(t('insightsMetricAmplitude'),`${fmt(x.medianAbsDelta,v==='temperature'?1:0)}${unit}`),fact(t('insightsMetricSince'),`H−${hours}`),fact(t('insightsMetricDirection'),trendText(x.trend,x.medianDelta,unit))])});
      }
    }
  }
  ...
  const lead=chosen.reduce((best,item)=>!best||item.p>best.p?item:best,null),ordered=[lead,...chosen.filter(x=>x!==lead).sort((a,b)=>a.date.localeCompare(b.date)||b.p-a.p)];
  const signalMeta=t('insightsSummaryMeta',{signals:ordered.length,days:dates.length});
  return `<section class="section insights-section insights-section-wide" id="insights"><div class="section-card insights-card"><div class="section-head insights-head"><div><div class="insights-kicker">${esc(t('insightsKicker'))}</div><h2>${esc(t('forecast_insights_title'))}</h2><p>${esc(t('forecast_insights_subtitle_generic'))}</p></div><div class="insights-head-meta"><span class="insights-summary-pill">${esc(signalMeta)}</span><span class="insights-status" aria-hidden="true"><i></i></span></div></div><div class="insights-grid">${ordered.map(x=>`<article class="insight-item ${x===lead?'lead':''} ${x.kind} tone-${x.tone}"><div class="insight-item-top"><div class="insight-item-head"><span class="insight-icon">${x.icon}</span><div class="insight-heading-copy"><span class="insight-category">${esc(x.category)}</span><span class="insight-date">${esc(dateLabel(x.date,i18n().locale,'long'))}</span></div></div><span class="insight-priority">${esc(t(x===lead?'insightsPriorityLead':x.tone==='stable'?'insightsPriorityStable':x.kind==='revision'?'insightsPriorityRevision':'insightsPriorityWatch'))}</span></div><p>${esc(x.text)}</p>${x.facts?.length?`<div class="insight-facts">${x.facts.map(factItem=>`<div class="insight-fact"><span>${esc(factItem.label)}</span><strong>${esc(factItem.value)}</strong></div>`).join('')}</div>`:''}</article>`).join('')}</div></div></section>`;
}
...
function evolutionVariableMeta(){const {t}=i18n();return {temperature:{label:t('temperature'),unit:' °C',icon:weatherIcons.renderMetric('temperature',{size:'tiny'})},precipitation:{label:t('precipitation'),unit:' mm',icon:weatherIcons.renderMetric('precipitation',{size:'tiny'})},wind:{label:t('wind'),unit:' km/h',icon:weatherIcons.renderMetric('wind',{size:'tiny'})}};}
...
function renderModal(){
  ...
  if(state.modal.type==='radar'){const c=state.cities.find(x=>x.id===state.modal.cityId);return `<div class="modal-backdrop" data-action="modal-backdrop"><div class="modal modal-wide radar-modal" role="dialog" aria-modal="true" aria-labelledby="modal-title"><div class="modal-head radar-modal-head"><div><h2 id="modal-title">${esc(t('rainRadar'))}</h2><span class="small">${esc(c?.name||t('city'))} · ${esc(t('radarSubtitle'))}</span></div><button class="icon-btn" data-action="close-modal" aria-label="${esc(t('close'))}">×</button></div><div class="modal-content radar-modal-content" data-radar-root><div class="radar-toolbar"><div class="radar-range-group" aria-label="${esc(t('radarRange'))}"><button class="seg-btn active" data-radar-range="near" aria-pressed="true">${esc(t('radarRangeNear'))}</button><button class="seg-btn" data-radar-range="regional" aria-pressed="false">${esc(t('radarRangeRegional'))}</button><button class="seg-btn" data-radar-range="wide" aria-pressed="false">${esc(t('radarRangeWide'))}</button></div><div class="radar-status" data-radar-status>${esc(t('radarLoading'))}</div></div><div class="radar-map-stage" data-radar-stage><div class="radar-base-layer" data-radar-base aria-hidden="true"></div><img class="radar-precip-layer" data-radar-image alt="" draggable="false"><canvas class="radar-nowcast-layer" data-radar-nowcast aria-hidden="true"></canvas><div class="radar-map-shade" aria-hidden="true"></div><div class="radar-nowcast-badge">${esc(t('radarNowcastBadge'))}</div><div class="radar-center-marker" aria-label="${esc(t('radarCenterLocality',{city:c?.name||t('city')}))}"><span></span><strong>${esc(c?.name||t('city'))}</strong></div><div class="radar-compass" aria-hidden="true">N</div><div class="radar-attribution"><a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener noreferrer">© OpenStreetMap</a> · <a href="https://www.rainviewer.com/" target="_blank" rel="noopener noreferrer">RainViewer</a></div></div><div class="radar-legend"><span>${esc(t('radarIntensity'))}</span><div class="radar-gradient" aria-hidden="true"></div><small>${esc(t('radarLight'))}</small><small>${esc(t('radarModerate'))}</small><small>${esc(t('radarHeavy'))}</small><div class="radar-projection-legend"><span>${esc(t('radarNowcastProjection'))}</span><i class="p15"></i><small>+15 min</small><i class="p30"></i><small>+30</small><i class="p45"></i><small>+45</small><i class="p60"></i><small>+60</small></div></div><div class="radar-playback"><button class="radar-play-button" data-radar-play aria-label="${esc(t('radarPlay'))}" title="${esc(t('radarPlay'))}">▶</button><div class="radar-time-block"><strong data-radar-time>—</strong><span>${esc(t('radarObserved'))}</span></div><input class="radar-slider" data-radar-slider type="range" min="0" max="0" value="0" aria-label="${esc(t('radarTimeline'))}"></div><div class="radar-nowcast-summary loading" data-radar-nowcast-summary><span class="loader"></span><span>${esc(t('radarNowcastAnalyzing'))}</span></div><section class="radar-nowcast-panel"><div class="radar-panel-head"><h3>${esc(t('radarNextHours'))}</h3><span class="radar-model-badge">${esc(t('multiModelForecast'))}</span></div><div data-radar-forecast></div></section><p class="small radar-privacy-note">${esc(t('radarPrivacyNote'))}</p></div></div></div>`;}
...
function handleAction(e){
  ...
  else if(action==='favorite-route-city'){const city=promoteRouteCity();if(city){void trackAnalyticsEvent('SEO City Favorite Added',state.route);toast(i18n().t('seoFavoriteAdded',{city:city.name}));render();}}
  ...
  else if(action==='open-radar'){if(!state.online){toast(i18n().t('radarOnlineRequired'));return;}if(state.route.name!=='city'||!state.route.id)return;void trackAnalyticsEvent('Rain Radar Opened',state.route);lastFocusedBeforeModal=document.activeElement;state.modal={type:'radar',cityId:state.route.id};render();}
...
function addCityFromSearch(id){
  const city=state.modal?.results?.find(c=>c.id===id);if(!city)return;cancelCitySearch();void trackAnalyticsEvent('City Added',state.route);let target=state.cities.find(c=>c.id===city.id||sameCityPlace(c,city));
  if(target){if(target.seoTransient){delete target.seoTransient;persistFavoriteCities();toast(i18n().t('seoFavoriteAdded',{city:target.name}));}}
  else{target=city;state.cities.push(target);persistFavoriteCities();state.evolution[target.id]=[];state.bias[target.id]={forecasts:[],observations:[],updatedAt:null};}
  state.modal=null;render();refreshCity(target.id,true);
}
