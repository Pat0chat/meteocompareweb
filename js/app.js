...
function renderForecastEngineCompareAction(){
  ...
  return `<div class="forecast-engine-compare-slot"><button class="btn tonal forecast-engine-overview-action" data-action="open-engine-comparison"><span class="forecast-engine-overview-symbol" aria-hidden="true">Σ</span><span class="forecast-engine-overview-copy"><strong>${esc(t('forecastEngineCompare'))}</strong><small>${esc(t('forecastEngineActiveLabel',{engine:forecastEngineName(engine)}))}</small></span><span class="forecast-engine-overview-chevron" aria-hidden="true">›</span></button></div>`;
}
...
function renderTodaySummary(f,agg,now,cityId,profile=localConsensusWeights(cityId)){
  ...
  const engineHintFor=key=>{const detail=agg.engineDetails?.[key];return detail?.fallback?`${engineLabel} → ${forecastEngineName(detail.effectiveEngine)}`:forecastEngineName(detail?.effectiveEngine||engine);},rainCore=Number.isFinite(agg.precipProbability)?t('rainCentralHint',{probability:Math.round(agg.precipProbability),amount:Number.isFinite(agg.precipConditional)?fmt(agg.precipConditional,1):'—'}):t('rainCentralNoProbability'),rainHint=`${engineHintFor('precipitation')} · ${rainCore}`;
  const cards=[
...
function renderForecastEngineComparisonModal(cityId){
  ...
  return `<div class="forecast-engine-overview">${engineCards}</div><div class="banner info forecast-engine-reading"><b>${esc(t('forecastEngineReading'))}</b><span>${esc(t('forecastEngineReadingBody'))}</span></div><section class="forecast-engine-comparison-days"><div class="forecast-engine-comparison-title"><h3>${esc(t('forecastEngineDailyOutlook'))}</h3><span>${esc(city.name)}</span></div>${dayTables||`<div class="empty-state compact">${esc(t('insufficientData'))}</div>`}</section>`;
}
