...
function renderHomeConsensusStrip(day,percent,familyCount){
  ...
  return `<div class="home-consensus" title="${attr(t('homeConsensusLegend'))}"><div class="home-consensus-head"><span>${esc(t('homeConsensusTitleShort'))}</span><strong class="home-consensus-range">${fmt(min,1)}°–${fmt(max,1)}°</strong></div><div class="home-consensus-rail" aria-label="${esc(t('homeConsensusAria',{min:fmt(min,1),max:fmt(max,1)}))}">${dots}<i class="home-consensus-center" style="--center:${pos(central)}%"></i></div><div class="home-consensus-scale"><span>${fmt(min,1)}°</span><b>${esc(t('homeConsensusCentralShort',{value:fmt(central,1)}))}</b><span>${fmt(max,1)}°</span></div><p class="home-consensus-help">${esc(t('homeConsensusLegendShort'))}</p></div>`;
}
