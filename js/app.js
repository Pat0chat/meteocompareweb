...
function renderBandChart(bands,metric,normals){
  ...
  const xTickIdx=chartTickIndices(bands.length,5),xGrid=xTickIdx.map((i,tickPosition)=>{const anchor=tickPosition===0?'start':tickPosition===xTickIdx.length-1?'end':'middle';return `<line class="chart-grid vertical" x1="${x(i)}" y1="${pad.t}" x2="${x(i)}" y2="${height-pad.b}"/><text class="chart-axis agreement-date-axis" x="${x(i)}" y="${height-19}" text-anchor="${anchor}">${esc(dateLabel(bands[i].timestamp.slice(0,10),i18n().locale,'long'))}</text><text class="chart-axis secondary" x="${x(i)}" y="${height-7}" text-anchor="${anchor}">${esc(timeLabel(bands[i].timestamp))}</text>`;}).join('');
