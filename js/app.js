...
function renderForecastEngineLineChart(dates,matrix,selected,{key,label,unit='',digits=1}){
  ...
  const w=1240,h=320,pad={l:58,r:24,t:22,b:44},rawMin=Math.min(...finite),rawMax=Math.max(...finite),rawSpan=Math.max(.5,rawMax-rawMin),min=rawMin-rawSpan*.10,max=rawMax+rawSpan*.10,span=Math.max(.5,max-min),x=i=>pad.l+(dates.length<=1?0:i/(dates.length-1))*(w-pad.l-pad.r),y=v=>pad.t+(max-v)/span*(h-pad.t-pad.b),ticks=Array.from({length:4},(_,i)=>min+(span*i/3));
