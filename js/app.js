...
function renderInsights(f,evolution,consensusWeights=null){
  ...
  for(const d of dates){
    ...
    if(Number.isFinite(pc?.probabilityPercent)&&Number.isFinite(pc?.percent)&&pc.percent<50)items.push({kind:'weather',p:90,date:d,icon:weatherIcons.renderMetric('precipitation',{size:'small'}),text:t('insightRainUncertainV2',{probability:pc.probabilityPercent,families:pc.familyCount||0})});
    else if(Number.isFinite(a.precip)&&a.precip>=5)items.push({kind:'weather',p:82,date:d,icon:weatherIcons.renderMetric('rain',{size:'small'}),text:t('insightRainStrong',{mean:fmt(a.precip,1),range:fmtRange(...a.precipRange,' mm',1)})});
    if(Number.isFinite(a.wind)&&a.wind>=35)items.push({kind:'weather',p:86,date:d,icon:weatherIcons.renderMetric('wind',{size:'small'}),text:t('insightWind',{mean:fmt(a.wind),gusts:Number.isFinite(a.gust)?t('insightGusts',{value:fmt(a.gust)}):''})});
    if(Number.isFinite(c)&&c>=85)items.push({kind:'weather',p:35,date:d,icon:'✓',text:t('insightHighAgreement',{percent:c})});
  }
  for(let i=1;i<dates.length;i++){const prev=cachedAggregateDay(f,dates[i-1],consensusWeights),cur=cachedAggregateDay(f,dates[i],consensusWeights);if(Number.isFinite(prev.tempMax)&&Number.isFinite(cur.tempMax)&&Math.abs(cur.tempMax-prev.tempMax)>=7){const delta=cur.tempMax-prev.tempMax;items.push({kind:'weather',p:80,date:dates[i],icon:weatherIcons.renderMetric('temperature',{size:'small'}),text:t('insightThermalChange',{delta:`${delta>0?'+':''}${fmt(delta,0)}`})});break;}}
  const ev=[];for(const day of evolution?.days||[])for(const [v,x] of Object.entries(day.variables||{})){if(x.trend!=='STABLE'&&Number.isFinite(x.medianAbsDelta)){const threshold=v==='temperature'?1:v==='precipitation'?2:5;if(x.medianAbsDelta>=threshold)ev.push({kind:'revision',p:92,date:day.date,icon:v==='temperature'?weatherIcons.renderMetric('temperature',{size:'small'}):v==='precipitation'?weatherIcons.renderMetric('precipitation',{size:'small'}):weatherIcons.renderMetric('wind',{size:'small'}),text:t('insightForecastRevised',{hours:x.previous?.[0]?.ageHours||'?',trend:trendText(x.trend,x.medianDelta,v==='temperature'?' °C':v==='precipitation'?' mm':' km/h')})});}}
  ...
  const lead=chosen.reduce((best,item)=>!best||item.p>best.p?item:best,null);
  return `<section class="section insights-section" id="insights"><div class="section-card insights-card"><div class="section-head insights-head"><div><h2>${esc(t('forecast_insights_title'))}</h2><p>${esc(t('forecast_insights_subtitle_generic'))}</p></div><span class="insights-status" aria-hidden="true"><i></i></span></div><div class="insights-grid">${chosen.map(x=>`<article class="insight-item ${x===lead?'lead':''} ${x.kind}"><div class="insight-item-head"><span class="insight-icon">${x.icon}</span><span class="insight-date">${esc(dateLabel(x.date,i18n().locale))}</span></div><p>${esc(x.text)}</p></article>`).join('')}</div></div></section>`;
}
