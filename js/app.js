...
function renderGraphicForecastView(cityId){
  ...
  const x=index=>leftPad+index*step+step/2;
  ...
  const timeAxis=points.map((point,index)=>`<div class="graphic-hour ${point.date!==points[index-1]?.date?'day-start':''}" data-graphic-hour-index="${index}" data-graphic-ruler-hour="${attr(timeLabel(point.timestamp))}" data-graphic-ruler-date="${attr(dateLabel(point.date,locale))}" style="left:${(x(index)-step/2).toFixed(1)}px;width:${step}px"><strong>${esc(timeLabel(point.timestamp))}</strong></div>`).join('');
  ...
  const era5Legend=era5MaxLine&&era5MinLine?`<span class="era5-max"><i></i>${esc(t('chart_normals_legend_temp_max'))}</span><span class="era5-min"><i></i>${esc(t('chart_normals_legend_temp_min'))}</span>`:`<span class="era5-pending"><i></i>${esc(t('webNormals'))} · ${esc(state.online?t('webLoading'):t('webUnavailableOffline'))}</span>`,legend=`<div class="graphic-legend"><span class="temp"><i></i>${esc(t('temperature'))}</span><span class="spread"><i></i>${esc(t('graphicDispersion'))}</span>${era5Legend}<span class="rain"><i></i>${esc(t('precipitation'))}</span><span class="wind"><i></i>${esc(t('wind'))}</span><span class="gust"><i></i>${esc(t('gusts'))}</span><span class="graphic-rain-scale"><b>${esc(t('graphicRainProbability'))}</b><i></i><small>0 %</small><small>50 %</small><small>100 %</small></span></div>`;
  ...
  return `<main class="graphic-view">${header}<div class="graphic-scroll" role="region" tabindex="0" aria-label="${attr(t('graphicExploreAria'))}"><div class="graphic-track" style="width:${totalWidth}px" data-graphic-ruler-track data-graphic-left-pad="${leftPad}" data-graphic-step="${step}" data-graphic-count="${points.length}"><div class="graphic-chart graphic-plots" style="width:${totalWidth}px"><section class="graphic-plot graphic-plot-temperature">${tint}<svg class="graphic-chart-svg" width="${totalWidth}" height="${tempPlotHeight}" viewBox="0 0 ${totalWidth} ${tempPlotHeight}" preserveAspectRatio="none" aria-hidden="true">${tempGrid}${dayBreaks(0,tempPlotHeight)}${bandPath?`<path class="graphic-temp-band" d="${bandPath}"/>`:''}${era5MaxLine?`<path class="graphic-era5-line max" d="${era5MaxLine}"/>`:''}${era5MinLine?`<path class="graphic-era5-line min" d="${era5MinLine}"/>`:''}<path class="graphic-temp-line" d="${tempLine}"/></svg>${tempAxisMarkup}${tempTargets}</section><section class="graphic-plot graphic-plot-rain"><svg class="graphic-chart-svg" width="${totalWidth}" height="${rainPlotHeight}" viewBox="0 0 ${totalWidth} ${rainPlotHeight}" preserveAspectRatio="none" aria-hidden="true">${rainGrid}${dayBreaks(0,rainPlotHeight)}<line class="graphic-rain-baseline" x1="${leftPad}" x2="${totalWidth-rightPad}" y1="${rainBottom}" y2="${rainBottom}"/></svg>${rainAxisMarkup}${rainTargets}</section><section class="graphic-plot graphic-plot-wind"><svg class="graphic-chart-svg" width="${totalWidth}" height="${windPlotHeight}" viewBox="0 0 ${totalWidth} ${windPlotHeight}" preserveAspectRatio="none" aria-hidden="true">${windGrid}${dayBreaks(0,windPlotHeight)}<line class="graphic-wind-baseline" x1="${leftPad}" x2="${totalWidth-rightPad}" y1="${windBottom}" y2="${windBottom}"/><path class="graphic-gust-line" d="${gustLine}"/><path class="graphic-wind-line" d="${windLine}"/></svg>${windAxisMarkup}${windTargets}</section><div class="graphic-ruler" data-graphic-ruler hidden aria-hidden="true"><span class="graphic-ruler-badge" data-graphic-ruler-label></span></div></div><div class="graphic-legend-row">${legend}</div><div class="graphic-time-axis" style="width:${totalWidth}px"><div class="graphic-hour-row">${timeAxis}</div><div class="graphic-date-row">${dateAxis}</div></div>${vigilance}</div></div><div class="graphic-tooltip-layer" data-graphic-tooltip-layer hidden><div class="graphic-floating-tooltip"></div></div></main>`;
}
...
function graphicRulerContext(plots){
  ...
  context={track,ruler,scroller,label:ruler.querySelector('[data-graphic-ruler-label]'),hours:[...track.querySelectorAll('[data-graphic-hour-index]')].map(hour=>[hour.dataset.graphicRulerDate||'',hour.dataset.graphicRulerHour||'']),viewportLeft:null,lastIndex:-1};graphicRulerContextCache.set(plots,context);return context;
}
