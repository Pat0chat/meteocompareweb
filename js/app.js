...
function homeRainTimelineEvents(points){
  const events=[];let active=null;
  const wet=point=>(Number.isFinite(point?.precipitationPercent)&&point.precipitationPercent>=30)||isWetPrecipitation(point?.precipitationConditionalMm);
  points.forEach((point,index)=>{
    if(!wet(point)){if(active){events.push(active);active=null;}return;}
    if(!active)active={start:index,end:index,maxProbability:null,maxAmount:null};
    active.end=index;
    if(Number.isFinite(point.precipitationPercent))active.maxProbability=Math.max(active.maxProbability??0,point.precipitationPercent);
    if(Number.isFinite(point.precipitationConditionalMm))active.maxAmount=Math.max(active.maxAmount??0,point.precipitationConditionalMm);
  });
  if(active)events.push(active);
  return events;
}
function homeTemperaturePath(coords){
  let path='',open=false;
  for(const point of coords){
    if(!point){open=false;continue;}
    path+=`${open?' L':'M'} ${point.x.toFixed(2)} ${point.y.toFixed(2)}`;open=true;
  }
  return path;
}
function renderHomeWeatherTimeline(points,chartKey='home'){
  ...
  const coords=points.map((point,index)=>Number.isFinite(point.temperatureC)?{x:x(index),y:y(point.temperatureC),temp:point.temperatureC}:null),path=homeTemperaturePath(coords),allFinite=coords.every(Boolean),area=allFinite&&path?`${path} L ${coords.at(-1).x.toFixed(2)} ${plotBottom} L ${coords[0].x.toFixed(2)} ${plotBottom} Z`:'';
  const gradientStops=points.map((point,index)=>`<stop offset="${points.length===1?0:(index/(points.length-1))*100}%" stop-color="${attr(homeTemperatureHeatColor(point.temperatureC))}"/>`).join('');
  const columns=points.map((point,index)=>`<rect class="home-weather-heat-column" x="${index*step}" y="0" width="${step}" height="${height}" fill="${attr(homeTemperatureHeatColor(point.temperatureC))}"/>`).join('');
  const pointMarks=points.map((point,index)=>{
    ...
    return `<g class="home-temperature-point"><title>${esc(tooltip)}</title><circle cx="${x(index)}" cy="${y(point.temperatureC).toFixed(2)}" r="3.8" fill="${attr(homeTemperatureHeatColor(point.temperatureC))}"/><text x="${x(index)}" y="${Math.max(12,y(point.temperatureC)-8).toFixed(2)}" text-anchor="middle">${esc(`${fmt(point.temperatureC)}°`)}</text></g>`;
  }).join('');
  const hourAxis=points.map(point=>{
    ...
    return `<div class="home-weather-axis-hour" style="--heat-color:${attr(homeTemperatureHeatColor(temp))}" title="${attr(tooltip)}"><span class="home-weather-axis-condition">${point.condition?aggregateConditionMarkup(point,'tiny'):'<span class="home-weather-axis-empty" aria-hidden="true">—</span>'}</span><time>${esc(timeLabel(point.timestamp))}</time></div>`;
  }).join('');
  const rainEvents=homeRainTimelineEvents(points),eventStarts=new Set(rainEvents.map(event=>event.start)),eventEnds=new Set(rainEvents.map(event=>event.end));
...
function renderDetailChronoTimeline(points,mode){
  ...
  const coords=points.map((point,index)=>{const value=detailChronoTemperature(point,mode);return Number.isFinite(value)?{x:x(index),y:y(value),value}:null;}),path=homeTemperaturePath(coords),allFinite=coords.every(Boolean),area=allFinite&&path?`${path} L ${coords.at(-1).x.toFixed(2)} ${plotBottom} L ${coords[0].x.toFixed(2)} ${plotBottom} Z`:'';
  const stops=points.map((point,index)=>`<stop offset="${points.length===1?0:(index/(points.length-1))*100}%" stop-color="${attr(homeTemperatureHeatColor(detailChronoTemperature(point,mode)))}"/>`).join('');
  const heat=points.map((point,index)=>`<rect class="detail-chrono-heat-column" x="${index*step}" y="0" width="${step}" height="${height}" fill="${attr(homeTemperatureHeatColor(detailChronoTemperature(point,mode)))}"/>`).join('');
  const tempMarks=points.map((point,index)=>{const value=detailChronoTemperature(point,mode);if(!Number.isFinite(value))return '';const low=mode==='DAILY'&&Number.isFinite(point.tempMinC)?point.tempMinC:null,high=mode==='DAILY'&&Number.isFinite(point.tempMaxC)?point.tempMaxC:null,label=mode==='DAILY'&&Number.isFinite(low)&&Number.isFinite(high)?`${fmt(high)}° / ${fmt(low)}°`:`${fmt(value)}°`;return `<g class="detail-chrono-temp-point"><circle cx="${x(index)}" cy="${y(value).toFixed(2)}" r="4" fill="${attr(homeTemperatureHeatColor(value))}"/><text x="${x(index)}" y="${Math.max(14,y(value)-9).toFixed(2)}" text-anchor="middle">${esc(label)}</text></g>`;}).join('');
  ...
  const rainEvents=homeRainTimelineEvents(points),starts=new Set(rainEvents.map(event=>event.start)),ends=new Set(rainEvents.map(event=>event.end));
