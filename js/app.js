...
async function hydrateRadarModal(){
  ...
  try{
    ...
    const {t,locale}=i18n(),initialMode=state.modal.radarMode||'observation',initialRange=state.modal.radarRange||'near';await radar.mountRadarModal({root,city,forecast:state.forecasts[cityId]||null,forecastOptions:forecastEngineContext(cityId),t,locale,initialMode,initialRange,onRangeChange:range=>{if(state.modal?.type==='radar'&&state.modal.cityId===cityId)state.modal.radarRange=range;void trackAnalyticsEvent('Rain Radar Range Changed',state.route,{range});},onModeChange:mode=>{if(state.modal?.type==='radar'&&state.modal.cityId===cityId)state.modal.radarMode=mode;void trackAnalyticsEvent('Rain Radar Mode Changed',state.route,{mode});}});
