...
function renderHomeMiniTimeline(points){
  ...
  return `<div class="home-mini-timeline-wrap"><div class="home-mini-timeline" aria-label="${esc(t('homeMiniTimelineAria'))}">${points.map(point=>{
    ...
    return `<div class="home-mini-hour ${Number.isFinite(prob)&&prob>=30?'wet':''}" style="--heat-color:${heat}" title="${attr(tooltip)}"><span class="home-mini-time">${esc(timeLabel(point.timestamp))}</span><span class="home-mini-condition">${condition?conditionMarkup(condition,'tiny',Boolean(point.conditionInferred)):'<span class="home-mini-condition-empty" aria-hidden="true">—</span>'}</span><strong>${Number.isFinite(temp)?`${fmt(temp)}°`:'—'}</strong><span class="home-mini-rain">${Number.isFinite(prob)&&prob>=20?`${weatherIcons.renderMetric('precipitation',{size:'micro'})} ${Math.round(prob)}%`:'·'}</span><small>${isWetPrecipitation(amount)?`${fmt(amount,1)} mm`:'—'}</small></div>`;
...
function addCityFromSearch(id){
  ...
  if(becameFavorite){const toastId=`city-add-${target.id}`;toast(i18n().t('homeCityAddedLoading',{city:target.name}),{id:toastId,type:'loading',title:i18n().t('homeCityAddedTitle')});void finishAddedCityForecast(target,toastId);}
}
