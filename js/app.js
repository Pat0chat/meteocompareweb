...
async function hydrateRadarModal(){
  ...
  try{
    ...
    const {t,locale}=i18n();await radar.mountRadarModal({root,city,forecast:state.forecasts[cityId]||null,forecastOptions:forecastEngineContext(cityId),t,locale,onRangeChange:range=>void trackAnalyticsEvent('Rain Radar Range Changed',state.route,{range})});
...
function renderAbout(){
  const {t}=i18n(),install=pwaInstallGuidance(),selectedEngine=state.settings.forecastEngine||'MULTI_CONSENSUS';
  const steps=[[t('aboutStepModelsTitle'),t('aboutStepModelsBody')],[t('aboutStepCompareTitle'),t('aboutStepCompareBody')],[t('aboutStepEngineTitle'),t('aboutStepEngineBody')],[t('aboutStepReadTitle'),t('aboutStepReadBody')]];
  const engineKey=engine=>({MULTI_CONSENSUS:'Multi',CALIBRATION:'Calibration',SCENARIOS:'Scenarios',ADAPTIVE:'Adaptive'}[engine]||'Multi');
  const engineCards=FORECAST_ENGINES.map(engine=>{const key=engineKey(engine),active=engine===selectedEngine;return `<article class="about-engine-card ${active?'selected-engine':''}"><div class="about-engine-head"><h3>${esc(t(`forecastEngine${key}`))}</h3>${active?`<span>${esc(t('forecastEngineSelected'))}</span>`:''}</div><p>${esc(t(`forecastEngine${key}Desc`))}</p></article>`;}).join('');
  return `<main class="page about-page about-page-simple">
    ...
    <section class="section-card about-engines"><div class="section-head"><div><h2>${esc(t('aboutForecastEnginesTitle'))}</h2><p>${esc(t('aboutForecastEnginesBody'))}</p></div><span class="about-engine-current">${esc(t('aboutForecastEngineCurrent',{engine:forecastEngineName(selectedEngine)}))}</span></div><div class="about-engine-grid">${engineCards}</div><div class="about-callout">${esc(t('aboutForecastEnginesCallout'))}</div></section>
...
function renderSeoDetailTitleContext(city){
  ...
  return `<div class="detail-seo-context"><h2>${esc(t('seoCityContextTitle',{city:seo.name}))}</h2><p>${esc(t('seoCityContextLead',{city:seo.name}))} ${esc(t('seoCityContextLocation',{city:seo.name,department:seo.department,region:seo.region}))}</p></div>`;
}
function renderHome(){
...
function renderForecastEngineCompareAction(){
  ...
  return `<div class="forecast-engine-compare-slot"><button class="forecast-engine-compare-card" data-action="open-engine-comparison"><span class="forecast-engine-overview-symbol" aria-hidden="true">Σ</span><span class="forecast-engine-overview-copy"><span class="forecast-engine-compare-kicker">${esc(t('forecastEngineModalTitle'))}</span><strong>${esc(t('forecastEngineCompare'))}</strong><small>${esc(t('forecastEngineCompareIntro'))}</small><span class="forecast-engine-compare-meta"><b>${esc(t('forecastEngineActiveLabel',{engine:forecastEngineName(engine)}))}</b><span class="forecast-engine-marks" aria-hidden="true">${engineMarks}</span></span></span><span class="forecast-engine-overview-chevron" aria-hidden="true">→</span></button></div>`;
}
...
function renderForecastEngineLineChart(dates,matrix,selected,{key,label,unit='',digits=1}){
  ...
  const w=720,h=238,pad={l:48,r:18,t:18,b:38},rawMin=Math.min(...finite),rawMax=Math.max(...finite),rawSpan=Math.max(.5,rawMax-rawMin),min=rawMin-rawSpan*.10,max=rawMax+rawSpan*.10,span=Math.max(.5,max-min),x=i=>pad.l+(dates.length<=1?0:i/(dates.length-1))*(w-pad.l-pad.r),y=v=>pad.t+(max-v)/span*(h-pad.t-pad.b),ticks=Array.from({length:4},(_,i)=>min+(span*i/3));
...
function renderForecastEngineDivergenceTimeline(dates,matrix){
  const {t}=i18n();
  const items=dates.map(date=>{const aggs=FORECAST_ENGINES.map(engine=>matrix[engine]?.[date]).filter(Boolean),temp=forecastEngineSpread(aggs.map(a=>a.tempMax)),rain=forecastEngineSpread(aggs.map(a=>forecastEngineMetricValue(a,'precipExpected'))),wind=forecastEngineSpread(aggs.map(a=>a.wind)),conditions=new Set(aggs.map(a=>a.condition).filter(Boolean)).size,score=Math.max(temp/4,rain/8,wind/15,conditions>1?.42:0),level=score>=.75?'high':score>=.35?'medium':'low',label=t(level==='high'?'forecastEngineDivergenceHigh':level==='medium'?'forecastEngineDivergenceMedium':'forecastEngineDivergenceLow'),width=Math.max(8,Math.min(100,Math.round(score*100)));
    return `<div class="forecast-engine-divergence-day ${level}"><div class="forecast-engine-divergence-date">${esc(forecastEngineShortDate(date))}</div><div class="forecast-engine-divergence-meter" aria-hidden="true"><i style="--divergence:${width}%"></i></div><strong>${esc(label)}</strong><small>ΔT ${fmt(temp,1)}° · Δ ${esc(t('precipitation'))} ${fmt(rain,1)} mm · Δ ${esc(t('wind'))} ${fmt(wind,0)} km/h</small></div>`;
  }).join('');
...
function renderForecastEngineComparisonModal(cityId){
  ...
  const visualCharts=`<div class="forecast-engine-chart-grid">${renderForecastEngineLineChart(dates,matrix,selected,{key:'tempMax',label:t('forecastEngineTempTrend'),unit:' °C',digits:1})}${renderForecastEngineLineChart(dates,matrix,selected,{key:'precipExpected',label:t('forecastEngineRainTrend'),unit:' mm',digits:1})}</div>`;
  return `<div class="forecast-engine-overview">${engineCards}</div><div class="banner info forecast-engine-reading"><b>${esc(t('forecastEngineReading'))}</b><span>${esc(t('forecastEngineReadingBody'))}</span></div><section class="forecast-engine-visual-section"><div class="forecast-engine-visual-head"><div><h3>${esc(t('forecastEngineVisualOverview'))}</h3><p>${esc(t('forecastEngineVisualOverviewBody'))}</p></div><span>${esc(city.name)}</span></div>${visualCharts}${renderForecastEngineDivergenceTimeline(dates,matrix)}</section><section class="forecast-engine-comparison-days"><div class="forecast-engine-comparison-title"><div><h3>${esc(t('forecastEngineInspectDetails'))}</h3><p>${esc(t('forecastEngineDailyOutlook'))}</p></div><span>${esc(city.name)}</span></div>${dayTables||`<div class="empty-state compact">${esc(t('insufficientData'))}</div>`}</section>`;
}
...
function renderModal(){
  ...
  if(state.modal.type==='forecastEngines'){const c=state.cities.find(x=>x.id===state.modal.cityId);return `<div class="modal-backdrop" data-action="modal-backdrop"><div class="modal modal-wide forecast-engine-modal" role="dialog" aria-modal="true" aria-labelledby="modal-title"><div class="modal-head forecast-engine-modal-head"><div><h2 id="modal-title">${esc(t('forecastEngineModalTitle'))}</h2><span class="small">${esc(c?.name||t('city'))} · ${esc(t('forecastEngineModalSubtitle'))}</span></div><button class="icon-btn" data-action="close-modal" aria-label="${esc(t('close'))}">×</button></div><div class="modal-content forecast-engine-modal-content">${renderForecastEngineComparisonModal(state.modal.cityId||state.route.id)}</div></div></div>`;}
...
function handleAction(e){
  ...
  else if(action==='open-engine-comparison'){if(state.route.name!=='city'||!state.route.id)return;lastFocusedBeforeModal=document.activeElement;state.modal={type:'forecastEngines',cityId:state.route.id};render();}
