...
const FORECAST_ENGINE_UI = Object.freeze({
  MULTI_CONSENSUS: Object.freeze({ key: 'Multi', modeClass: 'multi', symbol: 'Σ' }),
  CALIBRATION: Object.freeze({ key: 'Calibration', modeClass: 'calibration', symbol: '◎' }),
  SCENARIOS: Object.freeze({ key: 'Scenarios', modeClass: 'scenarios', symbol: '⑂' }),
  ADAPTIVE: Object.freeze({ key: 'Adaptive', modeClass: 'adaptive', symbol: '⇄' }),
});
function forecastEngineUi(engine){return FORECAST_ENGINE_UI[engine]||FORECAST_ENGINE_UI.MULTI_CONSENSUS;}
...
const runtime=new ApplicationKernel({
  ...
  featureLoaders:{
    bias:()=>import('./features/bias.js'),evolution:()=>import('./features/evolution.js'),
    comparison:()=>import('./features/comparison.js'),marine:()=>import('./features/marine.js'),radar:()=>import('./features/radar.js'),
  },
  analysisLoaders:{bias:loadBias,evolution:loadEvolution,normals:loadNormals,marine:loadMarine},
});
const state=runtime.state;
function analyticsRouteSnapshot(route=state.route){
  if(route?.name!=='city')return route;
  return {...route,view:{...(route.view||{}),tab:state.settings.detailTab,mode:state.settings.detailViewMode,metric:state.settings.confidenceMetric,horizon:state.settings.chartHorizon,timeline:state.settings.timelineMode,timelineLayout:state.settings.timelineLayout,compareModels:[...(state.compareModelIds||[])]}};
}
function trackCurrentPageView(route=state.route){return trackPageView(analyticsRouteSnapshot(route));}
...
const graphicTooltipContentCache = new Map();
let graphicRulerFrame = 0;
let pendingGraphicRulerEvent = null;
const graphicRulerContextCache = new WeakMap();
let graphicDeferredRenderKey = null;
let graphicDeferredRenderReady = false;
let graphicDeferredRenderFrame = 0;
let graphicDeferredRenderFrame2 = 0;
let graphicDeferredRenderToken = 0;
...
let i18nCache = null;
let documentMetaSignature = null;
...
const {numberFormatters,dateTimeFormatters,forecastViews:forecastViewCache,seriesIndexes:seriesIndexCache,chartHoverData:chartHoverDataCache,routeScrollPositions}=runtime.cache;
...
function warmCityFeatures(){void loadFeature('bias');void loadFeature('evolution');}
...
function ensureCityAnalysisLoaded(cityId){ensureEvolutionLoaded(cityId);ensureBiasLoaded(cityId);ensureNormalsLoaded(cityId);const city=state.cities.find(c=>c.id===cityId);if(city?.marineEnabled)ensureMarineLoaded(cityId);}
...
async function registerPwaServiceWorker(){
  ...
  try{
    ...
    scheduleIdle(()=>void warmOptionalPwaShell());
  }
  catch(err){console.warn('Service worker:',err);}
}
async function warmOptionalPwaShell(){
  try{
...
function init() {
  ...
  app.addEventListener('pointermove', handleGraphicRulerPointerMoveScheduled, {passive:true});
  ...
  app.addEventListener('pointerout', handleGraphicRulerPointerOut, {passive:true});
...
function startAutoRefreshTimer(){
  ...
  autoRefreshTimer=setInterval(()=>{if(!routeShowsWeatherActivity())return;const nextKey=viewTimeKey();if(lastViewTimeKey!==null&&nextKey!==lastViewTimeKey)render();lastViewTimeKey=nextKey;refreshDueCities();},60_000);
}
function viewTimeCities(){
  if(state.route.name==='city'||state.route.name==='bias')return state.cities.filter(city=>city.id===state.route.id);
  if(state.route.name==='compare'){const ids=new Set(state.route.ids||[]);return state.cities.filter(city=>ids.has(city.id));}
  return favoriteCities();
}
function viewTimeKey(){const keys=viewTimeCities().map(city=>roundedHourLocal(city.timezone||state.forecasts[city.id]?.city?.timezone||'UTC'));return keys.join('|')||new Date().toISOString().slice(0,13);}
...
async function hydrateForecastStorage(){
  ...
  const tasks=Array.from({length:workers},async()=>{
    while(index<cities.length){
      ...
      if(state.forecasts[city.id]){await yieldToMainThread();continue;}
      ...
      else if(f){state.forecasts[city.id]=f;changed=true;}
...
function applyRouteViewState(route){
  ...
  if(['TEMPERATURE','PRECIPITATION_PROBABILITY','PRECIPITATION','CLOUD','WIND','GUST'].includes(v.metric))state.settings.confidenceMetric=v.metric;
  ...
  if(['COLUMNS','CHRONO'].includes(v.timelineLayout))state.settings.timelineLayout=v.timelineLayout;
...
function syncCityViewUrl(){
  ...
  const q=new URLSearchParams();q.set('tab',state.settings.detailTab||'CONDITIONS');q.set('mode',state.settings.detailViewMode||'DAILY');q.set('metric',state.settings.confidenceMetric||'TEMPERATURE');q.set('h',String(state.settings.chartHorizon||168));q.set('timeline',state.settings.timelineMode||'HOURLY');q.set('timelineView',state.settings.timelineLayout||'COLUMNS');
...
function scrollControlSelector(target){
  ...
  const keys=[['confidenceMetric','data-confidence-metric'],['chartHorizon','data-chart-horizon'],['detailMode','data-detail-mode'],['detailTab','data-detail-tab'],['timelineMode','data-timeline-mode'],['timelineLayout','data-timeline-layout'],['theme','data-theme'],['language','data-language'],['refreshInterval','data-refresh-interval'],['modelSort','data-model-sort'],['modelToggle','data-model-toggle'],['biasRefreshCity','data-bias-refresh-city'],['compareModel','data-compare-model'],['density','data-density'],['evolutionVariable','data-evolution-variable'],['reliabilityVariable','data-reliability-variable'],['action','data-action']];
...
function forecastEngineModalControlSelector(attribute,value){
  const escaped=String(value??'').replace(/\\/g,'\\\\').replace(/"/g,'\\"');
  return `[${attribute}="${escaped}"]`;
}
function captureForecastEngineModalView(){
  const modal=document.querySelector?.('.forecast-engine-modal'),timeline=modal?.querySelector?.('.forecast-engine-divergence-track');
  return {pageY:currentScrollY(),scrollTop:Math.max(0,Number(modal?.scrollTop)||0),timelineScrollLeft:Math.max(0,Number(timeline?.scrollLeft)||0)};
}
function restoreForecastEngineModalView(view,focusSelector=null){
  if(!view)return;
  const restore=()=>{if(state.modal?.type!=='forecastEngines')return;const modal=document.querySelector?.('.forecast-engine-modal');if(!modal)return;modal.scrollTop=view.scrollTop;const timeline=modal.querySelector?.('.forecast-engine-divergence-track');if(timeline)timeline.scrollLeft=view.timelineScrollLeft;};
  restore();
  queueMicrotask(()=>{restore();const control=focusSelector?document.querySelector?.(focusSelector):null;try{control?.focus?.({preventScroll:true});}catch{}});
  requestAnimationFrame(()=>{restore();requestAnimationFrame(restore);});
}
function rerenderForecastEngineModalPreservingView(focusSelector=null){
  const view=captureForecastEngineModalView();
...
function applyRouteFromLocation(scrollY=0,options={}){
  ...
  if(options.newRoute)void trackCurrentPageView(state.route);
  onRouteSettled();
}
...
function i18n(){
  ...
  if(key!==i18nCacheKey){i18nCacheKey=key;i18nCache=makeI18n(state.settings.language);numberFormatters.clear();dateTimeFormatters.clear();documentMetaSignature=null;}
  return i18nCache;
}
...
function syncDocumentMeta(){
  const {t}=i18n(),city=state?.route?.name==='city'?state.cities.find(row=>row.id===state.route.id):null,seoCity=city?matchSeoCity(city):null,meta=city?cityMetaCopy(city):{title:t('siteTitle'),description:t('siteDescription')},canonicalHref=seoCity?`https://meteocompare.app${cityPublicPath(seoCity)}`:'https://meteocompare.app/',robotsContent=seoCity||state.route.name==='home'?'index,follow,max-image-preview:large':'noindex,follow',manifestHref=`manifest.${i18n().lang}.webmanifest`,signature=[meta.title,meta.description,canonicalHref,robotsContent,manifestHref].join('\u0000');
  if(signature===documentMetaSignature)return;documentMetaSignature=signature;
  ...
  const canonical=document.querySelector('link[rel="canonical"]');if(canonical)canonical.setAttribute('href',canonicalHref);
  const robots=document.querySelector('meta[name="robots"]');if(robots)robots.setAttribute('content',robotsContent);
  const manifest=document.querySelector('link[rel="manifest"]');if(manifest)manifest.setAttribute('href',manifestHref);
}
function syncStickyOffsets(){
  ...
  const update=()=>{
    ...
    if(topbarHeight===stickyTopbarHeight)return;stickyTopbarHeight=topbarHeight;
...
function dateTimeFormatter(locale,options){
  const key=`${locale}|${Object.entries(options||{}).sort(([a],[b])=>a.localeCompare(b)).map(([name,value])=>`${name}:${String(value)}`).join('|')}`;
  let formatter=dateTimeFormatters.get(key);
  if(!formatter){formatter=new Intl.DateTimeFormat(locale,options);dateTimeFormatters.set(key,formatter);}
  return formatter;
}
function formatDateTime(value,options,locale=i18n().locale){return dateTimeFormatter(locale,options).format(value);}
function fmt(v,d=0){
...
function storeBrandIcon(kind,size=24){
  if(kind==='google-play')return `<svg class="store-brand-icon google-play-brand-icon" viewBox="0 0 512 565" width="${size}" height="${size}" aria-hidden="true" focusable="false"><path fill="#00c7f2" d="M58 57C47 69 41 86 41 106v353c0 20 7 36 19 48l214-225L58 57Z"/><path fill="#00d17d" d="M75 49c13-5 27-2 42 7l294 173-137 53L75 49Z"/><path fill="#ffc928" d="m411 229 58 34c23 13 23 26 0 39l-60 35-135-55 137-53Z"/><path fill="#ff3b4b" d="m409 337-292 172c-17 10-32 10-49 0l206-227 135 55Z"/></svg>`;
  if(kind==='fdroid')return `<svg class="store-brand-icon fdroid-brand-icon" viewBox="0 0 512 512" width="${size}" height="${size}" aria-hidden="true" focusable="false"><path d="M92 83 45 31M420 83l47-52" fill="none" stroke="#7cb342" stroke-width="25" stroke-linecap="round"/><rect x="52" y="65" width="408" height="137" rx="43" fill="#aeea00"/><circle cx="159" cy="133" r="34" fill="#fff"/><circle cx="353" cy="133" r="34" fill="#fff"/><rect x="52" y="218" width="408" height="270" rx="43" fill="#2d86d4"/><path d="M347 354a91 91 0 1 1-21-58" fill="none" stroke="#1957a6" stroke-width="29" stroke-linecap="round"/><path d="M177 354h79" fill="none" stroke="#1957a6" stroke-width="29" stroke-linecap="round"/><circle cx="257" cy="354" r="35" fill="#1957a6"/></svg>`;
  return uiIcon('download',size);
}
...
function monitorAnalyticsItem(){
  ...
  if(runtime.state==='error'){const key=runtime.lastDeliveryError==='network'?'monitorAnalyticsDeliveryError':'monitorAnalyticsError';return {key:'analytics',icon:'chart',title:t('monitorAnalytics'),state:'warning',detail:t(key)};}
  if(runtime.state==='loaded'){
    const age=Number.isFinite(Number(runtime.lastDeliveryAt))?formatExactAge(new Date(Number(runtime.lastDeliveryAt)).toISOString()):null;
    return {key:'analytics',icon:'chart',title:t('monitorAnalytics'),state:'ok',detail:age?t('monitorAnalyticsDelivered',{age}):t('monitorAnalyticsActive')};
  }
...
function systemMonitorSnapshot(){
  const {t}=i18n(),connection={key:'connection',icon:'network',title:t('monitorConnection'),state:state.online?'ok':'error',detail:t(state.online?'monitorOnline':'monitorOffline')},items=[connection,monitorForecastItem(),monitorWorkerItem(),monitorVigilanceItem(),monitorAnalyticsItem(),monitorPwaItem()];
...
function refreshSystemMonitorDom(){
  const current=app?.querySelector?.('.topbar-system-monitor');if(!current)return;
  // Only preserve an explicit click/touch opening. Hover/focus visibility is CSS-only:
  // turning :hover into .is-open here made the monitor stick open after a refresh.
  const wasOpen=Boolean(current.classList?.contains?.('is-open')),activeAction=current.contains?.(document.activeElement)?document.activeElement?.dataset?.action:null;
  current.outerHTML=renderSystemMonitor(wasOpen);
  if(activeAction)app?.querySelector?.(`.topbar-system-monitor [data-action="${activeAction}"]`)?.focus?.();
}
...
function viewCache(f){let c=forecastViewCache.get(f);if(!c){c={days:new Map(),scenarios:new Map(),bands:new Map(),timelines:new Map(),evolutionSource:null,evolutionReport:null,biasSource:null,biasToday:null,biasReport:null,visibleModelIds:null,newestRunTimestamp:undefined};forecastViewCache.set(f,c);}else if(!c.timelines)c.timelines=new Map();return c;}
...
function cachedTimelinePoints(f,mode='HOURLY',now=new Date(),options=null){
  const o=normalizeForecastOptions(options),timezone=f?.city?.timezone||f?.timezone||'UTC',hourly=mode==='HOURLY',horizon=Math.max(1,Math.min(24*10,Number(o?.hourlyHorizonHours)||24)),anchorKey=hourly?roundedHourLocal(timezone,now):cityToday(timezone,now),key=`${mode}|${anchorKey}|${hourly?horizon:''}|${o?.includeModelValues?'models':'compact'}|${forecastOptionsSignature(o)}`,c=viewCache(f);
  if(c.timelines.has(key))return c.timelines.get(key);
  const points=buildTimelinePoints(f,mode,now,o);
  if(c.timelines.size>=8)c.timelines.clear();
  c.timelines.set(key,points);return points;
}
...
function resetGraphicDeferredRender(){
  graphicDeferredRenderToken++;graphicDeferredRenderKey=null;graphicDeferredRenderReady=false;
  if(graphicDeferredRenderFrame){cancelAnimationFrame(graphicDeferredRenderFrame);graphicDeferredRenderFrame=0;}
  if(graphicDeferredRenderFrame2){cancelAnimationFrame(graphicDeferredRenderFrame2);graphicDeferredRenderFrame2=0;}
}
function scheduleGraphicDeferredRender(key){
  const token=++graphicDeferredRenderToken;
  graphicDeferredRenderFrame=requestAnimationFrame(()=>{graphicDeferredRenderFrame=0;graphicDeferredRenderFrame2=requestAnimationFrame(()=>{graphicDeferredRenderFrame2=0;if(token!==graphicDeferredRenderToken||graphicDeferredRenderKey!==key||!isGraphicForecastRoute())return;graphicDeferredRenderReady=true;render({immediate:true});});});
}
function renderGraphicBuildShell(city){
  const {t}=i18n();return `<main class="graphic-view graphic-view-building" aria-busy="true">${graphicForecastHeader(city)}<div class="graphic-build-shell" role="status" aria-live="polite"><div class="graphic-build-card"><div class="loader" aria-hidden="true"></div><strong>${esc(t('graphicBuildingView'))}</strong><span>${esc(t('graphicBuildingViewHint'))}</span></div><div class="graphic-build-preview" aria-hidden="true"><i></i><i></i><i></i><b></b><em></em></div></div></main>`;
}
function renderNow(){
  const scrollDirective=pendingScrollDirective;pendingScrollDirective=null;
  resetGraphicInteractions();
  ...
  const graphicView=isGraphicForecastRoute(),graphicCity=graphicView?state.cities.find(row=>row.id===state.route.id):null,graphicKey=graphicView&&graphicCity?`city:${graphicCity.id}`:null;
...
function renderAbout(){
  const {t}=i18n();
  const engineCards=FORECAST_ENGINES.map(engine=>{const meta=forecastEngineUi(engine);return `<article class="about-visual-engine ${meta.modeClass}"><span class="about-visual-engine-symbol" aria-hidden="true">${meta.symbol}</span><div class="about-visual-engine-copy"><h3>${esc(t(`forecastEngine${meta.key}`))}</h3><p>${esc(t(`aboutVisualEngine${meta.key}Short`))}</p></div></article>`;}).join('');
  ...
  return `<main class="page about-page about-visual-page">
    ...
    <section class="section-card about-community" aria-labelledby="about-community-title"><div class="about-community-icon" aria-hidden="true">${blueskyIcon(28)}</div><div class="about-community-copy"><h2 id="about-community-title">${esc(t('aboutCommunityTitle'))}</h2><p>${esc(t('aboutCommunityBody'))}</p><span class="about-community-handle">@meteocompare.bsky.social</span></div><a class="btn tonal about-community-action" data-analytics-link="bluesky" href="https://bsky.app/profile/meteocompare.bsky.social" target="_blank" rel="noopener noreferrer"><span class="btn-icon bluesky-btn-icon" aria-hidden="true">${blueskyIcon(17)}</span>${esc(t('aboutCommunityAction'))}</a></section>
  </main>`;
}

function homeTimelinePoints(f,forecastOptions,maxPoints=12,now=new Date(),stepHours=1){
  return selectRegularTimelinePoints(cachedTimelinePoints(f,'HOURLY',now,normalizeForecastOptions(forecastOptions)),maxPoints,stepHours);
}
...
function renderHomeWeatherTimeline(points,chartKey='home'){
  const {t}=i18n();
  if(!points.length)return '';
  const step=64,width=points.length*step,height=92,plotTop=20,plotBottom=76,finiteTemps=points.map(point=>point.temperatureC).filter(Number.isFinite),rawMin=finiteTemps.length?Math.min(...finiteTemps):0,rawMax=finiteTemps.length?Math.max(...finiteTemps):1,center=(rawMin+rawMax)/2,span=Math.max(4,rawMax-rawMin+2),min=center-span/2,max=center+span/2,y=temp=>plotTop+(max-temp)*(plotBottom-plotTop)/(max-min),x=index=>index*step+step/2,safeKey=String(chartKey||'home').replace(/[^a-zA-Z0-9_-]/g,'-'),gradientId=`home-temp-gradient-${safeKey}`;
  const coords=points.map((point,index)=>Number.isFinite(point.temperatureC)?{x:x(index),y:y(point.temperatureC),temp:point.temperatureC}:null),path=svgPathFromPoints(coords),allFinite=coords.every(Boolean),area=allFinite&&path?`${path} L ${coords.at(-1).x.toFixed(2)} ${plotBottom} L ${coords[0].x.toFixed(2)} ${plotBottom} Z`:'';
  const gradientStops=points.map((point,index)=>`<stop offset="${points.length===1?0:(index/(points.length-1))*100}%" stop-color="${attr(temperatureHeatColor(point.temperatureC))}"/>`).join('');
  const columns=points.map((point,index)=>`<rect class="home-weather-heat-column" x="${index*step}" y="0" width="${step}" height="${height}" fill="${attr(temperatureHeatColor(point.temperatureC))}"/>`).join('');
  const pointMarks=points.map((point,index)=>{
    if(!Number.isFinite(point.temperatureC))return '';
    const conditionInfo=localizedConditionInfo(point.condition),prob=point.precipitationPercent,amount=point.precipitationConditionalMm,tooltip=`${timeLabel(point.timestamp)} · ${conditionInfo.label} · ${fmt(point.temperatureC,1)} °C${Number.isFinite(prob)?` · ${t('homeRainProbabilityValue',{percent:Math.round(prob)})}`:''}${isWetPrecipitation(amount)?` · ${t('homeWatchRainAmount',{amount:fmt(amount,1)})}`:''}`;
    return `<g class="home-temperature-point"><title>${esc(tooltip)}</title><circle cx="${x(index)}" cy="${y(point.temperatureC).toFixed(2)}" r="3.8" fill="${attr(temperatureHeatColor(point.temperatureC))}"/><text x="${x(index)}" y="${Math.max(12,y(point.temperatureC)-8).toFixed(2)}" text-anchor="middle">${esc(`${fmt(point.temperatureC)}°`)}</text></g>`;
  }).join('');
  const hourAxis=points.map(point=>{
    const conditionInfo=localizedConditionInfo(point.condition),prob=point.precipitationPercent,amount=point.precipitationConditionalMm,temp=point.temperatureC,tooltip=`${timeLabel(point.timestamp)} · ${point.condition?conditionInfo.label:t('dataUnavailable')} · ${Number.isFinite(temp)?`${fmt(temp,1)} °C`:'—'}${Number.isFinite(prob)?` · ${t('homeRainProbabilityValue',{percent:Math.round(prob)})}`:''}${isWetPrecipitation(amount)?` · ${t('homeWatchRainAmount',{amount:fmt(amount,1)})}`:''}`;
    return `<div class="home-weather-axis-hour" style="--heat-color:${attr(temperatureHeatColor(temp))}" title="${attr(tooltip)}"><span class="home-weather-axis-condition">${point.condition?aggregateConditionMarkup(point,'tiny'):'<span class="home-weather-axis-empty" aria-hidden="true">—</span>'}</span><time>${esc(timeLabel(point.timestamp))}</time></div>`;
  }).join('');
  const rainEvents=groupRainTimelineEvents(points),eventStarts=new Set(rainEvents.map(event=>event.start)),eventEnds=new Set(rainEvents.map(event=>event.end));
  const rainHours=points.map((point,index)=>{
    const probability=Number.isFinite(point.precipitationPercent)?Math.max(0,Math.min(100,point.precipitationPercent)):null,amount=point.precipitationConditionalMm,wet=eventStarts.has(index)||eventEnds.has(index)||rainEvents.some(event=>index>event.start&&index<event.end),classes=['home-weather-rain-hour',wet?'is-wet':'is-low',eventStarts.has(index)?'is-event-start':'',eventEnds.has(index)?'is-event-end':''].filter(Boolean).join(' '),probabilityLabel=Number.isFinite(probability)?`${Math.round(probability)}%`:'—',amountLabel=isWetPrecipitation(amount)?`${fmt(amount,1)} mm`:'',tooltip=`${timeLabel(point.timestamp)} · ${t('timelineRain')} · ${Number.isFinite(probability)?t('homeRainProbabilityValue',{percent:Math.round(probability)}):t('dataUnavailable')}${amountLabel?` · ${t('homeWatchRainAmount',{amount:fmt(amount,1)})}`:''}`;
    return `<div class="${classes}" style="--rain-soft:${Number.isFinite(probability)?Math.round(probability*.15):0}%;--rain-wet-top:${Number.isFinite(probability)?Math.round(12+probability*.42):12}%;--rain-wet-bottom:${Number.isFinite(probability)?Math.round(5+probability*.18):5}%" title="${attr(tooltip)}" aria-label="${attr(tooltip)}"><span class="home-weather-rain-probability">${weatherIcons.renderMetric('precipitation',{size:'micro'})}<strong>${esc(probabilityLabel)}</strong></span>${amountLabel?`<small>${esc(amountLabel)}</small>`:''}</div>`;
  }).join('');
  return `<div class="home-weather-timeline-wrap"><div class="home-weather-timeline-scroll" role="region" tabindex="0" aria-label="${esc(t('homeMiniTimelineAria'))}"><div class="home-weather-timeline" style="--home-timeline-cols:${points.length};--home-timeline-step:${step}px"><svg class="home-temperature-plot" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" role="button" tabindex="0" data-action="open-graphic-view" data-graphic-city-id="${attr(chartKey)}" aria-label="${attr(`${t('temperature')} · ${t('graphicView')}`)}"><defs><linearGradient id="${attr(gradientId)}" x1="0" x2="1" y1="0" y2="0">${gradientStops}</linearGradient></defs>${columns}<line class="home-temperature-baseline" x1="0" y1="${plotBottom}" x2="${width}" y2="${plotBottom}"/>${area?`<path class="home-temperature-area" d="${area}" fill="url(#${attr(gradientId)})"/>`:''}${path?`<path class="home-temperature-line" d="${path}" stroke="url(#${attr(gradientId)})"/>`:''}${pointMarks}</svg><div class="home-weather-axis" aria-hidden="true">${hourAxis}</div><div class="home-weather-event-lane"><div class="home-weather-rain-track" style="--home-timeline-cols:${points.length}">${rainHours}</div></div></div></div></div>`;
}
...
function vigilanceDateTime(timestamp,city,{date=true,time=true}={}){
  ...
  try{return formatDateTime(d,{timeZone:city?.timezone||'Europe/Paris',...(date?{weekday:'short',day:'numeric',month:'short'}:{}),...(time?{hour:'2-digit',minute:'2-digit'}:{})});}catch{return d.toLocaleString();}
}
function vigilanceFullDate(timestamp,city){
  const d=new Date(timestamp);if(!Number.isFinite(d.getTime()))return '—';
  try{return formatDateTime(d,{timeZone:city?.timezone||'Europe/Paris',weekday:'long',day:'numeric',month:'long'});}catch{return vigilanceDateTime(timestamp,city,{date:true,time:false});}
}
...
function renderHomeVigilance(){
  ...
  return `<section class="home-vigilance vigilance-level-${meta.key}" aria-label="${attr(t('vigilanceOfficialTitle'))}"><div class="home-vigilance-head"><div class="home-vigilance-title"><span class="home-vigilance-alert-mark" aria-hidden="true">${uiIcon('alert',18)}</span><div><span class="home-section-kicker">${esc(t('vigilanceOfficialSource'))}</span><h2>${esc(t('vigilanceOfficialTitle'))}</h2></div></div><span class="vigilance-level-badge vigilance-level-${meta.key}">${esc(vigilanceLevelLabel(globalMax))}</span></div><div class="home-vigilance-grid">${items.map(item=>{const level=vigilanceLevelMeta(item.max),names=item.phenomena.map(x=>vigilancePhenomenonLabel(x.id)).join(' · '),phenomena=item.phenomena.map(x=>`<span class="home-vigilance-phenomenon"><span class="home-vigilance-phenomenon-icon" aria-hidden="true">${vigilancePhenomenonIcon(x.id)}</span><span>${esc(vigilancePhenomenonLabel(x.id))}</span></span>`).join('<span class="home-vigilance-separator" aria-hidden="true">·</span>');return `<button class="home-vigilance-item vigilance-level-${level.key}" data-city-open="${attr(item.city.id)}" title="${attr(names||vigilanceLevelLabel(item.max))}"><span class="home-vigilance-status" aria-hidden="true">${uiIcon('alert',14)}</span><span class="home-vigilance-copy"><strong>${esc(item.city.name)}</strong><small class="home-vigilance-phenomena">${phenomena||esc(vigilanceLevelLabel(item.max))}</small></span><b>${esc(vigilanceLevelLabel(item.max))}</b></button>`;}).join('')}</div></section>`;
}
...
function renderCityCard(city){
  ...
  const engineContext=forecastEngineContext(city.id),now=currentConditions(f,new Date(),engineContext),today=cityToday(f.city.timezone),day=cachedAggregateDay(f,today,engineContext),info=localizedConditionInfo(now.condition||day.condition),conf=day.confidence?.overallPercent,minT=day.tempMin,maxT=day.tempMax,precipProb=day.precipProbability,precipAmount=day.precipConditional,wind=Number.isFinite(now.wind)?now.wind:day.wind,points=homeTimelinePoints(f,engineContext,12),modelCount=Object.keys(f.seriesByModel).length,familyCount=Math.max(day.consensusFamilyCount||0,day.confidence?.tempMax?.familyCount||0,day.confidence?.windMax?.familyCount||0,day.confidence?.precipitation?.familyCount||0),health=forecastHealth(f),marineAvailable=marineOptionAvailable(city);
  return `<article class="home-city-card" role="link" tabindex="0" data-city-open="${attr(city.id)}" style="--weather-accent:${info.accent}"><div class="home-city-accent"></div><div class="home-city-head"><div><h2>${marineTitleMarkup(city)}</h2><p>${esc(placeLine(city))}</p></div><button class="icon-btn home-city-menu-button" data-city-menu="${attr(city.id)}" aria-label="${esc(t('options'))}"${marineAvailable?` title="${attr(t('marineTitle'))}"`:''}>⋮${marineAvailable?'<span class="home-city-marine-available-dot" aria-hidden="true"></span>':''}</button></div><div class="home-weather-primary"><div class="home-weather-icon">${aggregateConditionMarkup(now.condition?now:day,'normal',true)}</div><div class="home-weather-value"><strong>${Number.isFinite(now.temperature)?`${fmt(now.temperature,1)}°`:'—'}</strong><span>${esc(info.label)}</span></div><div class="home-weather-coherence ${Number.isFinite(conf)?confidenceClass(conf):'neutral'}" title="${attr(homeAgreementText(conf,familyCount))}"><span>${esc(t('homeCoherenceLabel'))}</span><strong>${Number.isFinite(conf)?`${Math.round(conf)}%`:'—'}</strong><small>${esc(homeCoherenceText(conf,familyCount))}</small></div></div><div class="home-weather-facts"><div class="home-weather-fact temperature"><span class="home-weather-fact-icon">${weatherIcons.renderMetric('temperature',{size:'micro'})}</span><div class="home-weather-fact-copy"><span>${esc(t('tempMinMax'))}</span><strong>${Number.isFinite(minT)&&Number.isFinite(maxT)?`${fmt(minT)}° / ${fmt(maxT)}°`:'—'}</strong></div></div><div class="home-weather-fact rain"><span class="home-weather-fact-icon">${weatherIcons.renderMetric('precipitation-probability',{size:'micro'})}</span><div class="home-weather-fact-copy"><span>${esc(t('precipitation'))}</span><strong>${Number.isFinite(precipProb)?`${Math.round(precipProb)} %`:'—'}</strong><small>${isWetPrecipitation(precipAmount)?`${fmt(precipAmount,1)} mm ${esc(t('homeIfRainShort'))}`:esc(t('homeDryOrLowRain'))}</small></div></div><div class="home-weather-fact cloud"><span class="home-weather-fact-icon">${weatherIcons.renderMetric('cloud',{size:'micro'})}</span><div class="home-weather-fact-copy"><span>${esc(t('cloudCoverage'))}</span><strong>${Number.isFinite(day.cloud)?`${Math.round(day.cloud)} %`:'—'}</strong><small>${Number.isFinite(day.cloudRange?.[0])&&Number.isFinite(day.cloudRange?.[1])?`${Math.round(day.cloudRange[0])}–${Math.round(day.cloudRange[1])} %`:esc(t('weightedMedianCentral'))}</small></div></div><div class="home-weather-fact wind"><span class="home-weather-fact-icon">${weatherIcons.renderMetric('wind',{size:'micro'})}</span><div class="home-weather-fact-copy"><span>${esc(t('wind'))}</span><strong>${Number.isFinite(wind)?`${fmt(wind)} km/h`:'—'}</strong><small>${Number.isFinite(day.gust)?`${esc(t('gusts'))} ${fmt(day.gust)} km/h`:esc(t('homeNoGustData'))}</small></div></div></div>${renderHomeWeatherTimeline(points,city.id)}<div class="home-city-footer"><span>${modelCountLabel(modelCount)}</span><span class="cache-inline ${health.class}">${esc(health.label)} · ${esc(formatExactAge(f.fetchedAt))}${loading?' · ⟳':''}</span></div>${err?`<div class="banner error home-city-error">${esc(err)}</div>`:''}</article>`;
}
...
function marineChartDateTick(timestamp){
  ...
  try{return formatDateTime(new Date(Date.UTC(year,month-1,day,12)),{weekday:'short',day:'numeric'});}catch{return String(timestamp||'').slice(5,10);}
}
...
function summaryDispersionCard({agg,key,metricFamily,label,hint,iconKind,iconClass,central,range,engineDetail=null,confidence,agreementLabel=null,unit,digits=1,miniFacts=[],historicalReliability=null}){
  ...
  const normalizedInterval=interval=>Number.isFinite(interval?.low)&&Number.isFinite(interval?.high)?{low:Math.min(interval.low,interval.high),high:Math.max(interval.low,interval.high)}:null;
  const rawInterval=Number.isFinite(rawMin)&&Number.isFinite(rawMax)?{low:Math.min(rawMin,rawMax),high:Math.max(rawMin,rawMax)}:null;
  const probableInterval=normalizedInterval(engineDetail?.allSourceInterval||engineDetail?.interval),retainedInterval=normalizedInterval(engineDetail?.interval);
  const axisValues=[central,rawInterval?.low,rawInterval?.high,probableInterval?.low,probableInterval?.high,retainedInterval?.low,retainedInterval?.high].filter(Number.isFinite),min=axisValues.length?Math.min(...axisValues):null,max=axisValues.length?Math.max(...axisValues):null;
  const span=Number.isFinite(min)&&Number.isFinite(max)?Math.max(.0001,max-min):null,pos=value=>Number.isFinite(value)&&Number.isFinite(span)?(span<=.0001?50:Math.max(0,Math.min(1,(value-min)/span))*100):50;
  const intervalStyle=interval=>interval?`--range-start:${pos(interval.low)}%;--range-end:${pos(interval.high)}%`:'';
  const formatted=value=>Number.isFinite(value)?`${fmt(value,digits)}${unit}`:'—',formattedInterval=interval=>interval?`${formatted(interval.low)} – ${formatted(interval.high)}`:'—',modelDots=rows.map((row,index)=>{const model=getModel(row.modelId),name=model?.shortName||model?.name||row.modelId;return `<i class="summary-model-dot" style="--dot:${pos(row[key])}%;--dot-row:${index%2}" title="${attr(`${name} · ${formatted(row[key])}`)}" aria-label="${attr(`${name} ${formatted(row[key])}`)}"></i>`;}).join('');
  const confidenceValue=Number.isFinite(confidence?.percent)?Math.round(confidence.percent):null,confidenceClassName=Number.isFinite(confidenceValue)?confidenceClass(confidenceValue):'muted',rawRangeLabel=formattedInterval(rawInterval),probableRangeLabel=formattedInterval(probableInterval),retainedRangeLabel=formattedInterval(retainedInterval),centerPosition=pos(central),centerAtMin=Number.isFinite(central)&&Number.isFinite(min)&&Math.abs(central-min)<1e-7,centerAtMax=Number.isFinite(central)&&Number.isFinite(max)&&Math.abs(central-max)<1e-7;
  const miniCards=(miniFacts||[]).filter(fact=>fact&&fact.label&&fact.value!=null).map(fact=>`<span class="summary-dispersion-meta-item summary-mini-card ${fact.tone==='probability'?'probability':'secondary'} ${fact.wide?'wide':''}"${fact.title?` title="${attr(fact.title)}"`:''}><span class="summary-mini-card-label">${esc(fact.label)}</span><strong>${esc(fact.value)}</strong></span>`).join('');
  ...
  const supplementalMeta=`${reliabilityMeta}${evidenceMeta}`;
  const metadata=miniCards||supplementalMeta?`<div class="summary-dispersion-meta">${miniCards?`<div class="summary-dispersion-facts">${miniCards}</div>`:''}${supplementalMeta?`<div class="summary-dispersion-support">${supplementalMeta}</div>`:''}</div>`:'';
  const agreement=confidenceValue!=null?Math.max(0,Math.min(100,confidenceValue)):0;
  const railLabel=[`${label} · ${t('modelRange')} ${rawRangeLabel}`,probableInterval?`${t('summaryProbableRange')} ${probableRangeLabel}`:'',retainedInterval?`${t('summaryEngineRange')} ${retainedRangeLabel}`:''].filter(Boolean).join(' · ');
  const rangeValues=probableInterval||retainedInterval?`<div class="summary-range-values">${probableInterval?`<span class="probable"><span>${esc(t('summaryProbableRange'))}</span><strong>${formattedInterval(probableInterval)}</strong></span>`:''}${retainedInterval?`<span class="retained"><span>${esc(t('summaryEngineRange'))}</span><strong>${formattedInterval(retainedInterval)}</strong></span>`:''}</div>`:'';
  return `<article class="summary-dispersion metric-${iconClass} ${confidenceClassName}"><div class="summary-dispersion-head"><span class="summary-metric-icon ${iconClass}">${summaryMetricIcon(iconKind)}</span><div class="summary-dispersion-title"><div class="tile-label">${esc(label)}</div><div class="tile-hint">${esc(hint)}</div></div><strong class="summary-dispersion-value">${formatted(central)}</strong></div><div class="summary-dispersion-visual"><div class="summary-dispersion-rail" aria-label="${attr(railLabel)}"><div class="summary-dispersion-track">${rawInterval?`<span class="summary-dispersion-envelope" style="${intervalStyle(rawInterval)}" title="${attr(`${t('modelRange')} ${rawRangeLabel}`)}"></span>`:''}${probableInterval?`<span class="summary-probable-interval" style="${intervalStyle(probableInterval)}" title="${attr(`${t('summaryProbableRange')} ${probableRangeLabel}`)}"></span>`:''}${retainedInterval?`<span class="summary-engine-interval" style="${intervalStyle(retainedInterval)}" title="${attr(`${t('summaryEngineRange')} ${retainedRangeLabel}`)}"></span>`:''}${modelDots}<i class="summary-dispersion-center" style="--center:${centerPosition}%" title="${attr(`${hint} · ${formatted(central)}`)}"></i></div></div><div class="summary-dispersion-scale" style="--center:${centerPosition}%"><span class="${centerAtMin?'is-central':''}">${formatted(min)}</span>${!centerAtMin&&!centerAtMax?`<strong>${formatted(central)}</strong>`:''}<span class="${centerAtMax?'is-central':''}">${formatted(max)}</span></div>${rangeValues}</div>${metadata}<div class="summary-agreement"><div class="summary-agreement-label"><span>${esc(agreementLabel||t('modelConvergence'))}</span><strong>${confidenceValue!=null?`${confidenceValue}%`:'—'}</strong></div><div class="summary-agreement-track" aria-hidden="true"><i style="--agreement:${agreement}%"></i></div></div></article>`;
}
...
function graphicNiceAxis(maxValue,{minimum=1,targetTicks=3}={}){const safeMax=Math.max(minimum,Number(maxValue)||0),rough=Math.max(Number.EPSILON,safeMax/Math.max(1,targetTicks)),magnitude=10**Math.floor(Math.log10(rough)),normalized=rough/magnitude,factor=normalized<=1?1:normalized<=2?2:normalized<=2.5?2.5:normalized<=5?5:10,step=factor*magnitude,max=Math.max(step,Math.ceil(safeMax/step)*step),ticks=[];for(let value=0;value<=max+step*.25;value+=step)ticks.push(Number(value.toFixed(6)));return {max,step,ticks};}
...
function graphicDateRangeLabel(startDate,endDate,locale){
  const start=new Date(`${startDate}T12:00:00Z`),end=new Date(`${endDate}T12:00:00Z`);if(!Number.isFinite(start.getTime())||!Number.isFinite(end.getTime()))return '';
  const day=dateTimeFormatter(locale,{day:'numeric',timeZone:'UTC'}),monthDay=dateTimeFormatter(locale,{day:'numeric',month:'short',timeZone:'UTC'}),sameMonth=start.getUTCFullYear()===end.getUTCFullYear()&&start.getUTCMonth()===end.getUTCMonth();
  return sameMonth?`${day.format(start)}–${monthDay.format(end)}`:`${monthDay.format(start)} – ${monthDay.format(end)}`;
}
function graphicAgreementColor(value){
  const score=Math.max(0,Math.min(100,Number(value)||0));
  // Muted coral → amber → teal reads as uncertainty without overpowering the weather data.
  const hue=score<=50?8+(score/50)*30:38+((score-50)/50)*112;
  const light=52-score*.035;
  return `hsl(${Math.round(hue)} 62% ${light.toFixed(1)}%)`;
}
function graphicAgreementGradientStops(points,key,{sampleEvery=6}={}){
  if(!points.length)return '';const indexes=[];for(let i=0;i<points.length;i+=Math.max(1,sampleEvery))indexes.push(i);if(indexes.at(-1)!==points.length-1)indexes.push(points.length-1);
  return indexes.map(index=>{const point=points[index],score=Number.isFinite(point?.[key])?point[key]:point?.consensusPercent,offset=points.length>1?index/(points.length-1)*100:0;return `<stop offset="${offset.toFixed(2)}%" stop-color="${attr(graphicAgreementColor(score))}"/>`;}).join('');
}
function graphicSolarWindow(forecast,date,timezone){
  for(const series of Object.values(forecast?.seriesByModel||{})){
    const dates=series?.daily?.dates||[],index=dates.indexOf(date);if(index<0)continue;
    const sunrise=zonedTimestampEpoch(series.daily?.sunrise?.[index],timezone),sunset=zonedTimestampEpoch(series.daily?.sunset?.[index],timezone);
    if(Number.isFinite(sunrise)&&Number.isFinite(sunset)&&sunset>sunrise)return {sunrise,sunset};
  }
  const sunrise=zonedTimestampEpoch(`${date}T07:00`,timezone),sunset=zonedTimestampEpoch(`${date}T19:00`,timezone);
  return {sunrise:Number.isFinite(sunrise)?sunrise:null,sunset:Number.isFinite(sunset)?sunset:null};
}
function graphicDaylightGradient(forecast,points,groups,timezone,{leftPad,step,totalWidth}){
  if(!points.length||!Number.isFinite(points[0]?.epochMs))return 'linear-gradient(90deg,var(--gv-night-overlay),var(--gv-night-overlay))';
  const day='var(--gv-day-overlay)',night='var(--gv-night-overlay)',firstEpoch=points[0].epochMs,lastEpoch=points.at(-1)?.epochMs;
  const solarByDate=new Map(groups.map(group=>[group.date,graphicSolarWindow(forecast,group.date,timezone)]));
  const firstSolar=solarByDate.get(points[0].date)||{},firstPhase=Number.isFinite(firstSolar.sunrise)&&Number.isFinite(firstSolar.sunset)&&firstEpoch>=firstSolar.sunrise&&firstEpoch<firstSolar.sunset?'day':'night';
  const epochToX=epoch=>leftPad+step/2+((epoch-firstEpoch)/3600000)*step,pct=x=>Math.max(0,Math.min(100,x/totalWidth*100));
  const transitions=[];
  for(const group of groups){
    const solar=solarByDate.get(group.date)||{};
    if(Number.isFinite(solar.sunrise))transitions.push({epoch:solar.sunrise,next:'day'});
    if(Number.isFinite(solar.sunset))transitions.push({epoch:solar.sunset,next:'night'});
  }
  transitions.sort((a,b)=>a.epoch-b.epoch);
  const visibleStart=firstEpoch-((leftPad+step/2)/step)*3600000,visibleEnd=Number.isFinite(lastEpoch)?firstEpoch+((totalWidth-leftPad-step/2)/step)*3600000:firstEpoch;
  const fadePx=Math.max(8,step/3);let current=firstPhase,stops=[`${current==='day'?day:night} 0%`];
  for(const transition of transitions){
    if(transition.epoch<=visibleStart){current=transition.next;stops=[`${current==='day'?day:night} 0%`];continue;}
    if(transition.epoch>=visibleEnd)break;
    const eventX=epochToX(transition.epoch),before=pct(eventX-fadePx/2),after=pct(eventX+fadePx/2),nextColor=transition.next==='day'?day:night,currentColor=current==='day'?day:night;
    stops.push(`${currentColor} ${before.toFixed(3)}%`,`${nextColor} ${after.toFixed(3)}%`);current=transition.next;
  }
  stops.push(`${current==='day'?day:night} 100%`);return `linear-gradient(90deg,${stops.join(',')})`;
}
function graphicForecastHeader(city,{period=''}={}){
  const {t}=i18n();return `<header class="graphic-header"><div class="graphic-header-city"><button class="graphic-icon-button" data-action="close-graphic-view" aria-label="${attr(t('back'))}">${uiIcon('back',24)}</button><div><h1>${esc(city.name)}</h1><p>${esc(placeLine(city))}</p></div></div><div class="graphic-header-title"><strong>${esc(t('graphicView'))}</strong><span>${esc(t('graphicHourlySevenDays'))}</span></div><div class="graphic-header-context">${period?`<strong>${esc(period)}</strong>`:''}<span><b aria-hidden="true">↔</b>${esc(t('graphicExploreHint'))}</span></div></header>`;
}
function graphicTemperatureTooltip(point){
...
function renderGraphicForecastView(cityId){
  ...
  const f=state.forecasts[cityId],loading=state.loading.has(cityId),baseHeader=graphicForecastHeader(city);
  if(!f)return `<main class="graphic-view">${baseHeader}<div class="graphic-loading">${loading?'<div class="loader"></div>':`<button class="btn primary" data-refresh-city="${attr(city.id)}">${esc(t('refresh'))}</button>`}<p>${esc(t(loading?'loading':'dataUnavailable'))}</p></div></main>`;
  const engineContext=forecastEngineContext(cityId),normals=ensureNormalsLoaded(cityId)?.normals||null,points=cachedTimelinePoints(f,'HOURLY',new Date(),{...normalizeForecastOptions(engineContext),hourlyHorizonHours:168,includeModelValues:true}).slice(0,168);if(!points.length)return `<main class="graphic-view">${baseHeader}<div class="graphic-loading"><p>${esc(t('dataUnavailable'))}</p></div></main>`;
  const header=graphicForecastHeader(city,{period:graphicDateRangeLabel(points[0].date,points.at(-1).date,locale)}),step=54,leftPad=156,rightPad=34,totalWidth=Math.round(leftPad+points.length*step+rightPad),tempPlotHeight=340,rainPlotHeight=160,windPlotHeight=160,tempTop=58,tempBottom=292,rainTop=48,rainBottom=136,windTop=48,windBottom=136;
  const x=index=>leftPad+index*step+step/2,groups=[];for(let i=0;i<points.length;i++){if(i===0||points[i].date!==points[i-1].date){let end=i;while(end+1<points.length&&points[end+1].date===points[i].date)end++;groups.push({date:points[i].date,start:i,end});i=end;}}
  const graphicTimezone=f.city?.timezone||f.timezone||city.timezone||'UTC',daylightGradient=graphicDaylightGradient(f,points,groups,graphicTimezone,{leftPad,step,totalWidth});
  const normalByPoint=normals?points.map(point=>normals[point.date?.slice?.(5,10)]||null):points.map(()=>null),normalValues=normalByPoint.flatMap(normal=>normal?[normal.tempMinNormal,normal.tempMaxNormal]:[]).filter(Number.isFinite),tempValues=points.flatMap(point=>[point.temperatureMinAcrossModels,point.temperatureMaxAcrossModels,point.temperatureC]).filter(Number.isFinite).concat(normalValues),rawTempMin=tempValues.length?Math.min(...tempValues):0,rawTempMax=tempValues.length?Math.max(...tempValues):30,tempPad=Math.max(2,(rawTempMax-rawTempMin)*.12),tempMin=Math.floor(rawTempMin-tempPad),tempMax=Math.ceil(rawTempMax+tempPad),tempSpan=Math.max(1,tempMax-tempMin),yTemp=value=>tempBottom-(value-tempMin)/tempSpan*(tempBottom-tempTop);
  const rainMaxRaw=Math.max(0,...points.map(point=>Number.isFinite(point.precipitationMm)?point.precipitationMm:Number.isFinite(point.precipitationConditionalMm)?point.precipitationConditionalMm:0)),rainAxis=graphicNiceAxis(rainMaxRaw,{minimum:1,targetTicks:3}),rainMax=rainAxis.max,yRain=value=>rainBottom-Math.max(0,Math.min(rainMax,value||0))/rainMax*(rainBottom-rainTop);
  const windMaxRaw=Math.max(10,...points.flatMap(point=>[point.windKmh,point.windGustKmh]).filter(Number.isFinite)),windAxis=graphicNiceAxis(windMaxRaw,{minimum:20,targetTicks:3}),windMax=windAxis.max,yWind=value=>windBottom-Math.max(0,Math.min(windMax,value||0))/windMax*(windBottom-windTop);
  const tempLine=svgPathFromPoints(points.map((point,index)=>Number.isFinite(point.temperatureC)?{x:x(index),y:yTemp(point.temperatureC)}:null)),era5MaxLine=svgPathFromPoints(normalByPoint.map((normal,index)=>Number.isFinite(normal?.tempMaxNormal)?{x:x(index),y:yTemp(normal.tempMaxNormal)}:null)),era5MinLine=svgPathFromPoints(normalByPoint.map((normal,index)=>Number.isFinite(normal?.tempMinNormal)?{x:x(index),y:yTemp(normal.tempMinNormal)}:null)),windLine=svgPathFromPoints(points.map((point,index)=>Number.isFinite(point.windKmh)?{x:x(index),y:yWind(point.windKmh)}:null)),gustLine=svgPathFromPoints(points.map((point,index)=>Number.isFinite(point.windGustKmh)?{x:x(index),y:yWind(point.windGustKmh)}:null));
  ...
  const tempTicks=Array.from({length:6},(_,i)=>tempMin+(tempMax-tempMin)*i/5),linePath=(coords,vertical=false)=>coords.length?coords.map(([a,b,c,d])=>vertical?`M ${a.toFixed(1)} ${b} V ${d}`:`M ${a} ${b.toFixed(1)} H ${c}`).join(' '):'',verticalGrid=(height)=>{const minor=[],major=[];points.forEach((point,index)=>(index%6===0?major:minor).push([x(index),0,x(index),height]));return `${minor.length?`<path class="graphic-grid vertical" d="${linePath(minor,true)}"/>`:''}${major.length?`<path class="graphic-grid vertical major" d="${linePath(major,true)}"/>`:''}`;},horizontalGrid=(ticks,y,kind='')=>ticks.length?`<path class="graphic-grid horizontal${kind?` ${kind}`:''}" d="${linePath(ticks.map(value=>[leftPad,y(value),totalWidth-rightPad,y(value)]))}"/>`:'',dayBreaks=(top,bottom)=>{const lines=[];groups.slice(1).forEach(group=>lines.push([x(group.start)-step/2,top,x(group.start)-step/2,bottom]));return lines.length?`<path class="graphic-day-break" d="${linePath(lines,true)}"/>`:'';};
  const tempGrid=horizontalGrid(tempTicks,yTemp)+verticalGrid(tempPlotHeight),rainGrid=horizontalGrid(rainAxis.ticks,yRain,'rain')+verticalGrid(rainPlotHeight),windGrid=horizontalGrid(windAxis.ticks,yWind,'wind')+verticalGrid(windPlotHeight);
  const heatStops=points.flatMap((point,index)=>{const color=temperatureHeatColor(point.temperatureC),start=(index/points.length*100).toFixed(3),end=((index+1)/points.length*100).toFixed(3);return [`${color} ${start}%`,`${color} ${end}%`];}).join(','),tint=`<div class="graphic-temperature-tint" style="--graphic-heatmap:${attr(`linear-gradient(90deg,${heatStops})`)}"></div>`;
  const tempAgreementStops=graphicAgreementGradientStops(points,'temperatureAgreementPercent'),windAgreementStops=graphicAgreementGradientStops(points,'windAgreementPercent');
  const tempTargets=points.map((point,index)=>{if(!Number.isFinite(point.temperatureC))return '';const px=x(index),py=yTemp(point.temperatureC),id=`graphic-temp-tip-${index}`,label=`${dateLabel(point.date,locale)} ${timeLabel(point.timestamp)} · ${t('temperature')} ${fmt(point.temperatureC,1)} °C`;graphicTooltipContentCache.set(id,{type:'temperature',point,html:null});return `<div class="graphic-condition" style="left:${px}px;top:${(Math.max(10,py-46)/tempPlotHeight*100).toFixed(3)}%" title="${attr(localizedConditionInfo(point.condition).label)}">${point.condition?weatherIcons.render(point.condition,{size:'small'}):''}</div><button class="graphic-point graphic-temp-point" data-graphic-tooltip-target="temperature" data-graphic-tooltip-key="${attr(id)}" data-graphic-index="${index}" style="left:${px}px;top:${(py/tempPlotHeight*100).toFixed(3)}%" aria-label="${attr(label)}" aria-describedby="${attr(id)}" aria-expanded="false"><i></i><span class="graphic-temp-value">${fmt(point.temperatureC)}°</span></button>`;}).join('');
  const rainTargets=points.map((point,index)=>{const amount=Number.isFinite(point.precipitationMm)?point.precipitationMm:Number.isFinite(point.precipitationConditionalMm)?point.precipitationConditionalMm:null;if(!Number.isFinite(amount)&&!Number.isFinite(point.precipitationPercent))return '';const px=x(index),barAmount=Math.max(0,amount||0),height=Math.max(barAmount>0?3:1,(barAmount/rainMax)*(rainBottom-rainTop)),top=yRain(barAmount),prob=Math.max(0,Math.min(100,Number(point.precipitationPercent)||0)),id=`graphic-rain-tip-${index}`,label=`${dateLabel(point.date,locale)} ${timeLabel(point.timestamp)} · ${t('precipitation')} ${Number.isFinite(amount)?fmt(amount,1)+' mm':'—'} · ${Math.round(prob)} %`;graphicTooltipContentCache.set(id,{type:'rain',point,html:null});return `<button class="graphic-rain-bar" data-graphic-tooltip-target="rain" data-graphic-tooltip-key="${attr(id)}" data-graphic-index="${index}" style="left:${px-step*.31}px;top:${(top/rainPlotHeight*100).toFixed(3)}%;width:${Math.max(12,step*.62)}px;height:${(height/rainPlotHeight*100).toFixed(3)}%;--prob:${prob}%;--agreement-color:${graphicAgreementColor(point.precipitationAgreementPercent)}" aria-label="${attr(label)}" aria-describedby="${attr(id)}" aria-expanded="false">${barAmount>=.2?`<span>${fmt(barAmount,barAmount<10?1:0)}</span>`:''}</button>`;}).join('');
  const windTargets=points.map((point,index)=>{if(!Number.isFinite(point.windKmh))return '';const px=x(index),py=yWind(point.windKmh),id=`graphic-wind-tip-${index}`,label=`${dateLabel(point.date,locale)} ${timeLabel(point.timestamp)} · ${t('wind')} ${fmt(point.windKmh)} km/h · ${t('gusts')} ${Number.isFinite(point.windGustKmh)?fmt(point.windGustKmh)+' km/h':'—'}`;graphicTooltipContentCache.set(id,{type:'wind',point,html:null});return `<button class="graphic-point graphic-wind-point" data-graphic-tooltip-target="wind" data-graphic-tooltip-key="${attr(id)}" data-graphic-index="${index}" style="left:${px}px;top:${(py/windPlotHeight*100).toFixed(3)}%" aria-label="${attr(label)}" aria-describedby="${attr(id)}" aria-expanded="false"><i></i></button>`;}).join('');
  const axisMarkup=(kind,label,unit,ticks,y,formatter,start,end,plotHeight,icon)=>`<div class="graphic-y-axis ${kind}" style="--axis-start:${(start/plotHeight*100).toFixed(3)}%;--axis-end:${(end/plotHeight*100).toFixed(3)}%"><div class="graphic-axis-heading"><span class="graphic-axis-icon">${icon}</span><span><strong>${esc(label)}</strong><small>${esc(unit)}</small></span></div><div class="graphic-axis-rail" aria-hidden="true"></div>${ticks.map(value=>`<span class="graphic-axis-tick" style="top:${(y(value)/plotHeight*100).toFixed(3)}%"><em>${esc(formatter(value))}</em><i aria-hidden="true"></i></span>`).join('')}</div>`;
  const tempAxisMarkup=axisMarkup('temp',t('temperature'),'°C',tempTicks,yTemp,value=>`${fmt(value)}°`,tempTop,tempBottom,tempPlotHeight,weatherIcons.renderMetric('temperature',{size:'tiny'})),rainAxisMarkup=axisMarkup('rain',t('widget_metric_precipitation'),'mm/h',rainAxis.ticks,yRain,value=>fmt(value,value<1?1:0),rainTop,rainBottom,rainPlotHeight,weatherIcons.renderMetric('precipitation',{size:'tiny'})),windAxisMarkup=axisMarkup('wind',t('wind'),'km/h',windAxis.ticks,yWind,value=>fmt(value,0),windTop,windBottom,windPlotHeight,weatherIcons.renderMetric('wind',{size:'tiny'}));
  const timeAxis=points.map((point,index)=>{const amount=Number.isFinite(point.precipitationMm)?point.precipitationMm:Number.isFinite(point.precipitationConditionalMm)?point.precipitationConditionalMm:null,prob=Number.isFinite(point.precipitationPercent)?Math.round(point.precipitationPercent):null,temp=Number.isFinite(point.temperatureC)?`${fmt(point.temperatureC)}°`:'—',rain=Number.isFinite(amount)?`${fmt(amount,amount<10?1:0)} mm${prob!=null?` · ${prob}%`:''}`:(prob!=null?`${prob}%`:'—'),wind=Number.isFinite(point.windKmh)?`${fmt(point.windKmh)} km/h`:'—',gust=Number.isFinite(point.windGustKmh)?`${fmt(point.windGustKmh)} km/h`:'—';return `<div class="graphic-hour ${point.date!==points[index-1]?.date?'day-start':''}" data-graphic-hour-index="${index}" data-graphic-ruler-hour="${attr(timeLabel(point.timestamp))}" data-graphic-ruler-date="${attr(dateLabel(point.date,locale))}" data-graphic-ruler-temp="${attr(temp)}" data-graphic-ruler-rain="${attr(rain)}" data-graphic-ruler-wind="${attr(wind)}" data-graphic-ruler-gust="${attr(gust)}" style="left:${(x(index)-step/2).toFixed(1)}px;width:${step}px"><strong>${esc(timeLabel(point.timestamp))}</strong></div>`;}).join('');
  const dateAxis=groups.map((group,index)=>{const startX=x(group.start)-step/2,width=(group.end-group.start+1)*step,offset=Math.max(0,startX-leftPad);return `<button class="graphic-date-block ${index===0?'active':''}" style="left:${startX.toFixed(1)}px;width:${width}px" data-graphic-day="${offset.toFixed(0)}"><strong>${esc(dateLabel(group.date,locale))}</strong><span>${group.end-group.start+1} h</span></button>`;}).join('');
  const dayDividers=groups.slice(1).map(group=>`<span class="graphic-day-divider" style="left:${(x(group.start)-step/2).toFixed(1)}px"></span>`).join('');
  const era5Legend=era5MaxLine&&era5MinLine?`<span class="era5-max"><i></i>${esc(t('chart_normals_legend_temp_max'))}</span><span class="era5-min"><i></i>${esc(t('chart_normals_legend_temp_min'))}</span>`:`<span class="era5-pending"><i></i>${esc(t('webNormals'))} · ${esc(state.online?t('webLoading'):t('webUnavailableOffline'))}</span>`,agreementToggle=`<label class="graphic-agreement-toggle" title="${attr(t('graphicAgreementOverlayHint'))}"><input type="checkbox" data-graphic-agreement-toggle${state.graphicAgreementVisible?' checked':''}><span class="graphic-agreement-toggle-box" aria-hidden="true"></span><span class="graphic-agreement-toggle-copy"><b>${esc(t('graphicAgreementOverlay'))}</b><small>${esc(t('graphicAgreementDivergence'))} ↔ ${esc(t('graphicAgreementStrong'))}</small></span><i class="graphic-agreement-gradient" aria-hidden="true"></i></label>`,legend=`<div class="graphic-legend"><span class="temp"><i></i>${esc(t('temperature'))}</span><span class="spread"><i></i>${esc(t('graphicDispersion'))}</span>${era5Legend}<span class="rain"><i></i>${esc(t('precipitation'))}</span><span class="wind"><i></i>${esc(t('wind'))}</span><span class="gust"><i></i>${esc(t('gusts'))}</span>${agreementToggle}<span class="graphic-rain-scale"><b>${esc(t('graphicRainProbability'))}</b><i></i><small>0 %</small><small>50 %</small><small>100 %</small></span></div>`;
  const startMs=points[0].epochMs,endMs=(points.at(-1).epochMs||startMs)+3600000,vigilance=graphicVigilanceLane(city,vigilanceByCity.get(city.id),startMs,endMs,leftPad,step,totalWidth),agreementClass=state.graphicAgreementVisible?' show-agreement':'';
  return `<main class="graphic-view${agreementClass}">${header}<div class="graphic-scroll" role="region" tabindex="0" aria-label="${attr(t('graphicExploreAria'))}"><div class="graphic-track" style="width:${totalWidth}px;--graphic-daylight:${attr(daylightGradient)}" data-graphic-ruler-track data-graphic-left-pad="${leftPad}" data-graphic-step="${step}" data-graphic-count="${points.length}"><div class="graphic-chart graphic-plots" style="width:${totalWidth}px"><section class="graphic-plot graphic-plot-temperature">${tint}<svg class="graphic-chart-svg" width="${totalWidth}" height="${tempPlotHeight}" viewBox="0 0 ${totalWidth} ${tempPlotHeight}" preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id="graphic-temp-agreement-gradient" x1="0" x2="1" y1="0" y2="0">${tempAgreementStops}</linearGradient></defs>${tempGrid}${dayBreaks(0,tempPlotHeight)}${bandPath?`<path class="graphic-temp-band" d="${bandPath}"/>`:''}${era5MaxLine?`<path class="graphic-era5-line max" d="${era5MaxLine}"/>`:''}${era5MinLine?`<path class="graphic-era5-line min" d="${era5MinLine}"/>`:''}<path class="graphic-agreement-path temperature halo" d="${tempLine}" stroke="url(#graphic-temp-agreement-gradient)"/><path class="graphic-agreement-path temperature core" d="${tempLine}" stroke="url(#graphic-temp-agreement-gradient)"/><path class="graphic-temp-line" d="${tempLine}"/></svg>${tempTargets}</section><section class="graphic-plot graphic-plot-rain"><svg class="graphic-chart-svg" width="${totalWidth}" height="${rainPlotHeight}" viewBox="0 0 ${totalWidth} ${rainPlotHeight}" preserveAspectRatio="none" aria-hidden="true">${rainGrid}${dayBreaks(0,rainPlotHeight)}<line class="graphic-rain-baseline" x1="${leftPad}" x2="${totalWidth-rightPad}" y1="${rainBottom}" y2="${rainBottom}"/></svg>${rainTargets}</section><section class="graphic-plot graphic-plot-wind"><svg class="graphic-chart-svg" width="${totalWidth}" height="${windPlotHeight}" viewBox="0 0 ${totalWidth} ${windPlotHeight}" preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id="graphic-wind-agreement-gradient" x1="0" x2="1" y1="0" y2="0">${windAgreementStops}</linearGradient></defs>${windGrid}${dayBreaks(0,windPlotHeight)}<line class="graphic-wind-baseline" x1="${leftPad}" x2="${totalWidth-rightPad}" y1="${windBottom}" y2="${windBottom}"/><path class="graphic-agreement-path wind halo" d="${windLine}" stroke="url(#graphic-wind-agreement-gradient)"/><path class="graphic-agreement-path wind core" d="${windLine}" stroke="url(#graphic-wind-agreement-gradient)"/><path class="graphic-gust-line" d="${gustLine}"/><path class="graphic-wind-line" d="${windLine}"/></svg>${windTargets}</section><div class="graphic-day-divider-layer" aria-hidden="true">${dayDividers}</div><div class="graphic-ruler" data-graphic-ruler hidden aria-hidden="true"><span class="graphic-ruler-badge"><strong data-graphic-ruler-label></strong><span class="graphic-ruler-summary"><span class="temp"><i></i><b data-graphic-ruler-temp>—</b></span><span class="rain"><i></i><b data-graphic-ruler-rain>—</b></span><span class="wind"><i></i><b data-graphic-ruler-wind>—</b><em data-graphic-ruler-gust>—</em></span></span></span></div></div><div class="graphic-sticky-axes" style="width:${leftPad}px"><div class="graphic-sticky-axis-slot">${tempAxisMarkup}</div><div class="graphic-sticky-axis-slot">${rainAxisMarkup}</div><div class="graphic-sticky-axis-slot">${windAxisMarkup}</div></div><div class="graphic-legend-row">${legend}</div><div class="graphic-time-axis" style="width:${totalWidth}px"><div class="graphic-hour-row">${timeAxis}</div><div class="graphic-date-row">${dateAxis}</div></div>${vigilance}</div></div><div class="graphic-tooltip-layer" data-graphic-tooltip-layer hidden><div class="graphic-floating-tooltip"></div></div></main>`;
}
...
function renderTodaySummary(f,agg,now,cityId,profile=localConsensusWeights(cityId)){
  ...
  const expectedRain=Number.isFinite(agg.precipExpected)?agg.precipExpected:agg.precip,rainMiniFacts=[{label:t('insightsMetricProbability'),value:Number.isFinite(agg.precipProbability)?`${Math.round(agg.precipProbability)}%`:'—',tone:'probability'},{label:t('summaryRainExpectedLabel'),value:Number.isFinite(expectedRain)?`${fmt(expectedRain,1)} mm`:'—',title:t('rainExpectedAmountTitle')},{label:t('summaryRainIfWetLabel'),value:Number.isFinite(agg.precipConditional)?`${fmt(agg.precipConditional,1)} mm`:'—'}],windMiniFacts=Number.isFinite(agg.gust)?[{label:t('gusts'),value:`${fmt(agg.gust)} km/h`,wide:true}]:[];
  const cards=[
    summaryDispersionCard({agg,key:'tempMin',metricFamily:'temperature',label:t('tempMinimum'),hint:engineHintFor('tempMin'),iconKind:'temperature',iconClass:'temp-min',central:agg.tempMin,range:agg.tempMinRange,engineDetail:agg.engineDetails?.tempMin,confidence:c.tempMin,unit:' °C',digits:1}),
    summaryDispersionCard({agg,key:'tempMax',metricFamily:'temperature',label:t('tempMaximum'),hint:engineHintFor('tempMax'),iconKind:'temperature',iconClass:'temp-max',central:agg.tempMax,range:agg.tempMaxRange,engineDetail:agg.engineDetails?.tempMax,confidence:c.tempMax,unit:' °C',digits:1}),
    summaryDispersionCard({agg,key:'precipExpectedSource',metricFamily:'precipitation',label:t('precipitation'),hint:engineHintFor('precipitation'),iconKind:'precipitation',iconClass:'rain',central:agg.precip,range:agg.precipRange,engineDetail:agg.engineDetails?.precipitation,confidence:precipConfidence,agreementLabel:t('rainAgreement'),unit:' mm',digits:1,miniFacts:rainMiniFacts,historicalReliability:agg.precipitationDiagnostics?.historicalReliabilityPercent}),
    summaryDispersionCard({agg,key:'wind',metricFamily:'wind',label:t('wind'),hint:engineHintFor('wind'),iconKind:'wind',iconClass:'wind',central:agg.wind,range:agg.windRange,engineDetail:agg.engineDetails?.wind,confidence:c.windMax,unit:' km/h',digits:0,miniFacts:windMiniFacts}),
    summaryDispersionCard({agg,key:'cloud',metricFamily:'condition',label:t('cloudCoverage'),hint:engineHintFor('cloud'),iconKind:'cloud',iconClass:'cloud',central:agg.cloud,range:agg.cloudRange,engineDetail:agg.engineDetails?.cloud,confidence:agg.cloudConfidence,unit:' %',digits:0})
  ].join('');
  const railLegend=`<div class="summary-rail-legend" aria-label="${attr(t('summaryRailLegend'))}"><div class="summary-legend-item spread"><i aria-hidden="true"><b></b><b></b><b></b></i><span><strong>${esc(t('summarySpreadLegend'))}</strong><small>${esc(t('summarySpreadLegendHint'))}</small></span></div><div class="summary-legend-item interval"><i aria-hidden="true"><b></b></i><span><strong>${esc(t('summaryIntervalLegend'))}</strong><small>${esc(t('summaryIntervalLegendHint'))}</small></span></div></div>`;
  return `<section class="section today-summary" id="today-summary"><div class="summary-card"><div class="summary-accent" style="--summary-accent:${info.accent||'var(--primary)'}"></div><div class="summary-header"><div class="summary-now"><div class="summary-weather-icon">${aggregateConditionMarkup(now.condition?now:agg,'normal',true)}</div><div class="summary-now-copy"><div class="summary-kicker">${esc(dateLabel(agg.date,i18n().locale,'long'))}</div><div class="summary-now-line"><span class="summary-current-temp">${Number.isFinite(now.temperature)?`${fmt(now.temperature,1)}°`:'—'}</span><span class="summary-condition">${esc(info.label)}</span></div><div class="summary-context-chips"><span class="summary-context-item">${esc(t('analysedModels',{models:modelCountLabel(modelCount)}))}</span><span class="summary-context-item">${esc(t('independentFamilies',{count:familyCount}))}</span><span class="summary-context-item summary-context-method">${esc(method)}</span>${agg.sunrise||agg.sunset?`<span class="summary-solar-pair">${agg.sunrise?`<span class="summary-solar-chip">${weatherIcons.renderMetric('sunrise',{size:'micro'})} ${esc(t('sunrise'))} ${timeLabel(agg.sunrise)}</span>`:''}${agg.sunset?`<span class="summary-solar-chip">${weatherIcons.renderMetric('sunset',{size:'micro'})} ${esc(t('sunset'))} ${timeLabel(agg.sunset)}</span>`:''}</span>`:''}</div></div></div></div><div class="today-grid summary-dispersion-grid">${cards}</div>${railLegend}</div></section>`;
}
...
function detailChronoPointLabel(point,mode,index){
  const {t}=i18n();
  if(mode==='HOURLY')return index===0?t('now'):timeLabel(point.timestamp);
  return dateLabel(point.date,i18n().locale);
}
function detailChronoTemperature(point,mode){
  if(mode==='HOURLY')return point.temperatureC;
  if(Number.isFinite(point.tempMinC)&&Number.isFinite(point.tempMaxC))return (point.tempMinC+point.tempMaxC)/2;
  return Number.isFinite(point.tempMaxC)?point.tempMaxC:point.tempMinC;
}
function renderDetailChronoTimeline(points,mode,cityId=null){
  const {t}=i18n();if(!points.length)return '';
  const step=mode==='HOURLY'?82:126,width=points.length*step,height=118,plotTop=25,plotBottom=94,temps=points.map(point=>detailChronoTemperature(point,mode)).filter(Number.isFinite),rawMin=temps.length?Math.min(...temps):0,rawMax=temps.length?Math.max(...temps):1,center=(rawMin+rawMax)/2,span=Math.max(5,rawMax-rawMin+3),min=center-span/2,max=center+span/2,y=value=>plotTop+(max-value)*(plotBottom-plotTop)/(max-min),x=index=>index*step+step/2,gradientId=`detail-chrono-temp-${mode.toLowerCase()}-${points.length}`;
  const coords=points.map((point,index)=>{const value=detailChronoTemperature(point,mode);return Number.isFinite(value)?{x:x(index),y:y(value),value}:null;}),path=svgPathFromPoints(coords),allFinite=coords.every(Boolean),area=allFinite&&path?`${path} L ${coords.at(-1).x.toFixed(2)} ${plotBottom} L ${coords[0].x.toFixed(2)} ${plotBottom} Z`:'';
  const stops=points.map((point,index)=>`<stop offset="${points.length===1?0:(index/(points.length-1))*100}%" stop-color="${attr(temperatureHeatColor(detailChronoTemperature(point,mode)))}"/>`).join('');
  const heat=points.map((point,index)=>`<rect class="detail-chrono-heat-column" x="${index*step}" y="0" width="${step}" height="${height}" fill="${attr(temperatureHeatColor(detailChronoTemperature(point,mode)))}"/>`).join('');
  const tempMarks=points.map((point,index)=>{const value=detailChronoTemperature(point,mode);if(!Number.isFinite(value))return '';const low=mode==='DAILY'&&Number.isFinite(point.tempMinC)?point.tempMinC:null,high=mode==='DAILY'&&Number.isFinite(point.tempMaxC)?point.tempMaxC:null,label=mode==='DAILY'&&Number.isFinite(low)&&Number.isFinite(high)?`${fmt(high)}° / ${fmt(low)}°`:`${fmt(value)}°`;return `<g class="detail-chrono-temp-point"><circle cx="${x(index)}" cy="${y(value).toFixed(2)}" r="4" fill="${attr(temperatureHeatColor(value))}"/><text x="${x(index)}" y="${Math.max(14,y(value)-9).toFixed(2)}" text-anchor="middle">${esc(label)}</text></g>`;}).join('');
  const dateGroups=[];for(const point of points){const key=point.date||String(point.timestamp||'').slice(0,10),last=dateGroups.at(-1);if(last?.date===key)last.span++;else dateGroups.push({date:key,span:1});}
  const dates=dateGroups.map(group=>{const label=dateLabel(group.date,i18n().locale,mode==='HOURLY'?'long':'short');return `<div class="detail-chrono-date-cell" style="grid-column:span ${group.span}" title="${attr(label)}"><strong>${esc(label)}</strong></div>`;}).join('');
  const axis=points.map((point,index)=>{const ci=localizedConditionInfo(point.condition),pointLabel=detailChronoPointLabel(point,mode,index),conditionLabel=point.condition?ci.label:'',visibleLabel=mode==='HOURLY'?pointLabel:conditionLabel,tooltip=[dateLabel(point.date,i18n().locale,'long'),mode==='HOURLY'?pointLabel:'',conditionLabel].filter(Boolean).join(' · ');return `<div class="detail-chrono-axis-hour" title="${attr(tooltip)}"><span class="detail-chrono-condition-icon">${point.condition?aggregateConditionMarkup(point,'small'):'—'}</span>${visibleLabel?`<strong>${esc(visibleLabel)}</strong>`:''}</div>`;}).join('');
  const rainEvents=groupRainTimelineEvents(points),starts=new Set(rainEvents.map(event=>event.start)),ends=new Set(rainEvents.map(event=>event.end));
  const rain=points.map((point,index)=>{const probability=Number.isFinite(point.precipitationPercent)?Math.max(0,Math.min(100,point.precipitationPercent)):null,amount=point.precipitationConditionalMm,expected=point.precipitationExpectedMm,wet=starts.has(index)||ends.has(index)||rainEvents.some(event=>index>event.start&&index<event.end),classes=['detail-chrono-rain-cell',wet?'is-wet':'',starts.has(index)?'is-start':'',ends.has(index)?'is-end':''].filter(Boolean).join(' '),tooltip=[detailChronoPointLabel(point,mode,index),t('timelineRain'),Number.isFinite(probability)?`${Math.round(probability)}%`:'—',isWetPrecipitation(amount)?`${fmt(amount,1)} mm ${t('timelineRainIfWetShort').toLowerCase()}`:'',Number.isFinite(expected)?`${t('summaryRainExpectedLabel')} ${fmt(expected,1)} mm`:''].filter(Boolean).join(' · ');return `<div class="${classes}" style="--rain-strength:${Number.isFinite(probability)?Math.round(probability*.34):0}%" title="${attr(tooltip)}"><strong>${weatherIcons.renderMetric('precipitation',{size:'micro'})}${Number.isFinite(probability)?`${Math.round(probability)}%`:'—'}</strong>${isWetPrecipitation(amount)?`<small>${fmt(amount,1)} mm</small>`:'<small>—</small>'}</div>`;}).join('');
  const cloud=points.map(point=>{const value=Number.isFinite(point.cloudCoverPercent)?Math.max(0,Math.min(100,point.cloudCoverPercent)):null;return `<div class="detail-chrono-cloud-cell" style="--cloud:${value??0}%"><i aria-hidden="true"></i><strong>${value==null?'—':`${Math.round(value)}%`}</strong></div>`;}).join('');
  const wind=points.map(point=>{const speed=point.windKmh,gust=point.windGustKmh,maxValue=Math.max(Number(speed)||0,Number(gust)||0),strength=Math.max(0,Math.min(100,maxValue/90*100));return `<div class="detail-chrono-wind-cell" style="--wind-strength:${strength}%"><span>${weatherIcons.renderMetric('wind',{size:'micro'})}</span><strong>${Number.isFinite(speed)?`${fmt(speed)} km/h`:'—'}</strong><small>${Number.isFinite(gust)?`${esc(t('gustAbbr'))} ${fmt(gust)}`:'—'}</small></div>`;}).join('');
  const agreement=points.map(point=>{const value=Number.isFinite(point.consensusPercent)?Math.max(0,Math.min(100,Math.round(point.consensusPercent))):null,tone=value==null?'muted':confidenceClass(value),reasons=[...new Set(point.divergenceReasons||[])],reasonLabels=reasons.map(divergenceShort),reasonTitle=reasons.map(divergenceLabel).join(' · '),warning=reasons.length?`<div class="detail-chrono-disagreement" title="${attr(reasonTitle)}" aria-label="${attr(`${t('disagreementVariableLegend')} : ${reasonLabels.join(', ')}`)}"><span class="detail-chrono-disagreement-icons">${reasons.map(reason=>`<span class="detail-chrono-disagreement-icon" title="${attr(divergenceLabel(reason))}" aria-label="${attr(divergenceLabel(reason))}">${divergenceIcon(reason)}</span>`).join('')}</span></div>`:`<span class="detail-chrono-agreement-ok" aria-label="${attr(t('coherent'))}">✓</span>`;return `<div class="detail-chrono-agreement-cell ${tone}" style="--agreement:${value??0}%"><div class="detail-chrono-agreement-status">${warning}</div><div class="detail-chrono-agreement-bar" aria-hidden="true"><span></span></div><strong class="detail-chrono-agreement-percent">${value==null?'—':`${value}%`}</strong></div>`;}).join('');
  const labels=`<div class="detail-chrono-labels" aria-hidden="true"><div class="detail-chrono-row-label date">${uiIcon('calendar',17)}<span><strong>${esc(t('dateColumn'))}</strong></span></div><div class="detail-chrono-row-label temp">${weatherIcons.renderMetric('temperature',{size:'small'})}<span><strong>${esc(t('temperature'))}</strong><small>°C</small></span></div><div class="detail-chrono-row-label conditions"><span class="detail-chrono-label-dot">●</span><span><strong>${esc(t('conditions'))}</strong><small>${esc(mode==='HOURLY'?'24 h':t('dayMode7'))}</small></span></div><div class="detail-chrono-row-label rain">${weatherIcons.renderMetric('precipitation',{size:'small'})}<span><strong>${esc(t('timelineRain'))}</strong><small>% · mm</small></span></div><div class="detail-chrono-row-label cloud">${weatherIcons.renderMetric('cloud',{size:'small'})}<span><strong>${esc(t('cloudCoverage'))}</strong><small>%</small></span></div><div class="detail-chrono-row-label wind">${weatherIcons.renderMetric('wind',{size:'small'})}<span><strong>${esc(t('wind'))}</strong><small>km/h</small></span></div><div class="detail-chrono-row-label agreement"><span class="detail-chrono-label-dot">◎</span><span><strong>${esc(t('modelConvergence'))}</strong><small>%</small></span></div></div>`;
  return `<div class="detail-chrono-view" data-chrono-mode="${attr(mode)}" style="--detail-chrono-cols:${points.length};--detail-chrono-step:${step}px">${labels}<div class="detail-chrono-scroll" role="region" tabindex="0" aria-label="${attr(t('timelineChronoView'))}"><div class="detail-chrono-canvas"><div class="detail-chrono-date">${dates}</div><svg class="detail-chrono-temp-plot" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" role="button" tabindex="0" data-action="open-graphic-view"${cityId?` data-graphic-city-id="${attr(cityId)}"`:''} aria-label="${attr(`${t('temperature')} · ${t('graphicView')}`)}"><defs><linearGradient id="${attr(gradientId)}" x1="0" x2="1" y1="0" y2="0">${stops}</linearGradient></defs>${heat}<line class="detail-chrono-temp-baseline" x1="0" y1="${plotBottom}" x2="${width}" y2="${plotBottom}"/>${area?`<path class="detail-chrono-temp-area" d="${area}" fill="url(#${attr(gradientId)})"/>`:''}${path?`<path class="detail-chrono-temp-line" d="${path}" stroke="url(#${attr(gradientId)})"/>`:''}${tempMarks}</svg><div class="detail-chrono-axis">${axis}</div><div class="detail-chrono-lane detail-chrono-rain">${rain}</div><div class="detail-chrono-lane detail-chrono-cloud">${cloud}</div><div class="detail-chrono-lane detail-chrono-wind">${wind}</div><div class="detail-chrono-lane detail-chrono-agreement">${agreement}</div></div></div></div>`;
}

function renderTimeline(f,engineContext=null){
  const opts=normalizeForecastOptions(engineContext),{t}=i18n(),hourlyAll=cachedTimelinePoints(f,'HOURLY',new Date(),opts),dailyAll=cachedTimelinePoints(f,'DAILY',new Date(),opts);let mode=state.settings.timelineMode||'HOURLY';if(mode==='HOURLY'&&hourlyAll.length<2)mode='DAILY';if(mode==='DAILY'&&!dailyAll.length&&hourlyAll.length)mode='HOURLY';const analysis=mode==='HOURLY'?hourlyAll:dailyAll,points=selectRegularTimelinePoints(analysis,mode==='HOURLY'?24:7,1);if(!points.length)return '';
  const layout=['COLUMNS','CHRONO'].includes(state.settings.timelineLayout)?state.settings.timelineLayout:'COLUMNS',columns=`<div class="timeline-scroll" style="--timeline-cols:${points.length}"><div class="timeline-ruler" aria-hidden="true">${points.map((p,i)=>timelineEventMarker(p,points[i-1])).join('')}</div><div class="timeline-full" role="list" aria-label="${esc(t('forecastTimeline'))}">${points.map((p,i)=>renderTimelinePoint(p,mode,i===0,i===points.length-1)).join('')}</div></div><div class="timeline-legend"><span><i class="legend-swatch temp-gradient"></i> ${esc(t('thermalBand'))}</span><span>${weatherIcons.renderMetric('precipitation-probability',{size:'tiny'})} ${esc(t('precipSignalLegend'))}</span><span>${weatherIcons.renderMetric('precipitation-amount',{size:'tiny'})} ${esc(t('summaryRainExpectedLabel'))}</span><span>${weatherIcons.renderMetric('cloud',{size:'tiny'})} ${esc(t('cloudMedianLegend'))}</span><span>${weatherIcons.renderMetric('wind',{size:'tiny'})} ${esc(t('windMedianLegend'))}</span><span>${weatherIcons.renderMetric('gust',{size:'tiny'})} ${esc(t('gusts'))}</span><span class="timeline-rail-legend" aria-label="${attr(t('summaryRailLegend'))}" title="${attr(`${t('summarySpreadLegendHint')} · ${t('summaryIntervalLegendHint')}`)}"><i aria-hidden="true"><b class="raw"></b><b class="probable"></b><b class="retained"></b><b class="center"></b></i>${esc(t('summarySpreadLegend'))} · ${esc(t('summaryIntervalLegend'))}</span><span>⚠ ${esc(t('disagreementVariableLegend'))}</span></div>`;
  const controls=`<div class="timeline-controls"><div class="segmented timeline-mode" aria-label="${esc(t('timelineModeAria'))}"><button class="seg-btn ${mode==='HOURLY'?'active':''}" data-timeline-mode="HOURLY" ${hourlyAll.length<2?'disabled':''}>24 h</button><button class="seg-btn ${mode==='DAILY'?'active':''}" data-timeline-mode="DAILY" ${!dailyAll.length?'disabled':''}>${esc(t('dayMode7'))}</button></div><div class="segmented timeline-layout" aria-label="${attr(t('timelineLayoutAria'))}"><button class="seg-btn ${layout==='COLUMNS'?'active':''}" data-timeline-layout="COLUMNS">${esc(t('timelineColumnsView'))}</button><button class="seg-btn ${layout==='CHRONO'?'active':''}" data-timeline-layout="CHRONO">${esc(t('timelineChronoView'))}</button></div></div>`;
  return `<section class="section timeline-section" id="timeline"><div class="section-card timeline-card timeline-layout-${layout.toLowerCase()}"><div class="section-head"><div><h2>${esc(t('forecastTimeline'))}</h2><p>${esc(t(mode==='HOURLY'?'next24Regular':'nextDaysConsensus'))}</p></div>${controls}</div>${layout==='CHRONO'?renderDetailChronoTimeline(points,mode,f.city.id):columns}</div></section>`;
}

function renderConfidenceSection(f,cityId,engineContext=null){
  const {t}=i18n(),allowed=['TEMPERATURE','PRECIPITATION_PROBABILITY','PRECIPITATION','CLOUD','WIND','GUST'],metric=allowed.includes(state.settings.confidenceMetric)?state.settings.confidenceMetric:'TEMPERATURE',horizon=[24,72,168].includes(Number(state.settings.chartHorizon))?Number(state.settings.chartHorizon):168,bands=cachedBand(f,metric,horizon,engineContext),normals=state.normals[cityId]?.normals||null;
  const metrics=[['TEMPERATURE',t('temperature'),'temperature'],['PRECIPITATION_PROBABILITY',t('timelineRainProbabilityShort'),'precipitation-probability'],['PRECIPITATION',t('timelineRainAmountShort'),'precipitation-amount'],['CLOUD',t('cloudCoverage'),'cloud'],['WIND',t('wind'),'wind'],['GUST',t('gusts'),'gust']],metricHint=metric==='PRECIPITATION_PROBABILITY'?t('hourlyBandProbabilityHint'):metric==='PRECIPITATION'?t('hourlyBandAmountHint'):'';
  return `<section class="section agreement-section agreement-metric-${metric.toLowerCase().replaceAll('_','-')}" id="agreement"><div class="section-card"><div class="section-head"><div><h2>${esc(t('confidenceBand'))}</h2><p>${esc(t('chart_confidence_band_desc'))}</p></div></div><div class="chart-controls"><div class="segmented agreement-metric-tabs" data-control="confidence-metric">${metrics.map(([id,label,icon])=>`<button class="seg-btn ${metric===id?'active':''}" data-confidence-metric="${id}">${weatherIcons.renderMetric(icon,{size:'micro'})}<span>${esc(label)}</span></button>`).join('')}</div><div class="segmented" aria-label="${esc(t('chartHorizonAria'))}">${[[24,'24 h'],[72,'72 h'],[168,t('dayMode7')]].map(([hours,label])=>`<button class="seg-btn ${horizon===hours?'active':''}" data-chart-horizon="${hours}">${esc(label)}</button>`).join('')}</div></div>${metricHint?`<p class="agreement-metric-context">${esc(metricHint)}</p>`:''}${renderBandLegend(metric,Boolean(normals),bands)}<div class="chart-wrap agreement-chart-wrap" title="${esc(t('chartScrollTitle'))}">${renderBandChart(bands,metric,normals)}</div>${renderConfidenceTimeline(bands,metric)}${metric==='TEMPERATURE'&&!normals?`<div class="small">${esc(t('webNormals'))} : ${state.online?esc(t('webLoading')):esc(t('webUnavailableOffline'))}</div>`:''}</div></section>`;
}

function timelineMetricRail({label,central,rawMin,rawMax,engineDetail,unit,digits=0,tone=''}){
  const {t}=i18n(),normalizedInterval=interval=>Number.isFinite(interval?.low)&&Number.isFinite(interval?.high)?{low:Math.min(interval.low,interval.high),high:Math.max(interval.low,interval.high)}:null;
  const rawInterval=Number.isFinite(rawMin)&&Number.isFinite(rawMax)?{low:Math.min(rawMin,rawMax),high:Math.max(rawMin,rawMax)}:null,probableInterval=normalizedInterval(engineDetail?.allSourceInterval||engineDetail?.interval),retainedInterval=normalizedInterval(engineDetail?.interval);
  if(!rawInterval&&!probableInterval&&!retainedInterval)return '';
  const axisValues=[central,rawInterval?.low,rawInterval?.high,probableInterval?.low,probableInterval?.high,retainedInterval?.low,retainedInterval?.high].filter(Number.isFinite),min=axisValues.length?Math.min(...axisValues):null,max=axisValues.length?Math.max(...axisValues):null,span=Number.isFinite(min)&&Number.isFinite(max)?Math.max(.0001,max-min):null,pos=value=>Number.isFinite(value)&&Number.isFinite(span)?(span<=.0001?50:Math.max(0,Math.min(1,(value-min)/span))*100):50;
  const intervalStyle=interval=>interval?`--range-start:${pos(interval.low)}%;--range-end:${pos(interval.high)}%`:'',formatted=value=>Number.isFinite(value)?`${fmt(value,digits)}${unit}`:'—',formattedInterval=interval=>interval?`${formatted(interval.low)} – ${formatted(interval.high)}`:'—';
  const railLabel=[label,rawInterval?`${t('modelRange')} ${formattedInterval(rawInterval)}`:'',probableInterval?`${t('summaryProbableRange')} ${formattedInterval(probableInterval)}`:'',retainedInterval?`${t('summaryEngineRange')} ${formattedInterval(retainedInterval)}`:''].filter(Boolean).join(' · ');
  return `<div class="timeline-dispersion-rail metric-${tone}" style="--center:${pos(central)}%" role="img" aria-label="${attr(railLabel)}" title="${attr(railLabel)}">${rawInterval?`<span class="timeline-dispersion-envelope" style="${intervalStyle(rawInterval)}" aria-hidden="true"></span>`:''}${probableInterval?`<span class="timeline-dispersion-probable" style="${intervalStyle(probableInterval)}" aria-hidden="true"></span>`:''}${retainedInterval?`<span class="timeline-dispersion-retained" style="${intervalStyle(retainedInterval)}" aria-hidden="true"></span>`:''}${Number.isFinite(central)?'<i class="timeline-dispersion-center" aria-hidden="true"></i>':''}</div>`;
}
...
function renderTimelinePoint(p,mode,isFirst,isLast){
  ...
  const rainProbabilityRail=timelineMetricRail({label:t('insightsMetricProbability'),central:rainProb,rawMin:p.precipitationProbabilityMin,rawMax:p.precipitationProbabilityMax,engineDetail:null,unit:' %',digits:0,tone:'rain'}),rainAmountRail=timelineMetricRail({label:t('summaryRainExpectedLabel'),central:p.precipitationExpectedMm,rawMin:p.precipitationExpectedMinAcrossModelsMm,rawMax:p.precipitationExpectedMaxAcrossModelsMm,engineDetail:p.engineDetails?.precipitation,unit:' mm',digits:1,tone:'rain'}),cloudRail=timelineMetricRail({label:t('cloudCoverage'),central:p.cloudCoverPercent,rawMin:p.cloudCoverMinAcrossModels,rawMax:p.cloudCoverMaxAcrossModels,engineDetail:p.engineDetails?.cloud,unit:' %',digits:0,tone:'cloud'}),windRail=timelineMetricRail({label:t('wind'),central:p.windKmh,rawMin:p.windMinAcrossModels,rawMax:p.windMaxAcrossModels,engineDetail:p.engineDetails?.wind,unit:' km/h',digits:0,tone:'wind'}),gustRail=timelineMetricRail({label:t('gusts'),central:p.windGustKmh,rawMin:p.windGustMinAcrossModels,rawMax:p.windGustMaxAcrossModels,engineDetail:p.engineDetails?.gust,unit:' km/h',digits:0,tone:'wind'});
  const rainAgreement=Number.isFinite(p.precipitationOccurrenceAgreementPercent)?`${Math.round(p.precipitationOccurrenceAgreementPercent)}%`:'—',rainProbabilityMeta=`${t('insightsMetricProbability')} · ${t('rainAgreement')} ${rainAgreement}`,rainProbabilityCompact=`${t('timelineRainAgreementShort')} ${rainAgreement}`,rainConditionalValue=Number.isFinite(p.precipitationConditionalMm)?`${fmt(p.precipitationConditionalMm,1)} mm`:'—',rainConditionalMeta=Number.isFinite(p.precipitationConditionalMm)?`${t('summaryRainIfWetLabel')} ${rainConditionalValue}`:t('rainAmountUnavailable'),rainAmountMeta=`${t('summaryRainExpectedLabel')} · ${rainConditionalMeta}`,rainAmountCompact=`${t('timelineRainIfWetShort')} ${rainConditionalValue}`,cloudRange=Number.isFinite(p.cloudCoverMinAcrossModels)&&Number.isFinite(p.cloudCoverMaxAcrossModels)?`${p.cloudCoverMinAcrossModels}–${p.cloudCoverMaxAcrossModels}%`:'—',windRange=Number.isFinite(p.windMinAcrossModels)&&Number.isFinite(p.windMaxAcrossModels)?`${fmt(p.windMinAcrossModels)}–${fmt(p.windMaxAcrossModels)} km/h`:'—',gustRange=Number.isFinite(p.windGustMinAcrossModels)&&Number.isFinite(p.windGustMaxAcrossModels)?`${fmt(p.windGustMinAcrossModels)}–${fmt(p.windGustMaxAcrossModels)} km/h`:'—';
  return `<article class="timeline-point ${empty?'empty':''} ${consensusClass}" role="listitem"><div class="timeline-point-head"><strong>${esc(dateText)}</strong><span${context?` title="${attr(context)}"`:''}>${esc(context)}</span></div><div class="timeline-condition">${p.condition?aggregateConditionMarkup(p,'small'):'<span class="muted">—</span>'}<span${p.condition?` title="${attr(ci.label)}"`:''}>${p.condition?esc(ci.label):esc(t('dataUnavailable'))}</span></div><div class="timeline-temp-band" ${tempStyle}><div class="timeline-temp-value"><strong>${Number.isFinite(tempMain)?`${fmt(tempMain)}°`:'—'}</strong>${Number.isFinite(tempLow)&&Number.isFinite(tempHigh)&&Math.abs(tempHigh-tempLow)>=1?`<small>${fmt(tempLow)}–${fmt(tempHigh)}°</small>`:''}</div><div class="timeline-temp-track" aria-hidden="true"><i></i><b></b></div></div>${rainDot}<div class="timeline-metric timeline-rain-metric timeline-rain-probability-metric" title="${attr(`${rainProbabilityMeta} · ${source||t('limitedSignal')}`)}"><span class="timeline-metric-icon">${weatherIcons.renderMetric('precipitation-probability',{size:'tiny'})}</span><span class="timeline-metric-label">${esc(t('timelineRainProbabilityShort'))}</span><strong>${rainProb!=null?`${Math.round(rainProb)}%`:'—'}</strong>${rainProbabilityRail}<small>${esc(rainProbabilityCompact)}</small></div><div class="timeline-metric timeline-rain-metric timeline-rain-amount-metric" title="${attr(rainAmountMeta)}"><span class="timeline-metric-icon">${weatherIcons.renderMetric('precipitation-amount',{size:'tiny'})}</span><span class="timeline-metric-label">${esc(t('timelineRainAmountShort'))}</span><strong>${Number.isFinite(p.precipitationExpectedMm)?`${fmt(p.precipitationExpectedMm,1)} mm`:'—'}</strong>${rainAmountRail}<small>${esc(rainAmountCompact)}</small></div><div class="timeline-metric timeline-cloud-metric" title="${attr(`${t('cloudCoverage')} · ${cloudRange}`)}"><span class="timeline-metric-icon">${weatherIcons.renderMetric('cloud',{size:'tiny'})}</span><span class="timeline-metric-label">${esc(t('cloudCoverage'))}</span><strong>${Number.isFinite(p.cloudCoverPercent)?`${p.cloudCoverPercent}%`:'—'}</strong>${cloudRail}<small>${esc(cloudRange)}</small></div><div class="timeline-metric timeline-wind-metric" title="${attr(`${t('windMedianLegend')} · ${windRange}`)}"><span class="timeline-metric-icon">${weatherIcons.renderMetric('wind',{size:'tiny'})}</span><span class="timeline-metric-label">${esc(t('wind'))}</span><strong>${Number.isFinite(p.windKmh)?`${fmt(p.windKmh)} km/h`:'—'}</strong>${windRail}<small>${esc(windRange)}</small></div><div class="timeline-metric timeline-gust-metric" title="${attr(`${t('gusts')} · ${gustRange}`)}"><span class="timeline-metric-icon">${weatherIcons.renderMetric('gust',{size:'tiny'})}</span><span class="timeline-metric-label">${esc(t('gusts'))}</span><strong>${Number.isFinite(p.windGustKmh)?`${fmt(p.windGustKmh)} km/h`:'—'}</strong>${gustRail}<small>${esc(gustRange)}</small></div><div class="timeline-consensus"><div class="timeline-consensus-line"><span>${esc(t('modelConvergence'))}</span><strong>${consensus!=null?`${consensus}%`:'—'}</strong></div><div class="timeline-consensus-track" aria-hidden="true"><i style="--agreement:${consensus??0}%"></i></div><div class="timeline-consensus-meta">${consensus!=null?esc(modelCountLabel(p.modelCount)):esc(t('limitedComparison'))}</div>${divergence.length?`<div class="divergence-list" aria-label="${esc(t('disagreementVariableLegend'))}">${divergence.map(d=>`<span title="${esc(divergenceLabel(d))}">⚠ ${esc(divergenceShort(d))}</span>`).join('')}</div>`:`<div class="divergence-list stable"><span>✓ ${esc(t('coherent'))}</span></div>`}</div></article>`;
}
...
function divergenceIcon(x){
  if(x==='TEMPERATURE')return weatherIcons.renderMetric('temperature',{size:'micro'});
  if(x==='PRECIPITATION')return weatherIcons.renderMetric('precipitation',{size:'micro'});
  if(x==='WIND')return weatherIcons.renderMetric('wind',{size:'micro'});
  if(x==='CONDITION')return weatherIcons.render('PARTLY_CLOUDY',{size:'micro'});
...
function renderBandLegend(metric,hasNormals,bands=[]){
  const {t}=i18n(),unit=chartMetricUnit(metric),hasProbable=bands.some(b=>Number.isFinite(b.engineDetail?.allSourceInterval?.low)&&Number.isFinite(b.engineDetail?.allSourceInterval?.high)),hasRetained=bands.some(b=>Number.isFinite(b.engineDetail?.interval?.low)&&Number.isFinite(b.engineDetail?.interval?.high));
  return `<div class="chart-legend agreement-band-legend" aria-label="${esc(t('chartLegendAria'))}"><span><i class="legend-line mean"></i>${esc(t('centralForecastLine',{unit}))}</span><span><i class="legend-line raw-spread"></i>${esc(t('summarySpreadLegend'))}</span>${hasProbable?`<span><i class="legend-area probable-range"></i>${esc(t('summaryProbableRange'))}</span>`:''}${hasRetained?`<span><i class="legend-area retained-range"></i>${esc(t('summaryEngineRange'))}</span>`:''}${metric==='TEMPERATURE'&&hasNormals?`<span><i class="legend-line normal"></i>${esc(t('era5Thermal'))}</span>`:''}</div>`;
}

function renderConfidenceTimeline(bands,metric='TEMPERATURE'){
  const {t}=i18n();if(!bands?.length)return '';const agreementName=metric==='PRECIPITATION_PROBABILITY'?t('rainAgreement'):metric==='PRECIPITATION'?t('rainAmountAgreement'):t('modelConvergence'),maxSegments=24,step=Math.max(1,Math.ceil(bands.length/maxSegments)),sample=bands.filter((_,i)=>i%step===0).slice(0,maxSegments);if(sample[sample.length-1]!==bands[bands.length-1])sample[sample.length-1]=bands[bands.length-1];const first=bands[0],last=bands[bands.length-1],start=Number.isFinite(first.epochMs)?first.epochMs:localTimestampValue(first.timestamp),end=Number.isFinite(last.epochMs)?last.epochMs:localTimestampValue(last.timestamp),hours=Number.isFinite(start)&&Number.isFinite(end)?Math.max(0,Math.round((end-start)/36e5)):0,horizon=hours>=48?`J+${Math.max(1,Math.round(hours/24))}`:`+${hours} h`,percentLabel=b=>Number.isFinite(b?.percent)?`${Math.round(b.percent)}%`:'—',agreementLabel=b=>`${timeLabel(b.timestamp)} · ${agreementName} ${Number.isFinite(b?.percent)?`${Math.round(b.percent)}%`:t('unavailable')} · ${modelCountLabel(b.modelCount)}`;
  return `<div class="agreement-timeline" aria-label="${esc(t('agreementEvolutionAria'))}"><div class="agreement-timeline-head"><div><strong>${esc(t('agreementOverTime'))}</strong><span>${esc(agreementName)} · ${esc(t('agreementColorExplanation'))}</span></div><div class="agreement-level-legend"><span class="high"><i></i>${esc(t('levelHigh'))}</span><span class="medium"><i></i>${esc(t('levelMedium'))}</span><span class="low"><i></i>${esc(t('levelLow'))}</span></div></div><div class="agreement-strip">${sample.map(b=>`<button type="button" class="${confidenceClass(b.percent)}" data-agreement-time="${attr(b.timestamp)}" ${Number.isFinite(b.epochMs)?`data-agreement-epoch="${b.epochMs}"`:''} aria-label="${esc(agreementLabel(b))}" title="${esc(agreementLabel(b))}"></button>`).join('')}</div><div class="agreement-strip-labels"><span>${esc(t('now'))} <strong class="confidence ${confidenceClass(first.percent)}">${percentLabel(first)}</strong> · ${modelCountLabel(first.modelCount)}</span><span>${horizon} <strong class="confidence ${confidenceClass(last.percent)}">${percentLabel(last)}</strong> · ${modelCountLabel(last.modelCount)}</span></div></div>`;
}
...
function renderBandChart(bands,metric,normals){
  ...
  const {t}=i18n(),width=940,height=326,pad={l:62,r:24,t:30,b:54},unit=chartMetricUnit(metric),digits=chartMetricDigits(metric),interval=(b,key)=>{const value=key==='probable'?(b.engineDetail?.allSourceInterval||b.engineDetail?.interval):b.engineDetail?.interval;return Number.isFinite(value?.low)&&Number.isFinite(value?.high)?{low:Math.min(value.low,value.high),high:Math.max(value.low,value.high)}:null;};let ys=bands.flatMap(x=>[x.minValue,x.maxValue,x.meanValue,interval(x,'probable')?.low,interval(x,'probable')?.high,interval(x,'retained')?.low,interval(x,'retained')?.high]);
  ...
  const scale=metric==='PRECIPITATION_PROBABILITY'?chartScale(ys,{agreement:true}):chartScale(ys,{includeZero:metric!=='TEMPERATURE',ticks:6,minSpan:metric==='TEMPERATURE'?2:1,padding:.06}),x=i=>pad.l+i*(width-pad.l-pad.r)/(bands.length-1),y=v=>pad.t+(scale.max-v)*(height-pad.t-pad.b)/(scale.max-scale.min);
  const upper=bands.map((b,i)=>[x(i),y(b.maxValue)]),lower=bands.map((b,i)=>[x(i),y(b.minValue)]),mean=bands.map((b,i)=>[x(i),y(b.meanValue)]);
  const areaSegments=(kind,className,label)=>bands.slice(0,-1).map((b,i)=>{const next=bands[i+1],a=kind==='raw'?{low:b.minValue,high:b.maxValue}:interval(b,kind),z=kind==='raw'?{low:next.minValue,high:next.maxValue}:interval(next,kind);if(![a?.low,a?.high,z?.low,z?.high].every(Number.isFinite))return '';const points=`${x(i)},${y(a.high)} ${x(i+1)},${y(z.high)} ${x(i+1)},${y(z.low)} ${x(i)},${y(a.low)}`;return `<polygon class="chart-band-segment ${className}" points="${points}"><title>${esc(timeLabel(b.timestamp))} · ${esc(label)} ${fmt(a.low,digits)}–${fmt(a.high,digits)} ${esc(unit)}</title></polygon>`;}).join('');
  const rawSegments=areaSegments('raw','chart-band-raw',t('summarySpreadLegend')),probableSegments=areaSegments('probable','chart-band-probable',t('summaryProbableRange')),retainedSegments=areaSegments('retained','chart-band-retained',t('summaryEngineRange')),probableUpper=bands.map((b,i)=>{const v=interval(b,'probable');return v?[x(i),y(v.high)]:null;}),probableLower=bands.map((b,i)=>{const v=interval(b,'probable');return v?[x(i),y(v.low)]:null;}),retainedUpper=bands.map((b,i)=>{const v=interval(b,'retained');return v?[x(i),y(v.high)]:null;}),retainedLower=bands.map((b,i)=>{const v=interval(b,'retained');return v?[x(i),y(v.low)]:null;}),hasProbable=probableUpper.some(Boolean),hasRetained=retainedUpper.some(Boolean);
  ...
  const xTickIdx=chartTickIndices(bands.length,5),xGrid=xTickIdx.map((i,tickPosition)=>{const anchor=tickPosition===0?'start':tickPosition===xTickIdx.length-1?'end':'middle';return `<text class="chart-axis agreement-date-axis" x="${x(i)}" y="${height-19}" text-anchor="${anchor}">${esc(dateLabel(bands[i].timestamp.slice(0,10),i18n().locale,'long'))}</text><text class="chart-axis secondary" x="${x(i)}" y="${height-7}" text-anchor="${anchor}">${esc(timeLabel(bands[i].timestamp))}</text>`;}).join(''),dateGuides=xTickIdx.map(i=>`<line class="chart-date-guide" x1="${x(i)}" y1="${pad.t}" x2="${x(i)}" y2="${height-pad.b}"/>`).join('');
  ...
  const hoverValues=bands.map(b=>{const probable=interval(b,'probable'),retained=interval(b,'retained');return [Number.isFinite(b.meanValue)?b.meanValue:null,Number.isFinite(b.minValue)?b.minValue:null,Number.isFinite(b.maxValue)?b.maxValue:null,probable?.low??null,probable?.high??null,retained?.low??null,retained?.high??null,Number.isFinite(b.percent)?b.percent:null,Number.isFinite(b.modelCount)?b.modelCount:null,Number.isFinite(b.familyCount)?b.familyCount:null,Number.isFinite(b.precipitationProbability)?b.precipitationProbability:null,Number.isFinite(b.conditionalAmountMm)?b.conditionalAmountMm:null];}),agreementName=metric==='PRECIPITATION_PROBABILITY'?t('rainAgreement'):metric==='PRECIPITATION'?t('rainAmountAgreement'):t('modelConvergence'),rainMetric=['PRECIPITATION_PROBABILITY','PRECIPITATION'].includes(metric);
  return `<div class="chart-pro hover-chart-shell agreement-band-hover"><div class="chart-pro-head"><div class="chart-stat"><span>${esc(t('centralForecastLine',{unit}))}</span><strong data-band-hover-mean>—</strong>${rainMetric?`<small data-band-hover-rain></small>`:''}</div><div class="chart-stat"><span>${esc(t('summarySpreadLegend'))}</span><strong data-band-hover-range>—</strong></div>${hasProbable?`<div class="chart-stat"><span>${esc(t('summaryProbableRange'))}</span><strong data-band-hover-probable>—</strong></div>`:''}${hasRetained?`<div class="chart-stat"><span>${esc(t('summaryEngineRange'))}</span><strong data-band-hover-retained>—</strong></div>`:''}<div class="chart-stat compact"><span>${esc(agreementName)}</span><strong class="confidence" data-band-hover-agreement>—</strong><small data-band-hover-models></small></div></div><svg class="chart" data-hover-chart="agreement-band" data-hover-keys="${attr(JSON.stringify(bands.map(b=>b.timestamp)))}" data-hover-values="${attr(JSON.stringify(hoverValues))}" data-hover-mode="HOURLY" data-hover-unit="${attr(unit)}" data-hover-digits="${digits}" data-plot-left="${pad.l}" data-plot-right="${width-pad.r}" data-plot-top="${pad.t}" data-plot-bottom="${height-pad.b}" data-scale-min="${scale.min}" data-scale-max="${scale.max}" viewBox="0 0 ${width} ${height}" role="img" aria-label="${esc(t('agreementBandAria'))}"><rect class="chart-plot-bg" x="${pad.l}" y="${pad.t}" width="${width-pad.l-pad.r}" height="${height-pad.t-pad.b}" rx="9"/>${yGrid}${xGrid}<text class="chart-axis-unit" x="${pad.l}" y="${pad.t-11}">${unit}</text>${rawSegments}${probableSegments}${retainedSegments}${dateGuides}<path class="chart-range-edge" d="${svgLinePath(upper)}"/><path class="chart-range-edge" d="${svgLinePath(lower)}"/>${hasProbable?`<path class="chart-probable-edge" d="${svgLinePath(probableUpper)}"/><path class="chart-probable-edge" d="${svgLinePath(probableLower)}"/>`:''}${hasRetained?`<path class="chart-retained-edge" d="${svgLinePath(retainedUpper)}"/><path class="chart-retained-edge" d="${svgLinePath(retainedLower)}"/>`:''}<path class="chart-line" d="${svgLinePath(mean)}"/>${normalsSvg}${points}<line class="chart-hover-crosshair" data-hover-crosshair x1="${pad.l}" x2="${pad.l}" y1="${pad.t}" y2="${height-pad.b}"/><circle class="chart-hover-marker mean" data-hover-marker="0" cx="${pad.l}" cy="${pad.t}" r="5"/><circle class="chart-hover-marker range-min" data-hover-marker="1" cx="${pad.l}" cy="${pad.t}" r="4"/><circle class="chart-hover-marker range-max" data-hover-marker="2" cx="${pad.l}" cy="${pad.t}" r="4"/></svg><div class="chart-hover-status" data-hover-status>${esc(t('chartHoverHint'))}</div></div>`;
}
...
function evolutionRangeText(low,high,unit){return Number.isFinite(low)&&Number.isFinite(high)?`${fmt(low,1)}–${fmt(high,1)}${unit}`:'—';}
function renderEvolutionSummary(days,meta){
  const {t}=i18n(),candidates=days.filter(day=>day?.e),peak=candidates.reduce((best,row)=>(row.e.medianAbsDelta/(meta.threshold||1))>(best.e.medianAbsDelta/(meta.threshold||1))?row:best,candidates[0]);if(!peak)return '';
  const e=peak.e,delta=e.medianDelta,sign=delta>0?'+':'',revision=`${sign}${fmt(delta,1)}${meta.unit}`,trend=trendText(e.trend,delta,meta.unit),range=evolutionRangeText(e.currentLow,e.currentHigh,meta.unit);
  return `<div class="evolution-summary ${evolutionTrendClass(e.trend)}"><div class="evolution-summary-head"><div><strong>${esc(t('evolutionReadingTitle'))}</strong><span>${esc(t('evolutionReadingBody',{variable:meta.label}))}</span></div><span>${meta.icon}${esc(meta.label)}</span></div><div class="evolution-summary-grid"><div><span>${esc(t('evolutionStrongestLead'))}</span><strong>${esc(dateLabel(peak.date,i18n().locale))}</strong><small>${esc(t('modelsCompared',{count:e.comparedModels}))}</small></div><div><span>${esc(t('evolutionCentralRevision'))}</span><strong>${esc(revision)}</strong><small>${esc(trend)}</small></div><div><span>${esc(t('evolutionCurrentSpread'))}</span><strong>${esc(range)}</strong><small>${esc(t('evolutionComparableSpread'))}</small></div></div></div>`;
}
...
function renderEvolutionTrackSvg(data,width=640){
  ...
  const thresholdTopLine=thresholdTopVisible?`<line class="evolution-threshold-line" x1="${pad.l}" x2="${width-pad.r}" y1="${bandTop}" y2="${bandTop}"/>`:'';
  const thresholdBottomLine=thresholdBottomVisible?`<line class="evolution-threshold-line" x1="${pad.l}" x2="${width-pad.r}" y1="${bandBottom}" y2="${bandBottom}"/>`:'';
  const thresholdTopLabel=thresholdTopVisible?`<text class="evolution-threshold-label" x="${width-pad.r-2}" y="${Math.max(plotTop+9,bandTop-3)}" text-anchor="end">+${esc(fmt(thresholdValue,1))}${esc(unit)}</text>`:'';
  const thresholdBottomLabel=thresholdBottomVisible?`<text class="evolution-threshold-label" x="${width-pad.r-2}" y="${Math.min(plotBottom-3,bandBottom+10)}" text-anchor="end">−${esc(fmt(thresholdValue,1))}${esc(unit)}</text>`:'';
...
function exportCityData(cityId,format){
  const city=state.cities.find(c=>c.id===cityId),f=state.forecasts[cityId];if(!city||!f){toast(i18n().t('nothingToExport'),{type:'warning',title:i18n().t('export')});return;}const rows=buildExportRows(cityId),stamp=new Date().toISOString().slice(0,10),base=`meteocompare-${city.name.toLowerCase().replace(/[^a-z0-9]+/gi,'-')}-${stamp}`;
  if(format==='json'){const payload={exportedAt:new Date().toISOString(),city,view:{mode:state.settings.detailViewMode,tab:state.settings.detailTab,metric:state.settings.confidenceMetric,horizon:state.settings.chartHorizon,compareModels:state.compareModelIds},forecast:f,agreement:{temperature:cachedBand(f,'TEMPERATURE',168),precipitationProbability:cachedBand(f,'PRECIPITATION_PROBABILITY',168),precipitation:cachedBand(f,'PRECIPITATION',168),cloud:cachedBand(f,'CLOUD',168),wind:cachedBand(f,'WIND',168),gust:cachedBand(f,'GUST',168)},bias:state.bias[cityId],rows};if(downloadText(`${base}.json`,JSON.stringify(payload,null,2),'application/json;charset=utf-8'))void trackAnalyticsEvent('Data Exported',state.route,{format});return;}
  const keys=[...new Set(rows.flatMap(r=>Object.keys(r)))],csv=[keys.map(csvCell).join(';'),...rows.map(r=>keys.map(k=>csvCell(r[k])).join(';'))].join('\n');if(downloadText(`${base}.csv`,csv,'text/csv;charset=utf-8'))void trackAnalyticsEvent('Data Exported',state.route,{format});
}
...
function forecastDateCellParts(date,locale=i18n().locale){
  const d=new Date(`${date}T12:00:00Z`);
  try{return {weekdayDay:formatDateTime(d,{weekday:'short',day:'numeric',timeZone:'UTC'},locale),day:formatDateTime(d,{day:'numeric',timeZone:'UTC'},locale),month:formatDateTime(d,{month:'short',timeZone:'UTC'},locale)};}
  catch{return {weekdayDay:String(date||'').slice(5),day:String(date||'').slice(8,10),month:String(date||'').slice(5,7)};}
}
...
function renderDailyTable(f,tab,biases,normals=null){
  ...
  return `<div class="table-wrap"><table class="forecast-table"><thead><tr><th>${esc(t('day'))}</th>${ids.map(id=>{const m=getModel(id);return `<th title="${m?.family||''} · ${m?.resolutionKm||'?'} km">${renderForecastModelHeader(id,tab,biases,f.city.id||state.route.id,true)}</th>`;}).join('')}</tr></thead><tbody>${dates.map(date=>{const parts=forecastDateCellParts(date);return `<tr class="${date===today?'current':''}"><td><strong class="forecast-date-primary">${esc(parts.weekdayDay)}</strong><span class="forecast-date-month">${esc(parts.month)}</span>${date===today?`<span class="cell-sub">${esc(t('currentDay'))}</span>`:''}</td>${ids.map(id=>renderDailyCell(f.seriesByModel[id],date,tab,biases?.[id],normals?.[date.slice(5)])).join('')}</tr>`;}).join('')}</tbody></table></div>`;
}
...
function renderDailyCell(s,date,tab,modelBias,normal=null){
  ...
  if(tab==='CONDITIONS'){const x=dailyCondition(s,date),ci=localizedConditionInfo(x.condition),prob=s.daily.precipitationProbabilityMax[i],cloud=dailyCloudCoverMean(s,date);const isWet=['RAIN','RAIN_SHOWERS','THUNDERSTORM','FREEZING_RAIN','SNOW','SNOW_SHOWERS'].includes(x.condition),isSky=['CLEAR','MAINLY_CLEAR','PARTLY_CLOUDY','OVERCAST'].includes(x.condition);const badge=isWet?(Number.isFinite(prob)?`${weatherIcons.renderMetric('precipitation',{size:'micro'})} ${prob}%`:null):(isSky&&Number.isFinite(cloud)?`${weatherIcons.renderMetric('cloud',{size:'micro'})} ${cloud}%`:null);return `<td title="${esc(ci.label)}${x.inferred?' · '+i18n().t('conditionInferred'):''}">${conditionMarkup(x.condition,'small',x.inferred)}<span class="condition-label">${esc(ci.label)}</span>${badge?`<span class="cell-sub weather-cell-meta">${badge}</span>`:''}${partial}</td>`;}
...
function renderHourlyTable(f,tab,biases){
  ...
  return `<div class="table-wrap"><table class="forecast-table"><thead><tr><th>${esc(t('hour'))}</th>${ids.map(id=>`<th>${renderForecastModelHeader(id,tab,biases,f.city.id||state.route.id,false)}</th>`).join('')}</tr></thead><tbody>${rows.map(row=>{const parts=forecastDateCellParts(row.ts.slice(0,10));return `<tr class="${row.epochMs===targetEpoch?'current':''}"><td><strong class="forecast-date-primary">${esc(parts.day)}</strong><span class="forecast-date-month">${esc(parts.month)}</span><span class="forecast-date-time">${esc(timeLabel(row.ts))}</span>${row.epochMs===targetEpoch?`<span class="forecast-date-now">${esc(t('nowSuffix'))}</span>`:''}</td>${ids.map(id=>renderHourlyCell(f.seriesByModel[id],row,tab)).join('')}</tr>`;}).join('')}</tbody></table></div>`;
}
...
function biasFullDateLabel(date,withWeekday=false){
  const value=new Date(`${date}T12:00:00Z`);if(Number.isNaN(value.getTime()))return date;
  return formatDateTime(value,{...(withWeekday?{weekday:'long'}:{}),day:'2-digit',month:'short',year:'numeric',timeZone:'UTC'});
}

function renderBiasTrendChart(samples,variable){
  const {t}=i18n();if(samples.length<4)return `<div class="banner info">${esc(t('insufficientRecentComparison'))}</div>`;
  const rows=samples.slice(-Math.min(14,samples.length)),offset=samples.length-rows.length,absolute=samples.map(s=>Math.abs(s.error)),daily=rows.map(s=>Math.abs(s.error)),rolling=rows.map((_,i)=>{const absoluteIndex=offset+i;return absoluteIndex>=6?average(absolute.slice(absoluteIndex-6,absoluteIndex+1)):null;}),all=[...daily,...rolling].filter(Number.isFinite),unit=biasUnit(variable).trim(),digits=variable==='PRECIPITATION'?1:1;
  const width=720,height=260,pad={l:54,r:18,t:24,b:50},scale=chartScale(all,{includeZero:true,ticks:5,minSpan:variable==='TEMPERATURE'?1:variable==='PRECIPITATION'?0.5:3,padding:.08}),x=i=>pad.l+i*(width-pad.l-pad.r)/Math.max(1,rows.length-1),y=v=>pad.t+(scale.max-v)*(height-pad.t-pad.b)/(scale.max-scale.min);
  const yGrid=scale.ticks.map(value=>`<line class="bias-chart-grid" x1="${pad.l}" y1="${y(value)}" x2="${width-pad.r}" y2="${y(value)}"/><text class="bias-chart-axis" x="${pad.l-9}" y="${y(value)+4}" text-anchor="end">${fmt(value,digits)}</text>`).join('');
  const tickIdx=chartTickIndices(rows.length,4),xGrid=tickIdx.map((i,pos)=>{const anchor=pos===0?'start':pos===tickIdx.length-1?'end':'middle';return `<line class="bias-chart-grid vertical" x1="${x(i)}" y1="${pad.t}" x2="${x(i)}" y2="${height-pad.b}"/><text class="bias-chart-axis bias-date-axis" x="${x(i)}" y="${height-16}" text-anchor="${anchor}">${esc(biasFullDateLabel(rows[i].date))}</text>`;}).join('');
  const dailyPts=daily.map((v,i)=>[x(i),y(v)]),rollingPts=rolling.map((v,i)=>[x(i),y(v)]),dots=daily.map((v,i)=>`<circle class="bias-trend-point" cx="${x(i)}" cy="${y(v)}" r="3.2"><title>${esc(biasFullDateLabel(rows[i].date,true))} · ${esc(t('biasDailyAbsoluteError'))} ${fmt(v,digits)} ${unit}</title></circle>`).join('');
  const hoverValues=[daily.map(v=>Number.isFinite(v)?v:null),rolling.map(v=>Number.isFinite(v)?v:null)],hoverMarkers=`<circle class="chart-hover-marker" data-hover-marker="0" style="--series:var(--muted)" cx="${pad.l}" cy="${pad.t}" r="5"/><circle class="chart-hover-marker" data-hover-marker="1" style="--series:var(--primary)" cx="${pad.l}" cy="${pad.t}" r="5"/>`;
  const legend=`<div class="chart-legend bias-interactive-legend"><span data-hover-series="0"><i class="legend-line bias-trend-daily"></i>${esc(t('biasDailyAbsoluteError'))}<strong class="bias-legend-live"><em data-hover-value>—</em></strong></span><span data-hover-series="1"><i class="legend-line bias-trend-rolling"></i>${esc(t('biasRollingMae7d'))}<strong class="bias-legend-live"><em data-hover-value>—</em></strong></span></div>`;
  return `<div class="bias-trend-chart-shell chart-pro hover-chart-shell">${legend}<svg class="bias-trend-chart" data-hover-chart="bias-trend" data-hover-keys="${attr(JSON.stringify(rows.map(r=>r.date)))}" data-hover-values="${attr(JSON.stringify(hoverValues))}" data-hover-mode="DAILY" data-hover-date-style="FULL_DATE" data-hover-unit="${attr(unit)}" data-hover-digits="${digits}" data-plot-left="${pad.l}" data-plot-right="${width-pad.r}" data-plot-top="${pad.t}" data-plot-bottom="${height-pad.b}" data-scale-min="${scale.min}" data-scale-max="${scale.max}" viewBox="0 0 ${width} ${height}" role="img" aria-label="${attr(t('recentEvolution'))}"><rect class="chart-plot-bg" x="${pad.l}" y="${pad.t}" width="${width-pad.l-pad.r}" height="${height-pad.t-pad.b}" rx="9"/>${yGrid}${xGrid}<text class="chart-axis-unit" x="${pad.l}" y="${pad.t-9}">${esc(unit)}</text><path class="bias-trend-error-line" d="${svgLinePath(dailyPts)}"/><path class="bias-trend-mae-line" d="${svgLinePath(rollingPts)}"/>${dots}<line class="chart-hover-crosshair" data-hover-crosshair x1="${pad.l}" x2="${pad.l}" y1="${pad.t}" y2="${height-pad.b}"/>${hoverMarkers}</svg><div class="chart-hover-status" data-hover-status>${esc(t('chartHoverHint'))}</div></div>`;
}
...
function renderBiasHistoryChart(samples,variable){
  ...
  const width=980,height=350,pad={l:64,r:24,t:30,b:60},unit=biasUnit(variable).trim(),digits=variable==='PRECIPITATION'?1:0,values=samples.flatMap(x=>[x.forecast,x.observation]),scale=chartScale(values,{includeZero:variable!=='TEMPERATURE',ticks:6,minSpan:variable==='TEMPERATURE'?2:variable==='PRECIPITATION'?1:5,padding:.07}),x=i=>pad.l+i*(width-pad.l-pad.r)/Math.max(1,samples.length-1),y=v=>pad.t+(scale.max-v)*(height-pad.t-pad.b)/(scale.max-scale.min);
  const yGrid=scale.ticks.map(value=>`<line class="bias-chart-grid" x1="${pad.l}" y1="${y(value)}" x2="${width-pad.r}" y2="${y(value)}"/><text class="bias-chart-axis" x="${pad.l-10}" y="${y(value)+4}" text-anchor="end">${fmt(value,digits)}</text>`).join(''),tickIdx=chartTickIndices(samples.length,5),xGrid=tickIdx.map((i,pos)=>{const anchor=pos===0?'start':pos===tickIdx.length-1?'end':'middle';return `<line class="bias-chart-grid vertical" x1="${x(i)}" y1="${pad.t}" x2="${x(i)}" y2="${height-pad.b}"/><text class="bias-chart-axis bias-date-axis" x="${x(i)}" y="${height-17}" text-anchor="${anchor}">${esc(biasFullDateLabel(samples[i].date))}</text>`;}).join('');
  const forecastPts=samples.map((s,i)=>[x(i),y(s.forecast)]),observationPts=samples.map((s,i)=>[x(i),y(s.observation)]),connectors=samples.map((s,i)=>{const error=s.forecast-s.observation,cls=Math.abs(error)<=.0001?'neutral':error>0?'over':'under';return `<line class="bias-error-connector ${cls}" x1="${x(i)}" y1="${y(s.forecast)}" x2="${x(i)}" y2="${y(s.observation)}"><title>${esc(biasFullDateLabel(s.date,true))} · ${esc(t('chartError'))} ${error>0?'+':''}${fmt(error,digits)} ${unit}</title></line>`;}).join(''),dots=samples.map((s,i)=>`<circle class="bias-point observation" cx="${x(i)}" cy="${y(s.observation)}" r="4"><title>${esc(biasFullDateLabel(s.date,true))} · ${esc(t('observation'))} ${fmt(s.observation,digits)} ${unit}</title></circle><circle class="bias-point forecast" cx="${x(i)}" cy="${y(s.forecast)}" r="4"><title>${esc(biasFullDateLabel(s.date,true))} · ${esc(t('forecastD1'))} ${fmt(s.forecast,digits)} ${unit}</title></circle>`).join('');
  const errors=samples.map(s=>Math.abs(s.forecast-s.observation)),gaps=samples.map(s=>s.forecast-s.observation),mae=errors.reduce((a,v)=>a+v,0)/errors.length,last=samples[samples.length-1],lastError=last.forecast-last.observation,hoverValues=[samples.map(s=>s.forecast),samples.map(s=>s.observation)],hoverMarkers=`<circle class="chart-hover-marker" data-hover-marker="0" style="--series:var(--primary)" cx="${pad.l}" cy="${pad.t}" r="5"/><circle class="chart-hover-marker" data-hover-marker="1" style="--series:var(--text)" cx="${pad.l}" cy="${pad.t}" r="5"/>`;
  return `<div class="bias-chart-shell chart-pro hover-chart-shell"><div class="chart-pro-head"><div class="chart-stat"><span>${esc(t('biasMae'))}</span><strong>${fmt(mae,digits)} ${unit}</strong></div><div class="chart-stat"><span>${esc(t('chartLastGap'))}</span><strong class="${lastError>0?'chart-over':lastError<0?'chart-under':''}">${lastError>0?'+':''}${fmt(lastError,digits)} ${unit}</strong></div><div class="chart-stat compact"><span>${esc(t('biasSamples',{count:samples.length}))}</span></div></div><div class="chart-legend bias-interactive-legend"><span data-hover-series="0"><i class="legend-line bias-forecast"></i>${esc(t('forecastD1'))}<strong class="bias-legend-live"><em data-hover-value>—</em></strong></span><span data-hover-series="1"><i class="legend-line bias-observation"></i>${esc(t('observation'))}<strong class="bias-legend-live"><em data-hover-value>—</em></strong></span><span><i class="legend-line bias-gap"></i>${esc(t('chartForecastObservationGap'))}<strong class="bias-legend-live"><em data-bias-hover-gap>—</em></strong></span></div><svg class="bias-chart" data-hover-chart="bias-history" data-hover-keys="${attr(JSON.stringify(samples.map(s=>s.date)))}" data-hover-values="${attr(JSON.stringify(hoverValues))}" data-hover-gaps="${attr(JSON.stringify(gaps))}" data-hover-mode="DAILY" data-hover-date-style="FULL_DATE" data-hover-unit="${attr(unit)}" data-hover-digits="${digits}" data-plot-left="${pad.l}" data-plot-right="${width-pad.r}" data-plot-top="${pad.t}" data-plot-bottom="${height-pad.b}" data-scale-min="${scale.min}" data-scale-max="${scale.max}" viewBox="0 0 ${width} ${height}" role="img" aria-label="${attr(`${t('biasHistoryTitle')} · ${biasVariableLabel(variable)}`)}"><rect class="chart-plot-bg" x="${pad.l}" y="${pad.t}" width="${width-pad.l-pad.r}" height="${height-pad.t-pad.b}" rx="9"/>${yGrid}${xGrid}<text class="chart-axis-unit" x="${pad.l}" y="${pad.t-11}">${esc(unit)}</text>${connectors}<path class="bias-chart-observation" d="${svgLinePath(observationPts)}"/><path class="bias-chart-forecast" d="${svgLinePath(forecastPts)}"/>${dots}<line class="chart-hover-crosshair" data-hover-crosshair x1="${pad.l}" x2="${pad.l}" y1="${pad.t}" y2="${height-pad.b}"/>${hoverMarkers}</svg><div class="chart-hover-status" data-hover-status>${esc(t('chartHoverHint'))}</div></div>`;
}
...
function renderBiasDetailPage(route){
  ...
  const r=selection.reliability,rank=selection.rank,baseline=selection.baseline,deltaBaseline=baseline?r.meanAbsoluteError-baseline.meanAbsoluteError:null,hasTrend=Number.isFinite(r.recentMeanAbsoluteError)&&Number.isFinite(r.previousMeanAbsoluteError);
  const trendSummary=hasTrend?`<div class="bias-trend-summary"><div><span>${esc(t('previousPeriod'))}</span><strong>${formatBiasValue(r.previousMeanAbsoluteError,variable)}</strong></div><span class="bias-trend-arrow">→</span><div><span>${esc(t('lastSevenDays'))}</span><strong>${formatBiasValue(r.recentMeanAbsoluteError,variable)}</strong></div><span class="pill confidence ${r.trend==='IMPROVING'?'high':r.trend==='DECLINING'?'low':'medium'}">${esc(biasTrendLabel(r))}</span></div>`:`<div class="banner info">${esc(t('insufficientRecentComparison'))}</div>`;
  const baselineBlock=baseline?`<div class="bias-overview-baseline"><div class="bias-subhead"><div><h3>${esc(t('multiModelComparison'))}</h3><p>${esc(t('multiModelReferenceDesc'))}</p></div></div><div class="bias-baseline"><div><span>${esc(model.name)}</span><strong>${formatBiasValue(r.meanAbsoluteError,variable)} MAE</strong></div><div><span>${esc(t('multiModelAverage'))}</span><strong>${formatBiasValue(baseline.meanAbsoluteError,variable)} MAE</strong></div><div class="bias-baseline-result ${deltaBaseline<=0?'better':'worse'}">${esc(deltaBaseline<=0?t('modelBetterBy',{model:model.name,value:formatBiasValue(Math.abs(deltaBaseline),variable)}):t('ensembleBetterBy',{value:formatBiasValue(Math.abs(deltaBaseline),variable)}))}</div></div></div>`:'';
...
async function importLocalBackupFile(file){
  ...
  try{const backup=JSON.parse(await file.text());if(!confirm(t('backupImportConfirm')))return;const result=await restoreLocalBackup(backup,{replace:true});void trackAnalyticsEvent('Local Backup Imported',state.route);toast(t('backupImported',{cities:result.cities}),{type:'success',title:t('backupTitle')});setTimeout(()=>location.reload(),350);}
...
function renderLocalDataPage(){
  ...
  const generated=stats?.generatedAt?formatDateTime(new Date(stats.generatedAt),{dateStyle:'short',timeStyle:'short'},locale):'';
  ...
  const configTotal=categoryTotal('favorites','settings'),forecastTotal=categoryTotal('forecasts','marine'),analysisTotal=categoryTotal('normals','bias','evolution');
...
function disagreementAnalysis(cityId){
  const f=state.forecasts[cityId];if(!f)return null;const profile=localConsensusWeights(cityId),weights=state.settings.localWeightedConsensus&&profile.ready?profile.maps:null,opts={weightsByVariable:weights||{}},points=selectRegularTimelinePoints(cachedTimelinePoints(f,'HOURLY',new Date(),opts),24,1),bands={TEMPERATURE:cachedBand(f,'TEMPERATURE',24,weights),PRECIPITATION:cachedBand(f,'PRECIPITATION',24,weights),WIND:cachedBand(f,'WIND',24,weights)},variables=['TEMPERATURE','PRECIPITATION','WIND','CONDITION'];
...
function forecastEngineIntervalMeta(detail,unit='',digits=1){
  if(!detail)return '';
  const {t}=i18n(),retained=forecastEngineInterval(detail,unit,digits),all=forecastEngineFormatInterval(detail.allSourceInterval,unit,digits),rows=[];
  if(all&&all!==retained)rows.push(`<small class="forecast-engine-result-range all-sources">${esc(t('forecastEngineAllSourcesRange'))} ${esc(all)}</small>`);
  if(retained)rows.push(`<small class="forecast-engine-result-range retained">${esc(t('forecastEngineRetainedRange'))} ${esc(retained)}</small>`);
  return rows.join('');
}
...
function forecastEngineShortDate(date){
  try{return formatDateTime(new Date(`${date}T12:00:00`),{weekday:'short',day:'numeric'});}catch{return date.slice(5);}
}
...
function forecastEngineDivergenceForDate(date,matrix){
  const aggs=FORECAST_ENGINES.map(engine=>matrix[engine]?.[date]).filter(Boolean),temp=Math.max(forecastEngineSpread(aggs.map(a=>a.tempMax)),forecastEngineSpread(aggs.map(a=>a.tempMin))),rainProbability=forecastEngineSpread(aggs.map(a=>a.precipProbability)),rain=forecastEngineSpread(aggs.map(a=>forecastEngineMetricValue(a,'precipExpected'))),wind=forecastEngineSpread(aggs.map(a=>a.wind)),gust=forecastEngineSpread(aggs.map(a=>a.gust)),cloud=forecastEngineSpread(aggs.map(a=>a.cloud)),conditions=new Set(aggs.map(a=>a.condition).filter(Boolean)).size;
  const drivers=[{key:'temperature',value:temp,score:temp/4},{key:'rainProbability',value:rainProbability,score:rainProbability/35},{key:'rain',value:rain,score:rain/8},{key:'wind',value:wind,score:wind/15},{key:'gust',value:gust,score:gust/20},{key:'cloud',value:cloud,score:cloud/50},{key:'conditions',value:conditions,score:conditions>1?.42:0}].sort((a,b)=>b.score-a.score),score=drivers[0]?.score||0,level=score>=.75?'high':score>=.35?'medium':'low';
  return {date,temp,rainProbability,rain,wind,gust,cloud,conditions,drivers,driver:drivers[0],score,level};
}
function forecastEngineDriverMeta(driver){
  const {t}=i18n(),key=driver?.key||'temperature',value=Number(driver?.value)||0,map={temperature:{label:t('temperature'),icon:'temperature',value:`Δ ${fmt(value,1)}°`},rainProbability:{label:t('timelineRainProbabilityShort'),icon:'precipitation-probability',value:`Δ ${fmt(value,0)} %`},rain:{label:t('timelineRainAmountShort'),icon:'precipitation-amount',value:`Δ ${fmt(value,1)} mm`},wind:{label:t('wind'),icon:'wind',value:`Δ ${fmt(value,0)} km/h`},gust:{label:t('gusts'),icon:'gust',value:`Δ ${fmt(value,0)} km/h`},cloud:{label:t('cloudCoverage'),icon:'cloud',value:`Δ ${fmt(value,0)} %`},conditions:{label:t('conditions'),icon:'cloud',value:t('forecastEngineConditionVariants',{count:Math.round(value)})}};
  return map[key]||map.temperature;
}
function forecastEngineComparisonSummary(dates,matrix){
  const rows=dates.map(date=>forecastEngineDivergenceForDate(date,matrix));if(!rows.length)return null;
  const peak=rows.reduce((best,row)=>row.score>best.score?row:best,rows[0]),marked=rows.filter(row=>row.level!=='low').length;
  return {rows,peak,marked,level:peak.level,driver:peak.driver};
}
function renderForecastEngineComparisonSummary(dates,matrix){
  const {t}=i18n(),summary=forecastEngineComparisonSummary(dates,matrix);if(!summary)return '';
  const levelLabel=t(summary.level==='high'?'forecastEngineDivergenceHigh':summary.level==='medium'?'forecastEngineDivergenceMedium':'forecastEngineDivergenceLow'),driver=forecastEngineDriverMeta(summary.driver);
  return `<section class="forecast-engine-snapshot ${summary.level}"><div class="forecast-engine-snapshot-head"><div><h3>${esc(t('forecastEngineComparisonSummaryTitle'))}</h3><p>${esc(t('forecastEngineComparisonSummaryBody'))}</p></div><span>${esc(levelLabel)}</span></div><div class="forecast-engine-snapshot-grid"><article><span>${esc(t('forecastEngineOverallSpread'))}</span><strong>${esc(levelLabel)}</strong><small>${esc(t('forecastEngineMarkedDays',{count:summary.marked,total:dates.length}))}</small></article><article><span>${esc(t('forecastEnginePeakDay'))}</span><strong>${esc(forecastEngineShortDate(summary.peak.date))}</strong><small>${esc(t('forecastEngineInspectDay'))}</small></article><article><span>${esc(t('forecastEngineMainDriver'))}</span><strong>${weatherIcons.renderMetric(driver.icon,{size:'tiny'})}${esc(driver.label)}</strong><small>${esc(driver.value)}</small></article></div><p class="forecast-engine-comparison-note"><span aria-hidden="true">ⓘ</span>${esc(t('forecastEngineCentralDifferenceNote'))}</p></section>`;
}
function renderForecastEngineLineChart(dates,matrix,selected,{key,detailKey=null,label,unit='',digits=1}){
  const {t}=i18n(),rows=FORECAST_ENGINES.map(engine=>({engine,values:dates.map(date=>forecastEngineMetricValue(matrix[engine]?.[date],key))})),details=dates.map(date=>detailKey?matrix[selected]?.[date]?.engineDetails?.[detailKey]||null:null),intervalValues=details.flatMap(detail=>[detail?.allSourceInterval?.low,detail?.allSourceInterval?.high,detail?.interval?.low,detail?.interval?.high]).filter(Number.isFinite),finite=[...rows.flatMap(row=>row.values).filter(Number.isFinite),...intervalValues];
  ...
  const intervalBars=details.map((detail,i)=>{const bar=(interval,kind,width,labelKey)=>{const low=interval?.low,high=interval?.high;if(!Number.isFinite(low)||!Number.isFinite(high))return '';const top=y(Math.max(low,high)),bottom=y(Math.min(low,high));return `<rect class="forecast-engine-chart-interval ${kind}" x="${(x(i)-width/2).toFixed(1)}" y="${top.toFixed(1)}" width="${width}" height="${Math.max(2,bottom-top).toFixed(1)}" rx="${width/2}"><title>${esc(forecastEngineShortDate(dates[i]))} · ${esc(t(labelKey))} ${esc(forecastEngineFormatInterval(interval,unit,digits))}</title></rect>`;};return `${bar(detail?.allSourceInterval,'all-sources',30,'forecastEngineAllSourcesRange')}${bar(detail?.interval,'retained',10,'forecastEngineRetainedRange')}`;}).join('');
  ...
  const legend=FORECAST_ENGINES.map(engine=>`<span class="${engine===selected?'selected-engine':''}"><i class="engine-${engine.toLowerCase().replaceAll('_','-')}"></i>${esc(forecastEngineName(engine))}</span>`).join(''),intervalLegend=intervalValues.length?`<span class="forecast-engine-interval-legend all-sources"><i></i>${esc(t('forecastEngineAllSourcesRange'))}</span><span class="forecast-engine-interval-legend retained"><i></i>${esc(t('forecastEngineRetainedRange'))}</span>`:'';
  return `<article class="forecast-engine-chart-card"><div class="forecast-engine-chart-head"><h4>${esc(label)}</h4><span>${esc(t('forecastEngineChartSelected',{engine:forecastEngineName(selected)}))}</span></div><svg class="forecast-engine-chart" viewBox="0 0 ${w} ${h}" role="img" aria-label="${attr(`${label}. ${t('forecastEngineCentralDifferenceNote')}`)}" preserveAspectRatio="xMidYMid meet">${backgrounds}${intervalBars}${grid}${xLabels}${series}</svg><div class="forecast-engine-chart-legend">${legend}${intervalLegend}</div></article>`;
}
function renderForecastEngineDivergenceTimeline(dates,matrix,selectedDate){
  const {t}=i18n();
  const items=dates.map(date=>{const {drivers,score,level}=forecastEngineDivergenceForDate(date,matrix),label=t(level==='high'?'forecastEngineDivergenceHigh':level==='medium'?'forecastEngineDivergenceMedium':'forecastEngineDivergenceLow'),width=Math.max(8,Math.min(100,Math.round(score*100))),active=date===selectedDate;
    const metrics=drivers.slice(0,2).map((driver,index)=>{const meta=forecastEngineDriverMeta(driver),metricWidth=Math.min(100,Math.round(driver.score*100));return `<div class="forecast-engine-divergence-metric ${index===0?'primary':''}"><div><span>${meta.icon?weatherIcons.renderMetric(meta.icon,{size:'micro'}):''}${esc(meta.label)}</span><b>${esc(meta.value)}</b></div><i aria-hidden="true"><em style="--metric-width:${metricWidth}%"></em></i></div>`;}).join('');
    return `<button type="button" role="tab" aria-selected="${active}" aria-controls="forecast-engine-detail-panel" class="forecast-engine-divergence-day ${level} ${active?'selected':''}" data-engine-detail-date="${attr(date)}"><div class="forecast-engine-divergence-day-head"><span>${esc(forecastEngineShortDate(date))}</span><strong>${esc(label)}</strong></div><div class="forecast-engine-divergence-meter" aria-hidden="true"><i style="--divergence:${width}%"></i></div><div class="forecast-engine-divergence-driver">${esc(t('forecastEngineMainDriver'))}</div><div class="forecast-engine-divergence-metrics">${metrics}</div><small>${esc(t('forecastEngineInspectDay'))}</small></button>`;
  }).join('');
  return `<section class="forecast-engine-divergence"><div class="forecast-engine-visual-head compact"><div><h4>${esc(t('forecastEngineDivergenceTimeline'))}</h4><p>${esc(t('forecastEngineDivergenceTimelineBody'))}</p></div></div><div class="forecast-engine-divergence-track" role="tablist" aria-label="${attr(t('forecastEngineDetailDateAria'))}">${items}</div></section>`;
}
function renderForecastEngineComparisonModal(cityId){
  ...
  const comparisonSummary=forecastEngineComparisonSummary(dates,matrix),detailDate=dates.includes(state.modal?.detailDate)?state.modal.detailDate:comparisonSummary?.peak?.date||dates[0]||null;
  const engineCards=FORECAST_ENGINES.map(engine=>{const key=forecastEngineUi(engine).key,active=engine===selected;return `<article class="forecast-engine-overview-card ${active?'selected-engine':''}"><div class="forecast-engine-overview-head"><strong>${esc(t(`forecastEngine${key}`))}</strong>${active?`<span>${esc(t('forecastEngineSelected'))}</span>`:''}</div><p>${esc(t(`forecastEngine${key}Desc`))}</p></article>`;}).join('');
  const detailFor=(agg,key)=>agg?.engineDetails?.[key]||null;
  const cell=(engine,main,{rangeDetail=null,metaDetail=rangeDetail,sub=''}={})=>{const active=engine===selected,meta=forecastEngineDetailMeta(metaDetail),ranges=forecastEngineIntervalMeta(rangeDetail,main?.unit||'',main?.digits??1);return `<td class="forecast-engine-result ${active?'selected-engine':''}"><strong>${main?.text??'—'}</strong>${sub?`<span>${sub}</span>`:''}${ranges}${meta?`<small class="forecast-engine-result-meta">${esc(meta)}</small>`:''}</td>`;};
  const rowsForDate=date=>{
    ...
    return [
      row(t('tempMinimum'),'tempMin',(engine,agg,detail)=>cell(engine,{text:Number.isFinite(agg.tempMin)?`${fmt(agg.tempMin,1)} °C`:'—',unit:' °C',digits:1},{rangeDetail:detail})),
      row(t('tempMaximum'),'tempMax',(engine,agg,detail)=>cell(engine,{text:Number.isFinite(agg.tempMax)?`${fmt(agg.tempMax,1)} °C`:'—',unit:' °C',digits:1},{rangeDetail:detail})),
      row(t('timelineRainProbabilityShort'),'precipitation',(engine,agg)=>cell(engine,{text:Number.isFinite(agg.precipProbability)?`${Math.round(agg.precipProbability)} %`:'—',unit:' %',digits:0})),
      row(t('summaryRainExpectedLabel'),'precipitation',(engine,agg,detail)=>{const value=forecastEngineMetricValue(agg,'precipExpected');return cell(engine,{text:Number.isFinite(value)?`${fmt(value,1)} mm`:'—',unit:' mm',digits:1},{rangeDetail:detail});}),
      row(t('summaryRainIfWetLabel'),'precipitation',(engine,agg,detail)=>cell(engine,{text:Number.isFinite(agg.precipConditional)?esc(t('rainIfWetAmountShort',{amount:fmt(agg.precipConditional,1)})):'—',unit:' mm',digits:1},{rangeDetail:detail?.conditionalInterval?{interval:detail.conditionalInterval}:null,metaDetail:null})),
      row(t('wind'),'wind',(engine,agg,detail)=>cell(engine,{text:Number.isFinite(agg.wind)?`${fmt(agg.wind,0)} km/h`:'—',unit:' km/h',digits:0},{rangeDetail:detail})),
      row(t('gusts'),'gust',(engine,agg,detail)=>cell(engine,{text:Number.isFinite(agg.gust)?`${fmt(agg.gust,0)} km/h`:'—',unit:' km/h',digits:0},{rangeDetail:detail})),
      row(t('cloudCoverage'),'cloud',(engine,agg,detail)=>cell(engine,{text:Number.isFinite(agg.cloud)?`${fmt(agg.cloud,0)} %`:'—',unit:' %',digits:0},{rangeDetail:detail})),
  ...
  const dayTable=detailDate?`<section class="forecast-engine-day" id="forecast-engine-detail-panel" role="tabpanel"><div class="forecast-engine-day-head"><h3>${esc(dateLabel(detailDate,i18n().locale,'long'))}</h3><span>${esc(t('forecastEngineDetailSelectedDay'))}</span></div><div class="table-wrap forecast-engine-table-wrap"><table class="forecast-engine-table"><thead><tr><th>${esc(t('forecastEngineVariable'))}</th>${FORECAST_ENGINES.map(engine=>`<th class="${engine===selected?'selected-engine':''}">${esc(forecastEngineName(engine))}${engine===selected?`<span class="forecast-engine-selected-pill">${esc(t('forecastEngineSelected'))}</span>`:''}</th>`).join('')}</tr></thead><tbody>${rowsForDate(detailDate)}</tbody></table></div></section>`:'';
  const chartConfigs={tempMax:{key:'tempMax',detailKey:'tempMax',label:t('forecastEngineTempTrend'),button:t('tempMaximum'),unit:' °C',digits:1},tempMin:{key:'tempMin',detailKey:'tempMin',label:t('tempMinimum'),button:t('tempMinimum'),unit:' °C',digits:1},precipProbability:{key:'precipProbability',detailKey:null,label:t('forecastEngineRainProbabilityTrend'),button:t('timelineRainProbabilityShort'),unit:' %',digits:0},precipExpected:{key:'precipExpected',detailKey:'precipitation',label:t('forecastEngineRainTrend'),button:t('timelineRainAmountShort'),unit:' mm',digits:1},wind:{key:'wind',detailKey:'wind',label:t('forecastEngineWindTrend'),button:t('wind'),unit:' km/h',digits:0},gust:{key:'gust',detailKey:'gust',label:t('gusts'),button:t('gusts'),unit:' km/h',digits:0},cloud:{key:'cloud',detailKey:'cloud',label:t('cloudCoverage'),button:t('cloudCoverage'),unit:' %',digits:0}};
  ...
  return `${renderForecastEngineComparisonSummary(dates,matrix)}<div class="forecast-engine-overview">${engineCards}</div><section class="forecast-engine-visual-section"><div class="forecast-engine-visual-head"><div><h3>${esc(t('forecastEngineVisualOverview'))}</h3><p>${esc(t('forecastEngineVisualOverviewBody'))}</p></div></div>${chartSelector}${visualChart}${renderForecastEngineDivergenceTimeline(dates,matrix,detailDate)}</section><section class="forecast-engine-comparison-days"><div class="forecast-engine-comparison-title"><div><h3>${esc(t('forecastEngineInspectDetails'))}</h3><p>${esc(t('forecastEngineDailyOutlook'))}</p></div><span>${esc(city.name)}${detailDate?` · ${esc(forecastEngineShortDate(detailDate))}`:''}</span></div>${dayTable||`<div class="empty-state compact">${esc(t('insufficientData'))}</div>`}</section>`;
}
...
function chartHoverKeyLabel(key,mode,dateStyle='DEFAULT'){
  if(!key)return '';
  if(dateStyle==='FULL_DATE')return biasFullDateLabel(String(key).slice(0,10),true);
...
function chartHoverContext(svg){
  ...
  try{
    ...
    hover={keys,values,shell,crosshair:svg.querySelector('[data-hover-crosshair]'),markers:[...svg.querySelectorAll('[data-hover-marker]')],seriesItems:[],status:shell?.querySelector('[data-hover-status]')||null,lastIndex:-1};
    if(svg.dataset.hoverChart!=='agreement-band')hover.seriesItems=values.map((_,si)=>shell?.querySelector(`[data-hover-series="${si}"]`)||null);
    if(svg.dataset.hoverChart==='bias-history'){hover.gaps=JSON.parse(svg.dataset.hoverGaps||'[]');hover.gapEl=shell?.querySelector('[data-bias-hover-gap]')||null;}
    else hover.bandEls={mean:shell?.querySelector('[data-band-hover-mean]')||null,range:shell?.querySelector('[data-band-hover-range]')||null,probable:shell?.querySelector('[data-band-hover-probable]')||null,retained:shell?.querySelector('[data-band-hover-retained]')||null,agreement:shell?.querySelector('[data-band-hover-agreement]')||null,models:shell?.querySelector('[data-band-hover-models]')||null,rain:shell?.querySelector('[data-band-hover-rain]')||null};
    chartHoverDataCache.set(svg,hover);return hover;
  }catch{return null;}
}
function handleChartPointerMoveScheduled(e){
  if(e.pointerType==='touch'||!e.target?.closest?.('svg[data-hover-chart]'))return;
...
function handleChartPointerMove(e){
  ...
  if(hover.lastIndex===index)return;hover.lastIndex=index;
  const when=chartHoverKeyLabel(keys[index],svg.dataset.hoverMode,svg.dataset.hoverDateStyle),unit=svg.dataset.hoverUnit||'',digits=Number(svg.dataset.hoverDigits)||0;shell.classList.add('is-hovering');if(hover.status)hover.status.textContent=i18n().t('chartHoverAt',{when});
  ...
  if(svg.dataset.hoverChart==='agreement-band'){
    const row=values?.[index]||[],mean=row[0],min=row[1],max=row[2],probableLow=row[3],probableHigh=row[4],retainedLow=row[5],retainedHigh=row[6],percent=row[7],modelCount=row[8],familyCount=row[9],rainProbability=row[10],rainConditional=row[11],els=hover.bandEls||{},formatRange=(low,high)=>Number.isFinite(low)&&Number.isFinite(high)?`${fmt(low,digits)}–${fmt(high,digits)} ${unit}`:'—';
    if(els.mean)els.mean.textContent=Number.isFinite(mean)?`${fmt(mean,digits)} ${unit}`:'—';
    if(els.range)els.range.textContent=formatRange(min,max);
    if(els.probable)els.probable.textContent=formatRange(probableLow,probableHigh);
    if(els.retained)els.retained.textContent=formatRange(retainedLow,retainedHigh);
    ...
    if(els.models)els.models.textContent=[Number.isFinite(familyCount)?`${familyCount} ${i18n().t('familiesShort')}`:'',Number.isFinite(modelCount)?modelCountLabel(modelCount):''].filter(Boolean).join(' · ');
    if(els.rain)els.rain.textContent=[Number.isFinite(rainProbability)?`${i18n().t('timelineRainProbabilityShort')} ${Math.round(rainProbability)}%`:'',Number.isFinite(rainConditional)?`${i18n().t('timelineRainIfWetShort')} ${fmt(rainConditional,1)} mm`:''].filter(Boolean).join(' · ');
  ...
  if(svg.dataset.hoverChart==='bias-history'&&hover.gapEl){const gap=hover.gaps?.[index];hover.gapEl.textContent=Number.isFinite(gap)?`${gap>0?'+':''}${fmt(gap,digits)} ${unit}`:'—';}
}
function clearChartHover(svg){
  const hover=svg?chartHoverContext(svg):null,shell=hover?.shell||svg?.closest?.('.hover-chart-shell');if(!shell)return;if(hover)hover.lastIndex=-1;shell.classList.remove('is-hovering');if(hover?.status)hover.status.textContent=i18n().t('chartHoverHint');else{const status=shell.querySelector('[data-hover-status]');if(status)status.textContent=i18n().t('chartHoverHint');}(hover?.markers||[...svg.querySelectorAll('[data-hover-marker]')]).forEach(el=>el.classList.remove('active'));shell.querySelectorAll('[data-hover-value]').forEach(el=>{el.textContent='—';});
  if(svg?.dataset?.hoverChart==='agreement-band'){const els=hover?.bandEls||{};if(els.mean)els.mean.textContent='—';if(els.range)els.range.textContent='—';if(els.probable)els.probable.textContent='—';if(els.retained)els.retained.textContent='—';if(els.agreement){els.agreement.textContent='—';els.agreement.classList.remove('high','medium','low');}if(els.models)els.models.textContent='';if(els.rain)els.rain.textContent='';}
...
function graphicRulerContext(plots){
  let context=graphicRulerContextCache.get(plots);if(context)return context;
  const track=plots.closest('[data-graphic-ruler-track]'),ruler=plots.querySelector('[data-graphic-ruler]'),scroller=plots.closest('.graphic-scroll');if(!track||!ruler||!scroller)return null;
  context={track,ruler,scroller,label:ruler.querySelector('[data-graphic-ruler-label]'),summary:{temp:ruler.querySelector('[data-graphic-ruler-temp]'),rain:ruler.querySelector('[data-graphic-ruler-rain]'),wind:ruler.querySelector('[data-graphic-ruler-wind]'),gust:ruler.querySelector('[data-graphic-ruler-gust]')},hours:[...track.querySelectorAll('[data-graphic-hour-index]')].map(hour=>({date:hour.dataset.graphicRulerDate||'',hour:hour.dataset.graphicRulerHour||'',temp:hour.dataset.graphicRulerTemp||'—',rain:hour.dataset.graphicRulerRain||'—',wind:hour.dataset.graphicRulerWind||'—',gust:hour.dataset.graphicRulerGust||'—'})),viewportLeft:null,lastIndex:-1};graphicRulerContextCache.set(plots,context);return context;
}
function handleGraphicRulerPointerMoveScheduled(e){
  if(!isGraphicForecastRoute()||e.pointerType==='touch')return;const plots=e.target?.closest?.('.graphic-plots');if(!plots||!app.contains(plots))return;
  pendingGraphicRulerEvent={plots,clientX:e.clientX};if(graphicRulerFrame)return;graphicRulerFrame=requestAnimationFrame(()=>{graphicRulerFrame=0;const pending=pendingGraphicRulerEvent;pendingGraphicRulerEvent=null;if(!pending?.plots?.isConnected)return;const {plots,clientX}=pending,context=graphicRulerContext(plots);if(!context)return;const {track,ruler,scroller,label}=context,leftPad=Number(track.dataset.graphicLeftPad)||0,step=Number(track.dataset.graphicStep)||1,count=Math.max(0,Number(track.dataset.graphicCount)||0);if(context.viewportLeft==null)context.viewportLeft=scroller.getBoundingClientRect().left;const localX=clientX-context.viewportLeft+scroller.scrollLeft,minX=leftPad,maxX=leftPad+Math.max(0,count-1)*step+step;if(!count||localX<minX||localX>maxX){ruler.hidden=true;context.lastIndex=-1;return;}const index=Math.max(0,Math.min(count-1,Math.round((localX-leftPad-step/2)/step)));if(context.lastIndex!==index){const x=leftPad+index*step+step/2,slot=context.hours[index]||{};context.lastIndex=index;ruler.style.setProperty('--graphic-ruler-x',`${x}px`);if(label)label.textContent=[slot.date,slot.hour].filter(Boolean).join(' · ');if(context.summary.temp)context.summary.temp.textContent=slot.temp||'—';if(context.summary.rain)context.summary.rain.textContent=slot.rain||'—';if(context.summary.wind)context.summary.wind.textContent=slot.wind||'—';if(context.summary.gust)context.summary.gust.textContent=slot.gust||'—';}ruler.classList.toggle('badge-left',clientX>(document.documentElement.clientWidth||window.innerWidth)-310);ruler.hidden=false;});
}
function handleGraphicRulerPointerOut(e){
  if(!isGraphicForecastRoute())return;
  const plots=e.target?.closest?.('.graphic-plots');if(!plots||!app.contains(plots)||e.pointerType==='touch')return;if(e.relatedTarget&&plots.contains(e.relatedTarget))return;if(graphicRulerFrame){cancelAnimationFrame(graphicRulerFrame);graphicRulerFrame=0;}pendingGraphicRulerEvent=null;const ruler=plots.querySelector('[data-graphic-ruler]');if(ruler)ruler.hidden=true;const context=graphicRulerContextCache.get(plots);if(context)context.lastIndex=-1;
}
function handleGraphicTooltipPointerOver(e){
  if(!isGraphicForecastRoute()||e.pointerType==='touch')return;const target=graphicTooltipTarget(e.target);if(!target||!app.contains(target))return;if(e.relatedTarget&&target.contains(e.relatedTarget))return;showGraphicTooltip(target,{pinned:false});
}
function handleGraphicTooltipPointerOut(e){
  if(!isGraphicForecastRoute()||e.pointerType==='touch'||graphicTooltipState.pinned)return;const target=graphicTooltipTarget(e.target);if(!target||!app.contains(target))return;if(e.relatedTarget&&target.contains(e.relatedTarget))return;if(graphicTooltipState.target===target)clearGraphicTooltip();
}
function handleGraphicTooltipFocusIn(e){if(!isGraphicForecastRoute())return;const target=graphicTooltipTarget(e.target);if(target&&app.contains(target))showGraphicTooltip(target,{pinned:false});}
function handleGraphicTooltipFocusOut(e){if(!isGraphicForecastRoute()||graphicTooltipState.pinned)return;const target=graphicTooltipTarget(e.target);if(!target||graphicTooltipState.target!==target)return;if(e.relatedTarget&&target.contains(e.relatedTarget))return;clearGraphicTooltip();}
...
function clearGraphicTooltip(){
  ...
  if(layer){layer.hidden=true;layer.classList.remove('is-pinned');layer.removeAttribute('data-placement');const floating=layer.querySelector('.graphic-floating-tooltip');if(floating){floating.replaceChildren();floating.removeAttribute('style');floating.removeAttribute('id');floating.className='graphic-floating-tooltip';}}
}
...
function showGraphicTooltip(target,{pinned=false}={}){
  ...
  const key=target.dataset.graphicTooltipKey||'',cached=key?graphicTooltipContentCache.get(key):null,layer=graphicTooltipLayer(),floating=layer?.querySelector?.('.graphic-floating-tooltip');if(!cached||!layer||!floating)return;
  ...
  floating.className=`graphic-floating-tooltip graphic-tooltip-${cached.type}`;
  if(cached.html==null)cached.html=graphicTooltipMarkup(cached.type,cached.point);floating.innerHTML=cached.html;
  if(key)floating.id=key;else floating.removeAttribute('id');
...
function openGraphicTooltip(target){showGraphicTooltip(target,{pinned:true});}
function resetGraphicInteractions(){
  clearGraphicTooltip();
  graphicTooltipContentCache.clear();
  if(graphicRulerFrame){cancelAnimationFrame(graphicRulerFrame);graphicRulerFrame=0;}
  pendingGraphicRulerEvent=null;
}
function handleGraphicTooltipScroll(){if(graphicTooltipState.target)scheduleGraphicTooltipPosition();}
function handleGraphicTooltipViewportChange(){const plots=app?.querySelector?.('.graphic-plots'),context=plots?graphicRulerContextCache.get(plots):null;if(context)context.viewportLeft=null;if(graphicTooltipState.target)scheduleGraphicTooltipPosition();}
...
function handleGlobalKeydown(e){
  ...
  if((e.key==='Enter'||e.key===' ')&&e.target?.matches?.('[data-action="open-graphic-view"][role="button"]')){e.preventDefault();const cityId=e.target.dataset.graphicCityId||state.route.id,city=state.cities.find(row=>row.id===cityId);if(city)openGraphicForecastView(city,e.target);return;}
...
function handleAppInput(e){
  ...
  else if(e.target?.hasAttribute?.('data-graphic-agreement-toggle')){state.graphicAgreementVisible=Boolean(e.target.checked);e.target.closest?.('.graphic-view')?.classList?.toggle?.('show-agreement',state.graphicAgreementVisible);}
...
function handleAppClick(e){
  ...
  if(target.dataset.timelineLayout){state.settings.timelineLayout=target.dataset.timelineLayout;persistSettings();syncCityViewUrl();void trackAnalyticsEvent('Forecast View Changed',state.route,{control:'timeline-layout',value:target.dataset.timelineLayout});rerenderCitySectionOrPage('timeline');return;}
  ...
  if(target.dataset.engineChartVariable&&state.modal?.type==='forecastEngines'){const variable=target.dataset.engineChartVariable,allowed=['tempMax','tempMin','precipProbability','precipExpected','wind','gust','cloud'];if(!allowed.includes(variable))return;state.modal.chartVariable=variable;rerenderForecastEngineModalPreservingView(forecastEngineModalControlSelector('data-engine-chart-variable',variable));return;}
  if(target.dataset.engineDetailDate&&state.modal?.type==='forecastEngines'){const date=target.dataset.engineDetailDate;state.modal.detailDate=date;rerenderForecastEngineModalPreservingView(forecastEngineModalControlSelector('data-engine-detail-date',date));return;}
...
function openGraphicForecastView(city,sourceElement=null){
  if(!city)return;
  const q=new URLSearchParams();q.set('view','graphic');
  const source=sourceElement?.closest?.('.home-temperature-plot,.detail-chrono-temp-plot')||sourceElement||null;
  void trackAnalyticsEvent('Graphic Forecast Opened',state.route,{source:source?.classList?.contains('home-temperature-plot')?'home_temperature_plot':source?.classList?.contains('detail-chrono-temp-plot')?'detail_temperature_plot':'button'});
  go(cityViewUrl(city,q));
}
function handleAction(e){
  ...
  else if(action==='copy-link'){if(state.route.name==='city')syncCityViewUrl();const url=location.href;if(navigator.clipboard?.writeText)navigator.clipboard.writeText(url).then(()=>{void trackAnalyticsEvent('Share Link Copied',state.route,{method:'clipboard'});toast(i18n().t('linkCopied'),{type:'success'});}).catch(()=>{void trackAnalyticsEvent('Share Link Fallback Opened',state.route,{reason:'clipboard_error'});prompt(i18n().t('copyLinkPrompt'),url);});else{void trackAnalyticsEvent('Share Link Fallback Opened',state.route,{reason:'clipboard_unavailable'});prompt(i18n().t('copyLinkPrompt'),url);}}
  ...
  else if(action==='open-graphic-view'){const cityId=e.currentTarget.dataset.graphicCityId||state.route.id,city=state.cities.find(row=>row.id===cityId);if(!city)return;openGraphicForecastView(city,e.currentTarget);}
  ...
  else if(action==='refresh-vigilance'){const id=state.route.id;if(id){void trackAnalyticsEvent('Vigilance Refreshed',state.route);void refreshVigilanceData(id,true,true);}}
  else if(action==='toggle-analytics'){const status=analyticsStatus();setAnalyticsOptOut(!status.optedOut);if(status.optedOut)void trackCurrentPageView(state.route);render({scroll:interactionScrollContext,immediate:true});}
...
function addCityFromSearch(id){
  const city=state.modal?.results?.find(c=>c.id===id);if(!city)return;cancelCitySearch();let target=state.cities.find(c=>c.id===city.id||sameCityPlace(c,city)),becameFavorite=false;
