...
function renderDetailChronoTimeline(points,mode){
  ...
  const axis=points.map((point,index)=>{const ci=localizedConditionInfo(point.condition),context=mode==='HOURLY'?dateLabel(point.date,i18n().locale):'',tooltip=[detailChronoPointLabel(point,mode,index),context,point.condition?ci.label:''].filter(Boolean).join(' · ');return `<div class="detail-chrono-axis-hour" title="${attr(tooltip)}"><span class="detail-chrono-condition-icon">${point.condition?aggregateConditionMarkup(point,'small'):'—'}</span><strong>${esc(detailChronoPointLabel(point,mode,index))}</strong>${context?`<small>${esc(context)}</small>`:''}</div>`;}).join('');
