...
function renderSettings(){
  ...
  return `<main class="page settings-page"><section class="page-header settings-page-header"><div class="page-header-copy"><h1>${esc(t('settings'))}</h1><p>${esc(t('settingsIntro'))}</p></div></section><div class="settings-list settings-list-simple">
    ...
    <section class="settings-section settings-wide"><div class="settings-section-head"><div><h2>${esc(t('weatherModels'))}</h2><p>${esc(t('forecastModelSettingsDesc'))}</p></div></div><div class="segmented">${[['ZONE',t('sortZone')],['FAMILLE',t('sortFamily')],['FINESSE',t('sortResolution')]].map(([id,l])=>`<button class="seg-btn ${sort===id?'active':''}" data-model-sort="${id}">${esc(l)}</button>`).join('')}</div>${groups.map(g=>`${g.label?`<div class="model-group-title">${esc(g.label)}</div>`:''}${g.models.map(renderModelRow).join('')}`).join('')}</section>
  </div></main>`;
}
