...
function renderForecastEngineComparisonModal(cityId){
  ...
  return `<div class="forecast-engine-overview">${engineCards}</div><section class="forecast-engine-visual-section"><div class="forecast-engine-visual-head"><div><h3>${esc(t('forecastEngineVisualOverview'))}</h3><p>${esc(t('forecastEngineVisualOverviewBody'))}</p></div></div>${visualCharts}${renderForecastEngineDivergenceTimeline(dates,matrix)}</section><section class="forecast-engine-comparison-days"><div class="forecast-engine-comparison-title"><div><h3>${esc(t('forecastEngineInspectDetails'))}</h3><p>${esc(t('forecastEngineDailyOutlook'))}</p></div><span>${esc(city.name)}</span></div>${dayTables||`<div class="empty-state compact">${esc(t('insufficientData'))}</div>`}</section>`;
}
