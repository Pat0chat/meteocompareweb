...
async function hydrateRadarModal(){
  ...
  try{
    ...
    const {t,locale}=i18n();await radar.mountRadarModal({root,city,forecast:state.forecasts[cityId]||null,forecastOptions:forecastEngineContext(cityId),t,locale,onRangeChange:range=>void trackAnalyticsEvent('Rain Radar Range Changed',state.route,{range}),onModeChange:mode=>void trackAnalyticsEvent('Rain Radar Mode Changed',state.route,{mode})});
...
function renderHome(){
  ...
  return `<main class="page home-page"><section class="home-hero"><div class="home-hero-main"><div class="home-hero-copy"><span class="home-hero-kicker">${esc(t('homeModernKicker'))}</span><h1>${esc(t('homeModernTitle'))}</h1><p>${esc(t('homeModernLead'))}</p><div class="home-hero-pill-row" aria-label="${esc(t('multiModelForecast'))}"><span>${uiIcon('layers',16)}${esc(t('homeHeroPillModels'))}</span><span>${uiIcon('check',16)}${esc(t('homeHeroPillConvergence'))}</span><span>${uiIcon('info',16)}${esc(t('homeHeroPillDivergence'))}</span></div>${heroActions}</div>${renderForecastExpertiseDisclaimer()}</div><aside class="home-hero-side"><div class="home-hero-search-panel"><strong>${esc(t('homeHeroSearchTitle'))}</strong><p>${esc(t('homeHeroSearchLead'))}</p><button class="home-search-trigger" data-action="open-add-city"><span class="home-search-icon">⌕</span><span>${esc(t('homeSearchPrompt'))}</span><kbd>+</kbd></button></div></aside></section>${hasCities?`<div class="home-dashboard"><section class="home-cities-section">${columnHeading(t('homeFavoritesKicker'),t('cities'))}<div class="home-city-grid" aria-label="${esc(t('cities'))}">${cards}</div></section><div class="home-watch-column">${columnHeading(t('homeWatchKicker'),t('homeWatchTitle'))}${renderHomeWatchlist()}</div></div>`:`<section class="empty-state home-empty"><div class="big home-empty-logo-wrap"><img class="home-empty-logo" src="${attr(appAssetUrl('assets/icon.png'))}" alt="" aria-hidden="true"></div><h2>${esc(t('emptyTitle'))}</h2><p>${esc(t('emptyBody'))}</p><button class="btn primary" data-action="open-add-city">＋ ${esc(t('addCity'))}</button></section>`}${renderSeoCityDirectory()}</main>`;
}
...
function renderModal(){
  ...
  if(state.modal.type==='radar'){const c=state.cities.find(x=>x.id===state.modal.cityId);return `<div class="modal-backdrop" data-action="modal-backdrop"><div class="modal modal-wide radar-modal" role="dialog" aria-modal="true" aria-labelledby="modal-title"><div class="modal-head radar-modal-head"><div><h2 id="modal-title">${esc(t('rainRadar'))}</h2><span class="small">${esc(c?.name||t('city'))} · ${esc(t('radarSubtitle'))}</span></div><button class="icon-btn" data-action="close-modal" aria-label="${esc(t('close'))}">×</button></div><div class="modal-content radar-modal-content" data-radar-root data-radar-mode="observation"><div class="radar-toolbar"><div class="radar-toolbar-controls"><div class="radar-mode-group" aria-label="${esc(t('radarMode'))}"><button class="seg-btn active" data-radar-mode="observation" aria-pressed="true">${esc(t('radarModeObservation'))}</button><button class="seg-btn" data-radar-mode="projection" aria-pressed="false">${esc(t('radarModeProjection'))}</button></div><div class="radar-range-group" aria-label="${esc(t('radarRange'))}"><button class="seg-btn active" data-radar-range="near" aria-pressed="true">${esc(t('radarRangeNear'))}</button><button class="seg-btn" data-radar-range="regional" aria-pressed="false">${esc(t('radarRangeRegional'))}</button><button class="seg-btn" data-radar-range="wide" aria-pressed="false">${esc(t('radarRangeWide'))}</button></div></div><div class="radar-status" data-radar-status>${esc(t('radarLoading'))}</div></div><div class="radar-map-stage" data-radar-stage><div class="radar-base-layer" data-radar-base aria-hidden="true"></div><img class="radar-precip-layer" data-radar-image alt="" draggable="false"><canvas class="radar-nowcast-layer" data-radar-nowcast aria-hidden="true"></canvas><div class="radar-map-shade" aria-hidden="true"></div><div class="radar-nowcast-badge">${esc(t('radarNowcastBadge'))}</div><div class="radar-center-marker" aria-label="${esc(t('radarCenterLocality',{city:c?.name||t('city')}))}"><span></span><strong>${esc(c?.name||t('city'))}</strong></div><div class="radar-compass" aria-hidden="true">N</div><div class="radar-attribution"><a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener noreferrer">© OpenStreetMap</a> · <a href="https://www.rainviewer.com/" target="_blank" rel="noopener noreferrer">RainViewer</a></div></div><div class="radar-legend"><div class="radar-intensity-legend"><span>${esc(t('radarIntensity'))}</span><div class="radar-intensity-scale"><div class="radar-gradient" aria-hidden="true"></div><div class="radar-intensity-labels"><small>${esc(t('radarLight'))}</small><small>${esc(t('radarModerate'))}</small><small>${esc(t('radarHeavy'))}</small></div></div></div><div class="radar-projection-legend"><span>${esc(t('radarProjectionReading'))}</span><span class="radar-zone-key probable"><i></i>${esc(t('radarProbableZone'))}</span><span class="radar-zone-key forecast"><i></i>${esc(t('radarForecastZone'))}</span><span class="radar-zone-key trajectory"><i></i>${esc(t('radarTrajectory'))}</span></div><div class="radar-horizon-legend" aria-label="${esc(t('radarProjectionHorizons'))}"><i class="p15"></i><small>+15</small><i class="p30"></i><small>+30</small><i class="p45"></i><small>+45</small><i class="p60"></i><small>+60 min</small></div></div><div class="radar-playback" data-radar-observation-controls><button class="radar-play-button" data-radar-play aria-label="${esc(t('radarPlay'))}" title="${esc(t('radarPlay'))}">▶</button><div class="radar-time-block"><strong data-radar-time>—</strong><span>${esc(t('radarObserved'))}</span></div><input class="radar-slider" data-radar-slider type="range" min="0" max="0" value="0" aria-label="${esc(t('radarTimeline'))}"></div><div class="radar-projection-info" data-radar-projection-controls><div><strong>${esc(t('radarProjectionTitle'))}</strong><span>${esc(t('radarProjectionLead'))}</span></div><div class="radar-projection-horizons"><span>+15</span><span>+30</span><span>+45</span><span>+60 min</span></div></div><div class="radar-nowcast-summary loading" data-radar-nowcast-summary><span class="loader"></span><span>${esc(t('radarNowcastAnalyzing'))}</span></div><section class="radar-nowcast-panel"><div class="radar-panel-head"><h3>${esc(t('radarNextHours'))}</h3><span class="radar-model-badge">${esc(t('multiModelForecast'))}</span></div><div data-radar-forecast></div></section><p class="small radar-privacy-note">${esc(t('radarPrivacyNote'))}</p></div></div></div>`;}
...
function addCityFromSearch(id){
  ...
  state.modal=null;render();void checkMarineCapability(target.id);refreshCity(target.id,true);
}
