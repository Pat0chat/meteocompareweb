...
function renderCityComparisonMetric(cities,metric){
  ...
  const width=760,height=230,pad={l:50,r:18,t:18,b:38},min=metric==='PRECIPITATION'||metric==='WIND'||metric==='AGREEMENT'?0:Math.min(...all),max=metric==='AGREEMENT'?100:Math.max(...all),span=Math.max(1,max-min),x=i=>pad.l+i*(width-pad.l-pad.r)/Math.max(1,dates.length-1),y=v=>pad.t+(max-v)*(height-pad.t-pad.b)/span,colors=['#246bfd','#00a3a3','#8b5cf6'];
  const paths=rows.map((r,ri)=>`<polyline class="compare-line" style="--series:${colors[ri%colors.length]}" points="${r.values.map((v,i)=>Number.isFinite(v)?`${x(i)},${y(v)}`:'').filter(Boolean).join(' ')}"/>`).join('');
  const labels=dates.map((d,i)=>`<text class="compare-label" x="${x(i)}" y="${height-12}" text-anchor="middle">${esc(dateLabel(d,i18n().locale))}</text>`).join('');
  return `<div class="city-compare-chart"><svg viewBox="0 0 ${width} ${height}" role="img"><line class="compare-axis" x1="${pad.l}" y1="${height-pad.b}" x2="${width-pad.r}" y2="${height-pad.b}"/>${paths}${labels}</svg><div class="compare-legend">${rows.map((r,i)=>`<span><i style="--series:${colors[i%colors.length]}"></i>${esc(r.city.name)}</span>`).join('')}</div></div>`;
}
...
function renderBandChart(bands,metric,normals){
  ...
  return `<svg class="chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="${esc(i18n().t('agreementBandAria'))}">${grid}${rangeSegments}<polyline class="chart-range-edge" points="${upper}"/><polyline class="chart-range-edge" points="${lower}"/><polyline class="chart-line" points="${mean}"/>${normalsSvg}${xlabels}</svg>`;
}
...
function renderTargetedComparisonChart(f,ids,tab,mode){
  const {t}=i18n(),metric=comparisonMetricForTab(tab),metricLabel=t(metric==='TEMPERATURE'?'comparisonMetricTemperature':metric==='PRECIPITATION'?'comparisonMetricPrecipitation':'comparisonMetricWind'),unit=metric==='TEMPERATURE'?'°C':metric==='PRECIPITATION'?'mm':'km/h',series=ids.map(id=>({id,values:comparisonSeries(f,id,tab,mode)})).filter(x=>x.values.length>1);if(series.length<2)return '';
  const keys=[...new Set(series.flatMap(x=>x.values.map(v=>v.key)))].sort(),all=series.flatMap(x=>x.values.map(v=>v.value));if(keys.length<2||!all.length)return '';const width=900,height=220,p={l:46,r:16,t:18,b:32},ymin=Math.min(...all),ymax=Math.max(...all),span=Math.max(.5,ymax-ymin),lo=ymin-span*.08,hi=ymax+span*.08,x=i=>p.l+i*(width-p.l-p.r)/(keys.length-1),y=v=>p.t+(hi-v)*(height-p.t-p.b)/(hi-lo),colors=['#2563eb','#0f9f8f','#7c3aed','#e07a19'],lines=series.map((row,si)=>{const by=new Map(row.values.map(v=>[v.key,v.value])),pts=keys.map((k,i)=>Number.isFinite(by.get(k))?`${x(i)},${y(by.get(k))}`:null).filter(Boolean);return `<polyline class="compare-line" style="--series:${colors[si]}" points="${pts.join(' ')}"/>`;}).join(''),legend=series.map((row,si)=>`<span><i style="--series:${colors[si]}"></i>${esc(getModel(row.id)?.name||row.id)}</span>`).join('');
  return `<div class="target-compare-chart"><div class="target-compare-title"><strong>${esc(t('directComparison',{metric:metricLabel}))}</strong><span>${esc(mode==='HOURLY'?t('first48Hours'):t('dayMode7'))} · ${unit}</span></div><svg viewBox="0 0 ${width} ${height}" role="img" aria-label="${attr(t('compareModelsAria',{count:series.length}))}">${lines}<line class="compare-axis" x1="${p.l}" y1="${height-p.b}" x2="${width-p.r}" y2="${height-p.b}"/></svg><div class="compare-legend">${legend}</div></div>`;
}
...
function renderBiasHistoryChart(samples,variable){
  ...
  const width=960,height=300,pad={l:54,r:20,t:22,b:46},values=samples.flatMap(x=>[x.forecast,x.observation]);let ymin=Math.min(...values),ymax=Math.max(...values);if(variable!=='TEMPERATURE')ymin=0;const extra=variable==='TEMPERATURE'?1:variable==='PRECIPITATION'?.5:3;ymin-=variable==='TEMPERATURE'?extra:0;ymax+=extra;if(ymax-ymin<1)ymax=ymin+1;const x=i=>pad.l+i*(width-pad.l-pad.r)/Math.max(1,samples.length-1),y=v=>pad.t+(ymax-v)*(height-pad.t-pad.b)/(ymax-ymin);let grid='',labels='';for(let i=0;i<=4;i++){const value=ymin+(ymax-ymin)*i/4,yy=y(value);grid+=`<line class="bias-chart-grid" x1="${pad.l}" y1="${yy}" x2="${width-pad.r}" y2="${yy}"/><text class="bias-chart-axis" x="${pad.l-8}" y="${yy+4}" text-anchor="end">${fmt(value,variable==='PRECIPITATION'?1:0)}</text>`;}const every=Math.max(1,Math.floor(samples.length/5));samples.forEach((s,i)=>{if(i%every===0||i===samples.length-1)labels+=`<text class="bias-chart-axis" x="${x(i)}" y="${height-14}" text-anchor="middle">${esc(s.date.slice(5))}</text>`;});const forecast=samples.map((s,i)=>`${x(i)},${y(s.forecast)}`).join(' '),observation=samples.map((s,i)=>`${x(i)},${y(s.observation)}`).join(' ');
  return `<div class="bias-chart-shell"><div class="chart-legend"><span><i class="legend-line bias-forecast"></i>${esc(t('forecastD1'))}</span><span><i class="legend-line bias-observation"></i>${esc(t('observation'))}</span></div><svg class="bias-chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="${attr(`${t('biasHistoryTitle')} · ${biasVariableLabel(variable)}`)}">${grid}<polyline class="bias-chart-observation" points="${observation}"/><polyline class="bias-chart-forecast" points="${forecast}"/>${labels}</svg></div>`;
}
...
function renderModal(){
  ...
  if(state.modal.type==='donate')return `<div class="modal-backdrop" data-action="modal-backdrop"><div class="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title"><div class="modal-head"><h2 id="modal-title">♡ ${esc(t('supportTitle'))}</h2><button class="icon-btn" data-action="close-modal" aria-label="${esc(t('close'))}">×</button></div><div class="modal-content"><p>${esc(t('supportBody'))}</p><div class="grid"><a class="btn tonal" href="https://liberapay.com/Pat0chat" target="_blank" rel="noopener">💝 Liberapay</a><a class="btn tonal" href="https://github.com/sponsors/Pat0chat" target="_blank" rel="noopener">❤️ GitHub Sponsors</a><a class="btn tonal" href="https://ko-fi.com/pat0chat" target="_blank" rel="noopener">☕ Ko-Fi</a></div></div></div></div>`;
  return '';
}
