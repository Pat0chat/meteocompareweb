...
function renderHomeWeatherTimeline(points,chartKey='home'){
  ...
  return `<div class="home-weather-timeline-wrap"><div class="home-weather-timeline-scroll" role="region" tabindex="0" aria-label="${esc(t('homeMiniTimelineAria'))}"><div class="home-weather-timeline" style="--home-timeline-cols:${points.length};--home-timeline-step:${step}px"><svg class="home-temperature-plot" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" role="img" aria-label="${esc(t('temperature'))}"><defs><linearGradient id="${attr(gradientId)}" x1="0" x2="1" y1="0" y2="0">${gradientStops}</linearGradient></defs>${columns}<line class="home-temperature-baseline" x1="0" y1="${plotBottom}" x2="${width}" y2="${plotBottom}"/>${area?`<path class="home-temperature-area" d="${area}" fill="url(#${attr(gradientId)})"/>`:''}${path?`<path class="home-temperature-line" d="${path}" stroke="url(#${attr(gradientId)})"/>`:''}${pointMarks}</svg><div class="home-weather-axis" aria-hidden="true">${hourAxis}</div><div class="home-weather-event-lane"><div class="home-weather-rain-track" style="--home-timeline-cols:${points.length}">${rainHours}</div></div></div></div></div>`;
}
...
function renderDetailChronoTimeline(points,mode){
  ...
  return `<div class="detail-chrono-view" data-chrono-mode="${attr(mode)}" style="--detail-chrono-cols:${points.length};--detail-chrono-step:${step}px">${labels}<div class="detail-chrono-scroll" role="region" tabindex="0" aria-label="${attr(t('timelineChronoView'))}"><div class="detail-chrono-canvas"><div class="detail-chrono-date">${dates}</div><svg class="detail-chrono-temp-plot" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" role="img" aria-label="${attr(t('temperature'))}"><defs><linearGradient id="${attr(gradientId)}" x1="0" x2="1" y1="0" y2="0">${stops}</linearGradient></defs>${heat}<line class="detail-chrono-temp-baseline" x1="0" y1="${plotBottom}" x2="${width}" y2="${plotBottom}"/>${area?`<path class="detail-chrono-temp-area" d="${area}" fill="url(#${attr(gradientId)})"/>`:''}${path?`<path class="detail-chrono-temp-line" d="${path}" stroke="url(#${attr(gradientId)})"/>`:''}${tempMarks}</svg><div class="detail-chrono-axis">${axis}</div><div class="detail-chrono-lane detail-chrono-rain">${rain}</div><div class="detail-chrono-lane detail-chrono-cloud">${cloud}</div><div class="detail-chrono-lane detail-chrono-wind">${wind}</div><div class="detail-chrono-lane detail-chrono-agreement">${agreement}</div></div></div></div>`;
}
...
function renderTimeline(f,engineContext=null){
  ...
  return `<section class="section timeline-section" id="timeline"><div class="section-card timeline-card timeline-layout-${layout.toLowerCase()}"><div class="section-head"><div><h2>${esc(t('forecastTimeline'))}</h2><p>${esc(t(mode==='HOURLY'?'next24Regular':'nextDaysConsensus'))}</p></div>${controls}</div>${layout==='CHRONO'?renderDetailChronoTimeline(points,mode):columns}</div></section>`;
}
