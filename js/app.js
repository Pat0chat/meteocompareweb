...
function renderGraphicForecastView(cityId){
  ...
  const step=54,leftPad=148,rightPad=34,totalWidth=Math.round(leftPad+points.length*step+rightPad),chartHeight=620,tempTop=82,tempBottom=350,rainTop=396,rainBottom=548,windTop=404,windBottom=538;
  ...
  const axisMarkup=(kind,label,unit,ticks,y,formatter,start,end,icon)=>`<div class="graphic-y-axis ${kind}" style="--axis-start:${(start/chartHeight*100).toFixed(3)}%;--axis-end:${(end/chartHeight*100).toFixed(3)}%"><div class="graphic-axis-heading"><span class="graphic-axis-icon">${icon}</span><span><strong>${esc(label)}</strong><small>${esc(unit)}</small></span></div><div class="graphic-axis-rail" aria-hidden="true"></div>${[...ticks].reverse().map(value=>`<span class="graphic-axis-tick" style="top:${(y(value)/chartHeight*100).toFixed(3)}%"><em>${esc(formatter(value))}</em><i aria-hidden="true"></i></span>`).join('')}</div>`;
  const yLabels=`${axisMarkup('temp',t('temperature'),'°C',tempTicks,yTemp,value=>`${fmt(value)}°`,tempTop,tempBottom,weatherIcons.renderMetric('temperature',{size:'tiny'}))}${axisMarkup('rain',t('widget_metric_precipitation'),'mm/h',rainAxis.ticks,yRain,value=>fmt(value,value<1?1:0),rainTop,rainBottom,weatherIcons.renderMetric('precipitation',{size:'tiny'}))}${axisMarkup('wind',t('wind'),'km/h',windAxis.ticks,yWind,value=>fmt(value,0),windTop,windBottom,weatherIcons.renderMetric('wind',{size:'tiny'}))}`;
  ...
  const timeAxis=points.map((point,index)=>`<div class="graphic-hour ${point.date!==points[index-1]?.date?'day-start':''}" style="left:${(x(index)-step/2).toFixed(1)}px;width:${step}px"><strong>${esc(timeLabel(point.timestamp))}</strong></div>`).join('');
