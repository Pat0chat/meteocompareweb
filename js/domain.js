...
export function aggregateDay(forecast,date,options={}){
  ...
  const precipRows=data.filter(x=>x.comparable.precipitation).map(x=>({modelId:x.modelId,amount:x.precip,probability:x.precipProb})),precip=precipitationConsensus(precipRows,{threshold:1,localWeights:weights.precipitation||{},amountTight:1,amountWide:8}),cloudConsensus=continuousConsensus(entries('cloud'),{},10,50);
