...
assert.match(app,/<\/nav><\/div>\$\{renderForecastEngineCompareAction\(\)\}<button class="detail-back-button detail-sidebar-back"/,'engine comparison must sit between the sidebar navigation card and back action');
assert.match(app,/overview-secondary">\$\{renderGlobalAgreementCard\([^}]+\)\}\$\{renderScenarios\(/,'overview secondary keeps convergence followed by scenarios');
...
console.log('tests/detail/regression/styles.hierarchy.test.mjs: OK');
