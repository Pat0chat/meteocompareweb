...
assert.match(home,/home-weather-timeline-legend/);
assert.match(home,/home-temperature-scale/);
assert.match(home,/homeRainLegendDetail/);
assert.doesNotMatch(home,/home-weather-event-title/);
...
assert.match(css,/\.home-weather-heat-column\s*\{[^}]*opacity:/s);
assert.match(css,/\.home-temperature-line\s*\{[^}]*stroke-width:3/s);
assert.match(css,/\.home-weather-rain-hour\.is-wet\s*\{[^}]*background:linear-gradient/s);
assert.match(css,/\.home-weather-rain-hour\.is-wet\s*\{[^}]*box-shadow:none;/s,'Wet rain hours must stay flat without a box shadow');
assert.match(css,/\.home-weather-fact-icon\s*\{[^}]*background:color-mix/s,'Home weather facts must use compact metric icons');
assert.match(css,/\.home-temperature-scale>i\s*\{[^}]*linear-gradient\(90deg,#5b6ff9/s);
assert.match(css,/\.home-city-card\s*\{[^}]*contain-intrinsic-size:auto 515px/s);
