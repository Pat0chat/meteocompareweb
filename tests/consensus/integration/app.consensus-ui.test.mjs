...
assert.match(app,/const engineContext=forecastEngineContext\(city\.id\),now=currentConditions\(f,new Date\(\),engineContext\)/,'city cards must use the full selected forecast-engine context');
...
console.log('tests/consensus/integration/app.consensus-ui.test.mjs: OK');
