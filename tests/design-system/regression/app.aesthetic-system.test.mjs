...
const citySpecs=app.slice(app.indexOf('function collapsibleCitySpecs'),app.indexOf('function decorateCollapsibleCard'));
for(const id of ['timeline','agreement','evolution','reliability','details','marine']) assert.ok(citySpecs.includes(`['${id}'`),`missing collapsible spec ${id}`);
for(const id of ['today-summary','diagnostics']) assert.ok(!citySpecs.includes(`['${id}'`),`${id} must stay outside the generic foldable-card system`);
...
console.log('tests/design-system/regression/app.aesthetic-system.test.mjs: OK');
