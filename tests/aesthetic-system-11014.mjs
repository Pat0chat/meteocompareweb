...
for(const id of ['timeline','agreement','evolution','reliability','marine']) assert.ok(citySpecs.includes(`['${id}'`),`missing collapsible spec ${id}`);
for(const id of ['today-summary','details','diagnostics']) assert.ok(!citySpecs.includes(`['${id}'`),`${id} must stay outside the generic foldable-card system`);
