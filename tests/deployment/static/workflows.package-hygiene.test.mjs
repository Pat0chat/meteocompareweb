...
const tests=workflow.indexOf('npm run tests');
const packageStep=workflow.indexOf('Build release archive');
assert.ok(validate>=0&&tests>validate&&packageStep>tests,'release packaging must happen only after tag/version validation and the full test suite');
