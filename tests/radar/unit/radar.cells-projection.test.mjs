...
import { RADAR_PROJECTION_HORIZONS, RADAR_CELL_COLORS, extractRainCells, estimateRainCellMotions, projectRainCell, rainCellContours, rainCellDisplayMode } from '../../../js/features/radar.js';
...
assert.ok(latestCells.every(cell=>rainCellContours(cell).length>=1),'each detected cell must expose at least one vector contour');
assert.ok(rainCellContours(latestCells[0],2).length>=1,'probable-area expansion must remain contour-based');
...
const stationary={...tracked[0],motion:{...tracked[0].motion,vx:.003,vy:.002}};
const stationaryDisplay=rainCellDisplayMode(stationary,4,{minTrajectoryPixels:26});
assert.equal(stationaryDisplay.lowMotion,true,'almost stationary cells must consolidate overlapping lead-time rendering');
const movingDisplay=rainCellDisplayMode(east,4,{minTrajectoryPixels:26});
assert.equal(movingDisplay.lowMotion,false,'meaningfully moving cells must keep a visible trajectory');
